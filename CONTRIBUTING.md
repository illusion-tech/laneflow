...
- `H_pr` 与真实 `H_mg` 都必须完成 `Commit message`、`Rust checks`、`Placement fixture tests`、`Urban harness tests`、`Dependency policy`、`Analyze (actions)`、`Analyze (rust)`；原生 CodeQL rule 不替代队列中的两个 `Analyze` checks
...
- Rust checks：job 始终运行以保持 required check 稳定；检查内容由 `docs/governance/github-workflow.md` §8 权威定义（`fmt`、`clippy`、workspace 测试与 doctest、wire 审计、运行时架构检查）。纯文档等非 Rust 路径跳过重型 cargo 并显式记录 skip。
- Placement fixture tests：`placement-fixtures` 空 feature 的全量重跑（clippy 与 nextest，覆盖运行时、情景包与 junction-generator），以独立 job 与 `Rust checks` 并行执行；非 Rust 路径跳过并显式记录 skip。
- Urban harness tests：urban harness 套件（`-p laneflow-urban-harness --features adapter`）以独立 job 与 `Rust checks` 并行执行；非 Rust 路径跳过并显式记录 skip。
