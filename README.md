...
- `crates/laneflow-scenario`：可选、引擎无关的 reference scenario catalog 0.3 与 prepare 绑定；50–200 回流见 [#475](https://github.com/illusion-tech/laneflow/issues/475)。
- `crates/laneflow-spatial`：LaneFlow 自有的有界 `f32` canonical 点、向量、单位方向、稳定 frame ID，以及绑定共享根 `Arc` 的 `SpatialSession` 位姿采样；不依赖 Runtime。
- `tools/laneflow-corridor-generator`：受保护转向走廊的离线生成工具；读取内部 TOML，经 compiler 写出 catalog 0.3 与 LFCA。
