...
- 外部审阅、复审与 current-head G3 comment 按 `docs/governance/development-gates.md` 第 6 节和 `docs/governance/github-workflow.md` 第 6 节检查；本提示层不复制其状态机、证据字段或 rollout 规则。
...
- 涉及安全设置、扫描或公开发布时，转读 `docs/governance/security-scanning.md`，检查实际配置、最近分析、开放告警和状态语义是否一致。
- 涉及许可证、Cargo 依赖、RustSec、cargo-deny 或 Dependabot 时，转读 `docs/governance/dependency-security.md` 和 ADR 0002，检查开放/商业边界、许可证、来源、漏洞与例外记录。
