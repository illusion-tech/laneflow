...
7. PR 准备合并时，冻结 current exact head `H_pr` 并完成该 head 的 required checks 与适用 CodeQL；
   ...
   `Commit message`、`Rust checks`、`Placement fixture tests`、`Urban harness tests`、`Dependency policy`、`Analyze (actions)`、`Analyze (rust)`；
