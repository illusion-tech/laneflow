...
- content-equivalent rebase、provider / platform outage、security / emergency hotfix 只能走文档定义的显式 waiver；current comment 必须写 `- Gate 结果：G3 Waived` 并提供 `external-review-waiver:v1` 结构化、未过期证据，validator 保持 `waived` 而不映射成 `pass`。confirmed gate defect 不使用 waiver，只能写 `G3 Exception` + `confirmed_gate_defect` 并保持 `accepted_exception` 非成功状态；两类路径都不得扩展成日常 bypass。
