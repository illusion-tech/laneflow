...
5. 审阅 ADR、design、architecture、Issue 设计说明和 Agent Skill 时，检查领域术语
   ...
   权威，精确代码/协议标识符保留原文。
6. 审阅面向读者的数量时，禁止使用 `k`/`M` 等数量缩写，也禁止 `1 万`、`10 万`
   等阿拉伯数字与中文数量级混写；优先使用规范中文数量，表格、公式和机器输入可
   使用完整十进制数字。交通仿真规模不得使用 `Agent` 或“代理”计数；长期通用规模
   必须使用交通参与单元并按交通执行域报告六类计数。当前车辆 workload 应明确标注
   车辆特化，不得代表非机动车、行人或轨道交通；不可改标识符除外。
