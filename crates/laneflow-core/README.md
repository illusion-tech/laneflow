...
本 crate 由 issue #9 初始化，作为 v0.1 Core 的实现边界。当前已提供 fixed-step tick、最小 vehicle state，以及最小 lane graph / route validation 原语：
...
当前仍不实现 vehicle route following、route transition events、runtime commands、Adapter API、C ABI 或 WASM 绑定；这些能力由后续 v0.1 / v0.x 子 issue 增量实现。v0.1 的 lane graph / route 类型仍是内部实现输入，不是稳定 data spec。
