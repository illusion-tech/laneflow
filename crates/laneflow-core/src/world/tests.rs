    ...
    let mut vehicles = (0..background_count)
        .map(|index| {
            VehicleSpawnInput::active(
                ...
                "R",
                0,
                EdgeProgress::try_new(1_000.0 + 10.0 * index as f64).expect("background progress"),
                Speed::ZERO,
            )
        })
        .collect::<Vec<_>>();
...
fn spatial_operation_counts_depend_on_local_k_not_background_v() {
    let run = |background_count| {
        ...
        let input = VehicleSpawnInput::active("candidate", profile, "R", 0, progress, Speed::ZERO);
...
fn command_spatial_membership_follows_committed_physical_edge_transition() {
    ...
    let mut world = CoreWorld::with_traffic_data(20, traffic_data, vec![vehicle]).expect("world");
