...
pub enum CoreError {
    ...
    // ── 运行时 tick 不变量（NonFinite 家族；#288 热路径镜像面；domain traversal 不变量归各自域分区） ──
    ...
    // ── VehicleProfile / ParticipantClass / Facility 注册校验 ──
    ...
    // ── 静态路网结构校验（RoadSection / LaneGroup / RoadCorridor / LaneGraph） ──
    ...
    // ── Junction / Movement / ManeuverPath 校验与 route-maneuver 匹配 ──
    ...
    // ── Signal / StopLine / ManeuverGate 校验与能力防护 ──
    ...
    // ── Vehicle 生命周期（spawn / replace / despawn 校验） ──
