...
fn lower_verified_source(
    ...
    let alignments = lower_road_alignments(root, &locations).into_boxed_slice();
    ...
    declarations.extend(lower_independent_declarations(root, &locations));
    declarations.extend(lower_owner_scoped_declarations(root, &locations));
    declarations.extend(lower_topology_authoring_declarations(root, &locations)?);
    declarations.extend(lower_aggregate_declarations(root, &locations));
...
fn geometry_compilation_diagnostic(
    ...
    let violation = match error {
        ...
        NumericFreezeError::GeometryPointLimit => unreachable!("handled above"),
    };
    let input = verified.input();
