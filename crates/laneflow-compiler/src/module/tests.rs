...
fn document_digest_and_length_are_bound_to_the_module() {
    ...
    assert_eq!(module.descriptor().frontend_version(), 2);
...
fn empty_module_source_record_has_a_position_independent_known_vector() {
    ...
    assert_eq!(
        *document.source_document_digest(),
        [
            0x78, 0xb9, 0x1e, 0xf7, 0x3a, 0x2a, 0x0e, 0x0a, 0xbd, 0x17, 0x9d, 0x28, 0xf8, 0xe0,
            0xbe, 0x48, 0x23, 0xfb, 0x89, 0xd4, 0xdf, 0xd3, 0xc0, 0x42, 0x11, 0x2d, 0xa0, 0xc5,
            0x1b, 0x7c, 0x10, 0x65,
        ]
    );
    assert_eq!(
        *module.descriptor().source_document_set_digest(),
        [
            0xc2, 0x7a, 0xbd, 0xb5, 0x24, 0x8c, 0x5d, 0xfc, 0x03, 0x6e, 0x7e, 0xe2, 0x48, 0x07,
            0x4e, 0xd3, 0xfa, 0x88, 0x4b, 0x96, 0x34, 0x36, 0xb9, 0x75, 0xa5, 0xc4, 0xd3, 0xba,
            0x3a, 0xcb, 0x59, 0x1b,
        ]
    );
}
...
fn lane_edge_source_record_has_a_known_vector() {
    ...
    assert_eq!(
        *document.source_document_digest(),
        [
            0xa9, 0x40, 0x16, 0xb5, 0xf1, 0x21, 0x99, 0xc1, 0x7c, 0xdd, 0xfd, 0xe0, 0x7b, 0xdd,
            0xbc, 0x3d, 0xc7, 0xa6, 0xad, 0x4e, 0x6b, 0xf5, 0xe0, 0xd2, 0x35, 0xfa, 0xae, 0xea,
            0xe5, 0x54, 0xb0, 0xaa,
        ]
    );
}
...
fn candidate_frontend_peak_includes_the_live_builder_indexes() {
    ...
    assert_eq!(
        builder.totals.admission_peak_live_bytes,
