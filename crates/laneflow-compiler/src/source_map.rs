...
const ROAD_EDITING_ADDRESS_LOGICAL_BYTES: u64 = 2 + 4 + 1 + (3 * 4) + 4;
const ROAD_EDITING_OWNER_LOCAL_SUBJECT_LOGICAL_BYTES: u64 =
    1 + ROAD_EDITING_ADDRESS_LOGICAL_BYTES + 2 + 1 + 4;
// `OwnerLocal(Address)` 是闭合 subject 中最大的 variant；Declaration/Alignment 仅含
// address，Wire 仅含 root/table kind 与 physical index。
const ROAD_EDITING_SUBJECT_LOGICAL_BYTES: u64 = 4 + ROAD_EDITING_OWNER_LOCAL_SUBJECT_LOGICAL_BYTES;
const ROAD_EDITING_LOCATION_PAYLOAD_LOGICAL_BYTES: u64 =
    4 + ROAD_EDITING_SUBJECT_LOGICAL_BYTES + (1 + 4) + (1 + 4) + (1 + 4 + 4);
/// 一个来源位置的 4-byte variant tag 加最大闭合 payload。
///
/// Road-editing 地址内联最多三层 owner ordinal；属性、画布与 byte range 均包含
/// 显式 presence byte。采用最大 variant 让计数式冻结无需先复制并遍历每条位置记录。
pub(crate) const SOURCE_LOCATION_LOGICAL_BYTES: u64 =
    4 + ROAD_EDITING_LOCATION_PAYLOAD_LOGICAL_BYTES;

const LANE_EDGE_SOURCE_LOGICAL_BYTES: u64 = 4 + 16 + SOURCE_LOCATION_LOGICAL_BYTES;
const LANE_EDGE_SUCCESSOR_SOURCE_LOGICAL_BYTES: u64 =
    4 + 16 + 2 + 4 + SOURCE_LOCATION_LOGICAL_BYTES;
const STABLE_ENTITY_SOURCE_LOGICAL_BYTES: u64 = LANE_EDGE_SOURCE_LOGICAL_BYTES;
const CROSS_SECTION_RELATION_SOURCE_LOGICAL_BYTES: u64 =
    2 + 4 + 16 + 2 + 4 + SOURCE_LOCATION_LOGICAL_BYTES;
