...
/// 一个文本来源位置的文档序号和两个紧凑位置。
pub(crate) const TEXT_SOURCE_LOCATION_LOGICAL_BYTES: u64 = 4 + 8 + 8;
/// 一个道路编辑来源位置的 4-byte variant tag 加最大闭合 payload。
///
...
impl SourceLocationRecord {
    const fn logical_bytes(&self) -> u64 {
        match self {
            Self::Text { .. } => TEXT_SOURCE_LOCATION_LOGICAL_BYTES,
            Self::RoadEditing { .. } => ROAD_EDITING_SOURCE_LOCATION_LOGICAL_BYTES,
        }
    }
}
