...
启动时的文件读取、manifest 引用、size/digest、Traffic/Spatial normalization 或 Core world 构造失败会带路径和阶段信息返回；运行中的 Adapter 结构化错误写入 Bevy 日志。CI 在 Bevy / lockfile / CI workflow 变更时使用以下 dedicated compile check（与 `signalized_corridor` 合并为同一次 `native-example` 编译），GUI smoke 仍只在本机执行：
