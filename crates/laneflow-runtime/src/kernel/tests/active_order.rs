...
fn projection(world: &TrafficWorld) {
    let expected: Vec<_> = world
        ...
        .collect();
    assert_eq!(world.state.derived.active_order, expected);
}
...
fn live_prefix_extends_and_reused_slot_keeps_append_order() {
    ...
    let route = f.world.state.vehicle_state(f.vehicles[20]).unwrap().route;
    let extra = f
        .world
        ...
        .unwrap()
        .vehicle;
    assert_eq!(f.world.state.derived.live_order_index.indexed_len, prefix);
    assert_eq!(f.world.state.committed.live_order.len(), prefix + 1);
    leave(&mut f, 30);
    assert_eq!(
    ...
    assert_eq!(f.world.state.derived.active_order.last(), Some(&reused));
    projection(&f.world);
    assert_eq!(
        f.world
            .state
            .derived
            .live_order_index
            .position(f.vehicles[20], &f.world.state.committed.live_order),
        None
    );
    assert!(
        f.world
            ...
            .derived
            .live_order_index
            .position(extra, &f.world.state.committed.live_order)
            .is_some()
    );
...
fn optional_allocation_failure_matches_success_and_retry() {
    ...
    leave(&mut normal, 31);
    assert_eq!(fallback.world.state.derived.live_order_index.indexed_len, 0);
    ...
    leave(&mut normal, 29);
    assert_eq!(
        fallback.world.state.derived.live_order_index.indexed_len,
    ...
    for f in [&mut normal, &mut fallback] {
        let route = f.world.state.vehicle_state(f.vehicles[28]).unwrap().route;
        f.world
            .spawn_parked_vehicle(
                ParkedVehicleSpawnInput::new(VehicleProfileOrdinal::from_raw(0), route, 0, 0),
                ParkingTarget::VirtualPool(ParkingFacilityOrdinal::from_raw(0)),
...
fn rejected_command_preserves_error_snapshot_and_cold_cache() {
    ...
    assert_eq!(
        f.world.state.derived.live_order_index.positions.capacity(),
        0
    );
    f.world.despawn_vehicle(f.followers[0]).unwrap();
    let cursor = f.world.state.committed.command_cursor;
    f.world.state.committed.command_cursor = u64::MAX;
    ...
    f.world.state.committed.command_cursor = cursor;
    leave(&mut f, 0);
    let before = f.world.capture_snapshot().unwrap();
...
fn completed_replacement_invalidates_and_parking_removes_stably() {
    ...
    for _ in 0..200 {
        if f.world.state.vehicle_state(entering).unwrap().progress_mm == 10_000 {
            break;
        }
        f.world.step(TickInput::new(100)).unwrap();
    }
    ...
    assert_eq!(f.world.state.derived.active_order.last(), Some(&entering));
    projection(&f.world);
}
...
fn position_memory_follows_actual_slots_and_is_counted_once() {
    ...
    for high_water in [false, true] {
        ...
        let before = f.world.state.derived.retained_logical_bytes();
        let snapshot = f.world.capture_snapshot().unwrap();
        assert!(
            f.world
                .state
                .prepare_active_insertion(f.vehicles[0])
                .is_some()
        );
        let cache = &f.world.state.derived.live_order_index;
        assert_eq!(
            cache.positions.len(),
            f.world.state.committed.vehicles.len()
        );
        assert_eq!(
        ...
        assert_eq!(
            f.world.state.derived.retained_logical_bytes() - before,
...
fn restore_starts_cold_and_same_revision_cutover_preserves_valid_prefix() {
    ...
    assert_eq!(world.state.derived.live_order_index.positions.capacity(), 0);
    projection(&world);
    let parked = world.live_vehicles()[30];
    assert!(world.state.prepare_active_insertion(parked).is_some());
    ...
    let _commit = world
        ...
        .unwrap();
    assert_eq!(
        world.state.derived.live_order_index.indexed_len,
