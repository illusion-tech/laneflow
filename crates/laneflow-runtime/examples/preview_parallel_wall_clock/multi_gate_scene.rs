//! #705 multi-gate 合成场景（公开 API 复刻，#282 等待规模夹具同形）。
//! 例程与非插桩证据测试共用；不引用 crate 内部项。
...
/// 构造 count 条 idle-zone 三边路线的合成修订（与 kernel 测试夹具同形）。
pub fn build_revision(count: usize) -> Arc<SharedNetworkRevision> {
...
/// 安装世界并生成全部车辆：每条 3 边 idle 路线首边末端前一毫米、10 m/s。
pub fn install(revision: &Arc<SharedNetworkRevision>, workers: u32, world_id: u64) -> TrafficWorld {
    let origin = *revision.canonical_origin();
    let count = revision.traffic().maneuvers().maneuver_path_count();
    ...
    let mut world = TrafficWorld::install(
        Arc::clone(revision),
        WorldConfig::new(count, count, u64::from(count) * 3, 1, DELTA_MS),
...
pub fn routes_of(world: &TrafficWorld) -> Vec<RouteHandle> {
    world.live_routes().collect()
}

pub fn route_boundaries(world: &TrafficWorld, routes: &[RouteHandle]) -> Vec<u32> {
...
/// 补员：把已完成车辆原位替换回 Gate 前，保持稳态活动车队与恒定工作量。
pub fn replenish(world: &mut TrafficWorld, routes: &[RouteHandle], boundaries: &[u32]) {
...
pub fn step(world: &mut TrafficWorld) {
