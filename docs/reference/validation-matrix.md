...
所有切片默认需要一个有效 external reviewer。首版标准路径只接受 exact-head review；`unresolved=0` 是必要非充分条件。唯一长期替代是严格验证的 Dependabot 单提交 `Cargo.lock`-only trusted-ref policy。
...
| G3 comment / body / Issue permalink 或关联 PR/Issue timeline 活动后未新增严格更晚的精确 marker                                                                                  | Fail；marker 必须未编辑且晚于最终 body 与 lifecycle evidence；同秒或其他事件只能撤销旧 success    |
...
| 精确 Dependabot 单提交 `Cargo.lock`-only，provider 无法审阅                                                                                                                     | `dependabot_lockfile_policy` 机器 completion；仍须独立通过 CI、依赖政策和 CodeQL 适用性           |
| lockfile-only finding 声称的 reviewed commit/object 不在 PR range，完整匹配 identity-only paragraph allowlist，且已有回链 current identity 的 G3 Owner disposition 与已解决线程 | 记为已处置 provider identity anomaly，不计 actionable finding；机器 policy 可继续判断             |
| 错误 OID finding 添加任意第三句/额外 claim，或 identity-only 标题、paragraph、footer / Disposition 任一不精确                                                                   | 仍是 actionable finding，Fail；不得因含错误 OID 而整条忽略                                        |
| 上述错误 SHA finding 未 disposition、仍 unresolved，或 finding 不满足窄 identity 模式                                                                                           | `findings_open` / `awaiting_rereview`，Fail；机器 policy 不隐藏正常 finding                       |
| Copilot 返回 “wasn't able to review any files”，但 PR 不是精确 policy                                                                                                           | 不计 completion，保持 `awaiting_review` / 其他既有失败状态                                        |
| Dependabot PR 同时修改源码/workflow、commit identity 不精确、多个 commit 或分页截断                                                                                             | 不适用机器替代，Fail closed                                                                       |
...
`github-actions` 的 spoof fixture 必须是 `missing`。普通 PR 的 missing、
pending、cancelled、failure、多个 aggregate Check、同 head 但其他 PR/base association、
head/base 竞态、缺失/无效 `completedAt` 和无效 evidence URL
继续失败关闭。
...
- Draft、非 `main` base、非 open PR 不计入 R1 sample；same-repository 目标必须用 non-success 取代同一 head 的旧 success，cross-repository 不发布；漏事件只允许显式 manual dispatch 触发保守失效，重新取得 success 仍需新 marker，不运行周期 schedule；
