...
10. G3 comment / Issue G3 permalink 不完整，Related PR 独立 G3 未使用单一 `--related-pr <current-related-pr>` 模式，full-set 未使用 `--delivery-pr` 加全部 Related PR，`Gate 断言` 未记录与实际调用完全一致的规范命令和 `已通过` 结果，或 `cargo +1.96.0 run --locked -p xtask -- check-gate-evidence g3 ...` 失败。
