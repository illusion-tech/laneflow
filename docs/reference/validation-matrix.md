...
| lockfile-only finding 声称的 reviewed commit/object 不在 PR range，已有 G3 Owner disposition 且线程已解决 | 记为已处置 provider identity anomaly，不计 actionable finding；机器 policy 可继续判断             |
...
pending、cancelled、failure、多个 aggregate Check、head/base 竞态和无效 evidence URL
继续失败关闭。
