...
| 快速通道 files 分页溢出、含任一 `changeType=RENAMED` 文件、head commit 不在 commits 连接内、snapshot 字段缺失（`additions`/`deletions`/`message`，仅两条新通道）或 files 补页期间 current head/base 变化 | 通道 fail-closed 不成立，回标准路径；pre-activation（G1 冻结 `2026-08-20T04:20:39Z` 前）replay 不注入新通道机器 completion；dependabot 通道字段口径不变（旧快照兼容） |
