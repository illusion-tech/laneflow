...
矩阵不要求所有 PR 跑同一组重复检查，但要求每次变更显式说明验证结论。当前仓库尚无运行时代码，代码类检查在对应技术栈落地后逐步启用；在此之前，相关切片应记录“尚未具备该检查”而不是假装通过。
...
| `core-runtime`   | 单元测试、确定性行为说明、Core API 影响说明                        | adapter build、示例 smoke（除非影响主路径） |
