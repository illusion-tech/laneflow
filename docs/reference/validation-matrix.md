...
| lockfile-only finding 声称的 reviewed commit/object 不在 PR range，精确匹配 identity-only 文案，且已有回链 current identity 的 G3 Owner disposition 与已解决线程 | 记为已处置 provider identity anomaly，不计 actionable finding；机器 policy 可继续判断             |
| 错误 OID finding 同时混入 checksum/漏洞/许可证/unsafe 等实质问题，或 identity-only 文案/Disposition 不精确                                                       | 仍是 actionable finding，Fail；不得因含错误 OID 而整条忽略                                        |
