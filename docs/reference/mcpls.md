...
6. 用 `StartupTimeoutSeconds` 的单一截止时间约束初始 Git worktree discovery、自动清理、
   二进制校验、锁等待、CIM 创建、端口绑定和 HTTP 健康检查；CLI 只接受 `15..300`
   秒，最低值为创建前工作、至少一个整秒的 CIM 调用、3 秒创建后身份检查和 5 秒受检
   回收保留可用预算。CIM 只接受整秒超时，因此脚本只使用剩余预算内的整秒数，并在
   创建前保留后续身份检查预算；端口绑定和 HTTP readiness 还会在同一截止时间内保留
   5 秒受检回收预算，不能把总预算耗尽后再尝试停止已经外部创建的进程；
...
8. 进程创建后立即原子持久化 `starting` 状态，再等待端口绑定；只有健康检查、启用配置
   ...
   返回失败；WMI 已返回 PID 但首次身份读取异常时也走同一补偿路径，避免创建响应或检查
   结果不确定时留下无状态常驻进程。
