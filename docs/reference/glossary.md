...
| 规范发布描述符          | canonical publication descriptor                          | `CanonicalPublicationDescriptor` / `LFCP` | Accepted ADR 0024 冻结的当前目标版本是 LFCP v2，只绑定 LFCA/LFSM、来源沿袭和对象键，不含 receipt，也不自行证明已发布；LFCP v1 只保留为 #298 历史实现证据。                                        |
