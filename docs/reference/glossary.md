...
| 受检规范路网输入   | checked canonical network input  | `CheckedCanonicalNetworkInputV1` | 由 `laneflow-format` 定义和构造的字段私有进程内能力：绑定受检 LFCA view、实际 digest/length 与重算的 `NetworkRevisionId`，是共享静态路网 builder 的唯一输入。提案中（Proposed；#496）：G2 另分配并行 `CheckedCanonicalNetworkInputV2`，只承认 LFCA v2；不得把 v2 塞进 V1。 |
...
| 后发射检查         | post-emission check              | `check_post_emission_bundle_v1`  | Accepted ADR 0024 / #299 的检查：只依据最终 LFCA/LFSM/LFSD、显式 diff base 和 limits 重算 digest/length/revision 并核对跨对象 binding。                                                                                                                                    |
| 后发射受检制品组   | post-emission checked bundle     | `PostEmissionCheckedBundleV1`    | Accepted ADR 0024 的进程内能力：检查成功后借用三对象字节，字段私有且不可序列化；不表示已发布或可信。                                                                                                                                                                       |
