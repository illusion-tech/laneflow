...
| 当前态来源输入          | current-source input                                      | `CurrentSourceInput`                    | 目标态已接受（#297 G1），经公开零复制构造器借用原始 ScenarioManifest、具名制品、显示/审计来源和本次实际导入来源沿袭；不表示已验证状态，也不携带可脱离 builder 使用的资源余额。 |
| 当前态导入来源沿袭输入  | current-import provenance input                           | `CurrentImportProvenance`               | 目标态已接受（#297 G1）：借用实际 importer build ID、实际选项摘要和本次转换来源沿袭；由 builder 先验证并写入只读模块描述符，但调用方声明本身不是发布真实性或安全信任根。       |
...
| 当前态导入配置档        | P100 current-source import profile                        | `LF-CURRENT-SOURCE-P100-IMPORT-v1`      | 目标态已接受（#297 G1）：为 current 三文档严格导入冻结 source 专用硬上限，并与调用点 compiler 余额逐维取更严值；它不改变 production-compatible current loader 的接受域。       |
