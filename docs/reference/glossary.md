...
| 有类型抽象语法树        | typed abstract syntax tree                                | typed AST                                 | 保留前端语法、显式键、类型、单位和来源位置的第一层表示。提案中（#500）：准入后交通一维不再把编制 `f64` 米当存储权威。                                                                             |
| 高层中间表示            | high-level intermediate representation                    | HIR                                       | 已完成模块、命名空间、符号、引用、单位和编制语义解析的表示。提案中（#500）：交通一维存储权威为整数毫米。                                                                                          |
| 中层中间表示            | mid-level intermediate representation                     | MIR                                       | 已完成拓扑、几何展开和全局静态语义推导、但尚未绑定目标布局的表示。提案中（#500）：交通一维存储权威为整数毫米。                                                                                    |
| 已验证规范低层中间表示  | validated canonical low-level intermediate representation | canonical LIR                             | 后端唯一输入；包含稳定标识、有类型序号、规范数值、确定性关系和布局无关预计算。提案中（#500）：交通一维与 LFCA 同一套整数毫米。                                                                    |
...
| 整数毫米            | integer millimetre                  | mm                   | 已提交交通一维几何的权威长度量子；边长、边内进度、车长、间距与停车锚点使用整数毫米。提案中（#500）：准入后编译器 Typed AST / HIR / MIR / LIR 的交通一维存储权威同为整数毫米。 |
| 编制准入量化        | authoring admission quantization    | —                    | 提案中（#500）：编制 `f64` SI 在官方前端 / 共同接入边界 round-ties-to-even 一次进入整数毫米或受检 `f32` SI；之后 IR 不得再把交通一维存成编制米列。                            |
