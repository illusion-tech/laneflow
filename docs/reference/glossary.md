...
| 分区规划提示         | partition planning hints               | `PartitionPlanningHints`           | LFCA v1 不保存的非语义性能提示；由 `laneflow-static-network` 按显式 derivation version 从受检规范关系确定性派生，可安全忽略或重建。 |
...
| 占用索引             | occupancy index                        | `OccupancyIndex`                   | 交通运行时按物理车道边分桶、每固定步进由已提交状态重建的私有扁平占用表；整数毫米；一辆车可在多桶各有占用记录；供跟车查询，不进入公开 API。                    |
...
| 受检规范路网输入   | checked canonical network input  | `CheckedCanonicalNetworkInput` | 由 `laneflow-format` 定义和构造的字段私有进程内能力：绑定受检 LFCA view、实际 digest/length 与重算的 `NetworkRevisionId`，是共享静态路网 builder 的唯一输入。只承认当前对象 `formatVersion`。 |
...
| 后发射受检制品组   | post-emission checked bundle     | `PostEmissionCheckedBundle`    | Accepted ADR 0024 的进程内能力：检查成功后借用三对象字节，字段私有且不可序列化；不表示已发布或可信。                                                                                          |
| 最小发布闭合       | minimal publication closure      | —                              | Accepted ADR 0024 的边界：后发射检查、内容寻址安装、LFCP v2 安装和对象外认证 manifest 单提交共同成功；不新增证明制品。                                                                        |
...
| 内存映射           | memory mapping                   | mmap                           | 让文件字节映射到地址空间、供只读视图按偏移量访问的加载方式；ADR 0025 / #300 v1 不采用。                                                                                                       |
...
| 路线                | route                               | `Route`              | 当前道路机动车执行域中由有序遍历边和相关出现项组成的车辆行驶计划；唯一生产入口是 `register_route`。                                                           |
| 动态路线            | dynamic route                       | `Route`              | “路线”的运行时注册特化，具有代际感知句柄且不获得持久 `StableId128`；路网产品不再另设预编译静态路线。                                                          |
...
| 每世界可变状态      | per-world mutable state             | —                    | 交通参与单元、动态通行定义、控制器时钟、预约、占用和缓冲区等不能进入共享静态路网的状态；当前投影包括车辆、动态路线和停车。                                    |
...
| 静态路线     | `StaticRoute`                                              | 已退役的预编译路线实体。Identity 种类代码 21 与字段标签 30 保留空位；路线只经 `register_route` 进入每世界表。   |
