...
| 规范发布描述符          | canonical publication descriptor                          | `CanonicalPublicationDescriptor` / `LFCP` | LFCP v2 是位于制品字节外、绑定 LFCA/LFSM 摘要、精确长度、路网修订、编译来源沿袭和对象键的最小发布值；它不含 receipt，也不自行证明已发布，真实性来自外部认证 manifest。                            |
...
| 验证收据封套（历史）    | validation receipt envelope                               | `ValidationReceiptEnvelope`               | ADR 0020 原计划由 #299 交付的独立验证制品；ADR 0024 已取消 canonical publication receipt 和统一三类 receipt 方案。#300/#302 不得把该历史类型当作已冻结输入。                                      |
...
| 路网修订切换描述符   | network revision cutover descriptor    | `NetworkRevisionCutoverDescriptor` | 在镜像外可信绑定两侧修订、制品、镜像、语义差异与迁移策略的切换输入；具体字段和 trust anchor 由 #302 G1 冻结。                    |
...
| 静态镜像描述符         | static image descriptor          | `StaticImageDescriptor`         | 位于镜像字节外、绑定路网修订标识、规范制品/镜像/完整性清单各自摘要与精确长度、版本、目标、配置档和工具的值；当前精确字段由 #300 G1 冻结。                                            |
| 可信描述符             | trusted descriptor               | —                               | 经对象外 trust anchor 认证，并与路网修订标识、目标对象摘要/精确长度、版本、目标平台和配置档绑定的外部描述符。                                                                        |
| 验证收据（简称，历史） | validation receipt               | —                               | “验证收据封套”的历史简称；ADR 0024 后不属于 canonical publication 当前协议，#300/#302 若需要其他证明形态必须重新决策。                                                               |
...
| 可信静态镜像           | trusted static image             | `TrustedStaticImage`            | 与对象外认证描述符和完整性清单匹配，且只暴露已完成分块/结构验证目标节的能力对象；精确信任输入由 #300 G1 冻结。                                                                       |
| 后发射检查             | post-emission check              | `check_post_emission_bundle_v1` | ADR 0024 / #299：只依据最终 LFCA/LFSM/LFSD exact bytes、显式 diff base 和 limits 重算 digest/length/revision 并核对跨对象 binding 的无分配检查。                                     |
| 后发射受检制品组       | post-emission checked bundle     | `PostEmissionCheckedBundleV1`   | 后发射检查成功后借用三对象字节、字段私有且不可由调用方自报绑定构造的进程内能力；它不可序列化，不表示已发布或可信。                                                                   |
| 最小发布闭合           | minimal publication closure      | —                               | 后发射检查、内容寻址安装、LFCP v2 安装和外部认证 manifest 单提交共同成功后才成立的发布边界；不引入独立证明制品。                                                                     |
| 编译器语义验证         | compiler semantic validation     | —                               | 编译器对来源和 IR 执行的主语义检查。                                                                                                                                                 |
| 独立验证器（历史）     | independent validator            | `laneflow-validator`            | ADR 0020 原计划的第二套完整语义实现；ADR 0024 已决定 pre-1.0 的 #299 不创建该 crate/工具。                                                                                           |
