...
| 来源集合摘要            | source collection digest                                  | `sourceCollectionDigest`                | G1 待接受（#298；未实现），按依赖优先模块顺序对命名空间及每模块来源文档集摘要进行版本化、域分隔聚合所得的 SHA-256；它绑定一次 LFSM 的完整来源模块集合，不替代逐文档摘要，也不进入路网修订标识。   |
...
| 已结构验证规范制品 | checked canonical artifact      | `CheckedPortableArtifactView`  | G1 待接受（#298；未实现），通过有界结构检查、尚未证明语义或来源可信的可移植规范制品只读视图。                                                                                        |
| 规范发布候选       | canonical publication candidate | `PortablePublicationCandidate` | G1 待接受（#298；未实现），同次成功编译原子产生的制品、源映射、差异与绑定；尚未独立验证或发布。                                                                                      |
