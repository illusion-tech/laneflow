...
| 当前态来源输入          | current-source input                                      | `CurrentSourceInput`                    | 提案中（Proposed；#315），只借用原始 ScenarioManifest、具名制品和显示/审计来源；不表示已验证状态，也不携带可脱离 builder 使用的资源余额。                                      |
| 当前态生产兼容策略      | current-source production compatibility policy            | —                                       | 提案中（Proposed；#315），供 `laneflow-data` 保持 v0.1 既有接受集合；不新增制品数、单个不透明引用长度或引用总字节拒绝条件。                                                    |
...
| 当前态来源位置表        | current-source location table                             | `CurrentSourceLocationTable`            | 提案中（Proposed；#315），与 current 线格式验证原子产生、关联真实来源行列的位置伴随表；严格导入时受资源配置档约束，消费后不保留第二份原始来源全文。                            |
| 已验证当前态来源包      | validated current-source bundle                           | `ValidatedCurrentSourceBundle`          | 提案中（Proposed；#315），按所选官方策略验证三文档后铸造的字段私有能力；原子绑定 DTO、文档身份、来源与位置，不证明发布真实性。                                                 |
