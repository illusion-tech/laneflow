...
| 当前 JSON               | current JSON                                              | —                                         | Traffic v0.10、SpatialPackage v0.1 与 ScenarioManifest v0.1 的合称；#301 已删除生产 schema 与加载 crate。走廊生成器仍可能写出同名 JSON 字节，但不是运行时快照、目标静态镜像、长期编制来源或已发布兼容格式。 |
