...
| 当前态来源输入          | current-source input                                      | `CurrentSourceInput`                    | 目标态边界已接受（#315 G1），#297 G1 Review 候选经公开零复制构造器借用原始 ScenarioManifest、具名制品、显示/审计来源和本次实际导入来源沿袭；不表示已验证状态，也不携带可脱离 builder 使用的资源余额。 |
| 当前态导入来源沿袭输入  | current-import provenance input                           | `CurrentImportProvenance`               | Review（#297 G1 候选）：借用实际 importer build ID、实际选项摘要和本次转换来源沿袭；由 builder 先验证并写入只读模块描述符，但调用方声明本身不是发布真实性或安全信任根。                               |
