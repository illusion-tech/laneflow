...
| 来源模块头              | source module header                                      | `SourceModuleHeader`                    | 提案中（Proposed；#292），调用方提供命名空间、工具、选项和来源沿袭等非内容字段的受检输入；内容摘要与完整描述符由官方前端构建器派生。                                |
...
| 已验证源映射输入        | validated source-map input                                | `ValidatedSourceMapInput`               | 提案中（Proposed；#292），与 LIR 同次成功编译冻结、只保存来源模块、来源位置、来源沿袭及 LIR 键关联且不能补充静态语义的伴随数据。                                    |
| 已验证编译结果          | validated compilation output                              | `CompilationOutput`                     | 提案中（Proposed；#292），原子拥有 LIR、已验证源映射输入和非错误级诊断的成功编译结果；错误级诊断存在时不得构造。                                                    |
...
| 编译资源上限            | compile limits                                            | `CompileLimits`                         | 提案中（Proposed；#292），在后继阶段分配前限制来源、记录、关系、字符串、诊断、暂存区和编译器控制总存续内存的失败关闭边界；v0.10 首轮精确值由具名 P100 配置档冻结。  |
| P100 首轮编译资源配置档 | P100 initial compile limits profile                       | `LF-COMP-P100-INITIAL-v1`               | 提案中（Proposed；#292），以 #308 压力分层逐维上包络冻结、由宿主显式选择且不提供无限模式的首轮生产编译资源上限向量。                                                |
| P100 首轮编译性能门槛   | P100 initial compiler performance gate                    | `LF-COMP-P100-R0-v1`                    | 提案中（Proposed；#292），在 P100 上按 #308 校准/压力分层精确建议约束首轮生产编译实现的时延与内存输入；不是产品 SLA，超限须带语义差额回到 G1。                      |
