...
| 拐点评估               | knee assessment                                           | `knees[]`                               | 提案中（Proposed；#308），以候选、指标和上下级分层为自然身份，引用 batch 0/1 比值并绑定性能分析制品；规模由上级分层派生。                                           |
...
| 泰尔－森增长斜率       | Theil–Sen growth slope                                    | `growthSlopes[]`                        | 提案中（Proposed；#308），对线性区间全部级别对的整数观测使用冻结 Q16.16 编码、再取中位数得到的稳健增长估计；两批结果与建议上界不得通过宿主浮点或对数库生成。        |
| 有符号 Q16.16 有理斜率 | signed Q16.16 rational slope                              | `signedQ16_16Rational`                  | 提案中（Proposed；#308），以有符号整数 numerator、约分后 `1`/`2` denominator 和十六个小数位表示中位斜率或上界，真实值为 `(numerator / denominator) / 65536`。       |
