...
| 受检规范路网输入   | checked canonical network input  | `CheckedCanonicalNetworkInputV1` | 由 `laneflow-format` 定义和构造的字段私有进程内能力：绑定受检 LFCA view、实际 digest/length 与重算的 `NetworkRevisionId`，是共享静态路网 builder 的唯一输入。提案中（Proposed；#496）：G2 另提供只承认 LFCA v2 的并行能力；不得把 v2 塞进 V1。 |
