...
| 来源模块头              | source module header                                      | `SourceModuleHeader`                    | 目标态已接受（#292 G1），调用方提供命名空间、工具、选项和来源沿袭等非内容字段的受检输入；内容摘要与完整描述符由官方前端构建器派生。                                            |
...
| P100 首轮编译性能门槛   | P100 initial compiler performance gate                    | `LF-COMP-P100-R0-v1`                    | 目标态已接受（#292 G1），在 P100 上按 #308 校准/压力分层精确建议约束首轮生产编译实现的时延与内存输入；不是产品 SLA，超限须带语义差额回到 G1。                                  |
