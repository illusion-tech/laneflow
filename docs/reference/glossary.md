...
| 当前态来源资源配置档    | current-source resource profile                           | `CurrentSourceLimits`                   | 提案中（Proposed；#315），解析当前态三文档前限制制品数、引用字节、文档/组合字节、字符串、记录、来源、位置与存续内存的显式有界配置；无默认或无限配置。                          |
...
| 已验证当前态来源包      | validated current-source bundle                           | `ValidatedCurrentSourceBundle`          | 提案中（Proposed；#315），当前态来源包有界验证三文档配对、身份和线格式后铸造的字段私有能力；原子绑定 DTO、三文档身份、来源与位置，不证明发布真实性。                           |
