...
| 规范发布描述符          | canonical publication descriptor                          | `CanonicalPublicationDescriptor` / `LFCP` | 当前已实现版本是 LFCP v1；Proposed ADR 0024 的候选 LFCP v2 只绑定 LFCA/LFSM、来源沿袭和对象键，不含 receipt，也不自行证明已发布。                                                                 |
...
| 验证收据封套            | validation receipt envelope                               | `ValidationReceiptEnvelope`               | ADR 0020 原计划由 #299 交付的独立验证制品；Proposed ADR 0024 拟取消该方案，只有正式 Accepted 后才转为历史。                                                                                       |
...
| 验证收据（简称）   | validation receipt               | —                               | “验证收据封套”的简称；Proposed ADR 0024 拟从 LFCP v2 移除，正式 Accepted 前仍属于现行 v1 设计输入。                                                                                  |
...
| 后发射检查         | post-emission check              | `check_post_emission_bundle_v1` | Proposed ADR 0024 / #299 的候选检查：只依据最终 LFCA/LFSM/LFSD、显式 diff base 和 limits 重算 digest/length/revision 并核对跨对象 binding。                                          |
| 后发射受检制品组   | post-emission checked bundle     | `PostEmissionCheckedBundleV1`   | Proposed ADR 0024 的候选进程内能力：检查成功后借用三对象字节，字段私有且不可序列化；不表示已发布或可信。                                                                             |
| 最小发布闭合       | minimal publication closure      | —                               | Proposed ADR 0024 的候选边界：后发射检查、内容寻址安装、LFCP v2 安装和对象外认证 manifest 单提交共同成功；不新增证明制品。                                                           |
| 编译器语义验证     | compiler semantic validation     | —                               | 编译器对来源和 IR 执行的主语义检查。                                                                                                                                                 |
| 独立验证器         | independent validator            | `laneflow-validator`            | ADR 0020 原计划的第二套完整语义实现；Proposed ADR 0024 拟决定 pre-1.0 的 #299 不创建该 crate/工具，正式 Accepted 后才转为历史。                                                      |
