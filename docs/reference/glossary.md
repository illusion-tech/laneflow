...
| 跟车前视            | vehicle-following front lookahead   | `front_query_horizon` | 每车每拍由当前速度与车辆 profile 按跟车 §10.1 推导的查询下界；SI 有限后向上取整到毫米。占用查询只访问视距内出现项；占用索引仍按全部 Active 重建；不是目视距离。 |
