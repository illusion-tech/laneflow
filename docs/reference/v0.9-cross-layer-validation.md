...
- 200 车全栈 headless soak 运行 10,000 tick：真实发生 137 次
  completion/replacement 轮换与 183 次 blocked，pending FIFO 峰值 2，
  `running + pending = 200` 逐帧恒定，无 unbounded queue 或 retained growth。
- 稳态 lifecycle 零分配与 10,000 次 completion/replacement 轮换覆盖 50 与
  200；outer-frame chunking replay 一致性覆盖 50/100/200。
...
| 性能     | `steady_lifecycle_is_allocation_free_and_retained_capacity_is_bounded` 10,000 轮换零分配（50/200 @ 11）；200 车 soak 有界性见第 3 节                                                                                        | 50/200      |
...
200 车 seed 0，1,250 outer frame × 8 tick = 10,000 tick。
completed = replaced = 137，blocked = 183，pending FIFO 峰值 2（断言上界
16），逐帧 `running + pending = 200` 且无错误；200 个 logical identity 始终
绑定原 proxy entity。
...
真实回流（completed/replaced 26–109），最大 pending 峰值 5（200 @ 42 高拥堵，
blocked = 2,165），不变量 `completed == replaced + final_pending` 处处成立。
...
`running + entry pending = target` 在三规模全部保持；200 车入口饱和时 pending
排队有界且持续回流，与 headless soak 断言一致。HUD step 成本为 example-local
诊断，不构成 renderer 或跨机器 SLA。
