...
**2026-07-24 修订**: #222 将 `min_gap` 冻结为可行 snapshot 的最终净距下限，并保留既有 sub-min-gap 输入的非恶化语义<br>
**关联文档**:
...
2. 基于 leader final travel 的 deterministic minimum-gap-preserving geometry projection。
...
正常受控模式下，车辆几何不得重叠。`min_gap` 既是 IIDM 的强行为目标，也是从可行 snapshot 出发时最终几何投影不可主动侵入的 bumper-gap 下限，但它仍不是 world initialization、spawn/replace 的物理碰撞合法性边界：

- snapshot bumper gap `g0 >= min_gap` 时，tick 后必须保持 `final_gap >= min_gap`；
- `0 <= g0 < min_gap` 的既有状态仍可进入 Core，但 tick 后必须保持 `final_gap >= g0`，不得继续恶化；
- Core 不通过倒车、瞬移或 Adapter 视觉偏移恢复既有 sub-min-gap；
- `min_gap == 0` 时退化为原有 no-overlap 下限。

该下限必须在每个有 leader 的 tick 生效，而不是等双方完全静止后才应用；否则停车前一个 tick 形成的 sub-min-gap 无法在不倒车的前提下恢复。事故仍必须由未来显式 out-of-control/incident 模式表达，不能由 Vehicle Following 数值失败隐式产生。

leader query 必须至少覆盖 `min_gap + follower 本 tick 最大可能 travel + minimum-gap tolerance`；否则低速 follower 可能在 gap 略高于 `min_gap` 时暂时看不到 leader，并在单个 tick 内侵入下限。
...
- 从可行净距出发的队列不会再被 same-tick hard stop 压到保险杠接触；既有 sub-min-gap 状态也不会继续收缩。
