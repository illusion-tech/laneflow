...
**日期**: 2026-08-30；2026-09-23（#743）修订每世界 binding 的物理存储<br>
...
- 每世界停车增量状态是 `O(F + S + max(V_cap, H))`，不是 `O(C)`。`V_cap` 是配置的
  车辆容量，`H` 是 binding 槽位表覆盖的最高车辆下标加 1。安装时按 `V_cap` 建立空槽；
  句柄代际耗尽后追加的更高下标只在写入预检扩容。账本按槽位表 `capacity` 计算，不再按
  实际 binding 数 `B` 描述这块物理存储；
- tick 热路径不得扫描全部设施、泊位、声明虚拟容量或全部空槽；
- 10k/100k 声明虚拟容量但稀疏占用的设施不能按 `C` 物化车位。binding 槽位跟随车辆
  容量，不跟随声明虚拟容量。10k/100k 实际 live parked vehicle 的车辆和 binding 状态
  仍不可避免。
