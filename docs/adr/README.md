...
- `0014`: 补偿残差感知的 f32 Core 数值权威、产品范围与生产迁移门槛（Accepted；Spatial 精度条款已被 Accepted ADR 0015 部分取代；生产一维几何下一合同见 Proposed ADR 0028，未 Pass）
...
- `0028`: 交通运行时已提交一维几何改为整数毫米 + 微米余数，固定步进
  ...
  若通过则部分取代 ADR 0014 的 current-`f64` 生产实施与残差 `f32` 目标合同
  （Proposed；#496 G1；未 Pass，不授权实现）
