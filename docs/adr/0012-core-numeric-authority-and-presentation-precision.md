...
本 ADR 的 production 结论仍保持 Accepted，原因不是上述旧证据继续成立，而是 #140 仍是 research-only model，尚未转换完整 production Signals、Parking、route-distance、command spatial、Core API/error 和 Data normalization。#141 将通过 superseding ADR 决定是否采用 residual-aware f32；在此之前不得由研究结果静默改变 production authority。完整数字与 argmax provenance 见 `../reference/v0.6-numeric-validation.md` 第 9 节。
...
这不是“`f64` 在所有硬件上天然更快”的普遍声明。#122 最初观测到所有 f32 候选均无 wall-time 优势且只有保留 f64 progress 的 mixed 候选通过；#140 已证明该候选定义和性能结论不完整。当前继续使用 f64 的依据改为：完整 production residual-aware f32 转换、跨组件 oracle 和 API/Data 迁移尚未完成，而不是 f32 已被永久证明无收益。
...
旧候选的 residual 只参与写入，不参与 gap/remaining/boundary/snapshot 读取，因此其 dense/stop 失败不能再作为拒绝完整 compensation 的依据。#140 的 residual-aware 候选通过当前研究预算、候选内存降低 31.25%，同轮 Criterion 也快于 f64。该方案升级为“需要完整 production candidate 与 superseding ADR”，但在 Core API/Data/Signals/Parking/route/command 全链验证前仍不作为 production authority。

### mixed `f32 compute + f64 progress`

#122 阶段它是唯一通过差分预算的 f32 候选；#140 已证明 residual-aware f32 progress 也能通过，因此“唯一”不再成立。它仍是有效对照，但不是默认胜者；后续 superseding ADR 应在完整 production layout 上比较 residual-aware f32、f32 compute + f64 progress 与 current f64。
...
- #141 承接 superseding ADR；只有完整 production residual-aware candidate 补齐正确性、性能、总内存和 API/Data 迁移证据后，才改变 authority。#140 足以重开设计，不足以直接迁移。
