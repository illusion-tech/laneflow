...
**状态**: 已被 ADR 0014 取代（Superseded）；当前生产实现在迁移闸口完成前仍保持本文实现状态
...
ADR 0014 已接受补偿残差感知 `f32` 的下一目标契约，并取代本文关于“目标权威继续统一为 `f64`”的决策。当前生产实现仍保持本文的 `f64` 状态，直到独立原子迁移议题通过正确性、完整内存、性能、API/Data 版本与 G0-G4 闸口；不得由研究结果或本 ADR 状态变更静默改变生产权威。完整数字与最大误差来源见 `../reference/v0.6-numeric-validation.md` 第 9 节。
