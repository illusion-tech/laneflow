...
**状态**: Accepted<br>
**日期**: 2026-07-22<br>
**最后更新**: 2026-07-22（#186 G1 authority correction）<br>
**适用范围**: v0.8 Signalized Corridor MVP 的 caller-owned population policy、Core atomic replace command、seeded 回流、Bevy proxy 复用与启动配置边界
...
`laneflow-core` 不提供人口 controller，也不把目标数量、seed、portal/route catalog、pending recycle 或抽样策略纳入 Core public API。Core 只提供第 2 节的通用原子 replace command。城市模拟游戏、headless host 或 v0.8 reference scenario 可以各自拥有生命周期策略，并在两个 Core step 之间显式调用该命令。

#203 只实现 v0.8 reference scenario 所需的 caller-owned 确定性策略；它不是 `CoreWorld` 字段，不得成为 `CoreWorld::step` 的隐藏状态，也不构成所有 LaneFlow 集成必须采用的默认人口模型。未来城市游戏可以完全替换 #203 的 policy，而无需修改 Core。
...
Core 增加通用 typed command：

```text
replace_completed_vehicle(old_handle, &replacement_input)
  -> Replaced(VehicleReplaceRecord { old, new })
...
物理 overlap 是 allocation-free、可重试的 typed `Blocked`，只携带 old/blocker handle、前后关系和 bumper gap；其他 validation/invariant failure 返回 fatal `CoreError`。任一失败结果都保持 `CoreWorld` committed authority 不变。

成功后旧 `VehicleHandle` 立即 stale，新 vehicle 获得不同 generation 的 handle；public contract 不保证复用相同 slot index。replace 保留旧 vehicle 的 stable update-order position，不产生 tombstone；独立 spawn 仍追加到尾部。v0.8 的 logical population slot 可以复用相同 external ID，但只能由 replace command 在同一事务内完成 duplicate-ID 预检和转移。
...
#203 的 v0.8 reference policy 使用项目自有的 `SplitMix64` 序列，不把 RNG 或 seed 带入 Core。state 由 caller 提供的 `u64 seed` 直接初始化，零 seed 合法；`next_u64` 使用 SplitMix64 标准 state increment、xor-shift 和乘法常量。
...
| 目标人口、seed、portal/lane/route 决策   | caller policy（v0.8 为 #203） |
...
- #203 reference policy 可用于 v0.8 headless/Bevy 验证，同时未来游戏可完全替换；
...
- Core 和 Adapter 都增加 public typed lifecycle API；#203 policy 不成为 Core API；
- #203 必须维护 PRNG golden sequence，算法变更会改变同版本 reference replay；
...
- Core API：新增 caller-driven typed atomic replace command，属于 pre-1.0 public API change；不新增人口 controller、车辆数量限制或 RNG。
...
- Determinism：Core replacement 保留 stable update order；#203 seeded policy 的承诺范围继续是同一实现版本和运行环境，seed 是 caller policy 的显式 input/state。
...
- #203 在相同 seed 和 fixed-step input sequence 下得到相同 initial/recycle decisions；
...
- 下游实施：#185、#186、#187、#188、#189、#203；v0.8 收口 #195 / Parent #193。
