...
- Dependabot security updates 是否启用及其升级策略。
- Rust/Cargo 依赖漏洞审计工具和 CI 集成。
- 许可证允许列表、拒绝列表和供应链约束。

`#88` 不以 GitHub 当前无 Dependabot open alerts 代替 `#56` 的依赖与许可证基线。
...
- Dependabot：vulnerability alerts 可用，open alerts API 成功返回；security updates 状态单独记录，不与 vulnerability alerts 混写。
