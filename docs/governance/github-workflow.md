...
- R2 激活前必须注册并仅在 base repository 安装专用 External Review Gate GitHub App；其短期 installation token 只能提供给 trusted publisher。App repository permissions 固定为 `Checks: read and write`（创建 / 复核 Check Run）与 `Commit statuses: read and write`（允许 ruleset 选择 expected source），不授予 Contents、Actions、Issues 或 Pull requests；evaluator 读取继续使用 trusted workflow 的 `GITHUB_TOKEN`。PR D 把正式 Check 政名为 `External Review Gate`，验证 API 返回该专用 App 的 slug / integration ID，并在 ruleset 中同时绑定 Check 名和 expected source App。缺少该独立身份、仍由 `github-actions` 发布或 spoof canary 能满足 required check 时，R2 一律阻断。
