...
- 在 `merge-queue-g4-evidence:v1` 中按 Delivery-first、随后全部 Related PR 顺序逐项保存证据：
  ...
  的 trusted merge metadata、对应 PR 的 GitHub `merge_group` run、merge 前最后一个 queue timeline event，以及
  live Ruleset 全部分页结果中 required checks 在 `H_mg` 上的最新完成结论；不能用旧 queue run、一个 Delivery
