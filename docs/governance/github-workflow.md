...
- `publish-external-review-check` 在 evaluator 前后复核 PR number、open/draft、main base、same-repository、head/base OID；只有 identity 稳定时才创建完成态 Check Run。Check 固定名为 `External Review Gate`，external ID 绑定 repository、PR、head、trusted validator OID 与 workflow run/attempt，并在 API 响应中复核 source App 为 `github-actions`。
