...
1. 确认任务类型和影响范围。
2. 审计当前 Issue 的 GitHub 元数据 / 依赖关系，包括 Project、Project status、Milestone、Labels、Parent / sub-issues、Blocked by、Blocking、Delivery PR 和 Related PRs；Delivery PR 创建后须在 G3 前确认 `closingIssuesReferences` 覆盖目标 Issue，Related PR 不得误用 closing keyword。G3 前必须取得 trusted reviewer 对当前 exact head 的完成态外部审阅；唯一长期替代是 `development-gates.md` 定义、由 trusted-ref validator 严格证明的 Dependabot 单提交 `Cargo.lock`-only 机器路径。PR author 自审不计入外部 reviewer，有正常 findings 时必须在处置后取得新的当前 head clean re-review。G3 证据必须写入一条新的 append-only PR comment、G4 证据必须写入 Issue comment，body checkbox 只回链对应 permalink；`Gate 断言` 行必须包含与实际参数完全一致的反引号命令并明确写 `已通过`，pending、缺少结果、参数不匹配或运行失败均不得推进对应 Gate。若只能手动关联 GitHub Development 面板，必须记录显式例外；不适用项必须有 `N/A` 原因，无法设置的必需元数据必须记录显式例外。
...
提交信息应遵守 `docs/reference/commit-convention.md`，标题使用 Conventional Commits，正文必须包含 `Gate`、`Slice`、`Impact`、`Scope`、`Validation`、`Docs`，底部 footer 必须包含 `Refs` 或 `Closes`。新提交的本地 `Gate` 使用 `G3 Candidate` 或 `G3 Block`，但 PR / push range 不得包含 `G3 Block`；正式 `G3 Pass` 只存在于 PR Check 和当前 head 的 append-only G3 comment。
