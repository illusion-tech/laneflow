...
Dependabot PR 仍必须通过测试、cargo-deny、CodeQL 适用性判断与外部审阅门禁。自动生成不等于自动批准或自动合并。精确单提交 `Cargo.lock`-only PR 可以按 `development-gates.md` 由 `dependabot_lockfile_policy` 提供机器审阅替代，并由 `security-scanning.md` 的 `dependabot-cargo-lock-only-v1` 把 CodeQL aggregate `NEUTRAL` / no-analysis 记为 `not_applicable`；两个 policy 共用同 repository bot ref、GitHub `web-flow` verified signature、完整 Dependabot force-push provenance 与严格 author/commit/path/head 证明，且不豁免本节任何依赖、许可证、漏洞、来源或测试要求。
