...
- G4 live validation 还须从 GitHub API 读取该 `H_mg` 的 `merge_group` workflow run 与目标分支当前生效的 required-status-check rules；队列 run 必须绑定对应 PR，且每个 live required check 的最新同名 check run 必须在该 `H_mg` 上 `completed/success`。该边界用于排除任意自报 SHA/结论；不要求本地抓取已删除的临时 ref 或执行逐补丁形式化重放。
