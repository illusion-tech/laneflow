...
- `docs-only-v1`：变更文件完整分页后，全部匹配 `docs/**/*.md`、根级 `*.md`（不含 `AGENTS.md`——agent 指令 SSOT 属机器消费面，非惰性文档）或 `research/**/*.md`，且不含任何 `changeType=RENAMED` 文件。
...
快速通道与 `dependabot-cargo-lock-only-v1` 同构：精确机器条件命中后由 evaluator 注入机器 completion 证据（两条新通道的 provider 为通道名，actor 为 `github-metadata`，completion 时间取 head commit 时间），免外部 review；它是机器 completion 而非 waiver，不需要 `external-review-waiver:v1` 记录。三条通道共享同一失效闸门：任何受信 actor 的 finding（含 P2/P3 deferred，按 findingCount 层面判定）抵达、unresolved blocking thread、dismissed 活动或 files 分页溢出，通道立即失效并回标准路径，不接受人工修补；pre-activation（#406 G1 冻结 `2026-08-20T04:20:39Z` 前）replay 不适用新通道；回标准路径后 deferred、round-cap 等既有语义照常。snapshot 字段完整性前置（任一文件的 `additions`/`deletions` 或 head commit 的 `message` 缺失即失效）仅适用两条新通道；dependabot 通道字段口径不变（旧快照兼容）。files 补页绑定 current head 与 base，补读期间 head 或 base 变化即通道失效回标准路径。
...
- `check-gate-evidence g4` 已成功运行；正常 G4 comment 的 `Gate 断言` 行以规范格式记录语义一致的命令和 `已通过` 结果。合格的 `g3-exception:v1` historical replay 保留 `未通过` 并输出 `accepted_exception`；合格的历史合并失败接受使 G4 通过，但原失败保持 `historical_failure`，两者都不得描述为原失败 Pass。`待运行`、无结构化记录的失败、缺少成功标记或参数不匹配不得通过 G4。
- `2026-08-20T04:00:00Z` 是 Merge Queue G4 证据的固定 activation boundary；该时刻后合并的 Delivery 或 Related PR 都必须在 `merge-queue-g4-evidence:v1` 中各有一条 record：普通成功使用 `merge_queue`，只有满足下述接受协议的历史失败可以使用 `historical_failure`。validator 按 Delivery-first、随后 Issue Related PR 顺序验证完整集合，把每条 `H_pr` / `H_main` 与 GitHub `headRefOid` / `mergeCommit.oid` 对照；普通成功还必须精确绑定 `H_mg` checks、规范 chain 与 `H_pr...H_mg` inclusion compare。
- 普通 `merge_queue` record 的 G4 live validation 还须从 GitHub API 读取 `merge_group` workflow runs、PR timeline 与目标分支全部当前生效的 rules；live `merge_queue` rule 必须仍存在。通常合并前最后一个 queue event 必须为 `added_to_merge_queue`；GitHub 成功完成队列合并时可能由 `github-merge-queue[bot]` 在 `merged` 前自动记录终态 `removed_from_merge_queue`，validator 只在该事件直接邻接唯一 `merged`、前一个 queue event 为有效入队且秒级时间满足 `queued_at <= removed_at <= merged_at` 时接受；同秒顺序只由这两个 timeline 事件的邻接证明。人工/缺失 actor、其他 bot、非邻接移出、缺少先前入队或异常时间继续失败关闭。终态自动移出路径还以 `removed_at` 严格截断可信证据窗口：PR-bound `merge_group` run 的 `created_at/updated_at`、live required check completion 与 advanced CodeQL analysis 必须严格早于移出；不同 API 的同秒时间没有可信顺序，不能补证。通常路径则继续以 `merged_at` 包含式截断。最后一代 queue head 必须等于记录的 `H_mg` 并存在 trusted success run；每个 live required check 还必须以 `started_at >= queued_at` 且 `started_at <= completed_at` 绑定当前入队，只按窗口内已完成的最后一个同名 check run 判定，并在该 `H_mg` 上 `completed/success`。两个 CodeQL checks 还须绑定 `integration_id=15368`，并存在同一 `H_mg` 的 advanced workflow analysis。该边界用于排除激活后回滚、任意自报 SHA、人工或失败出队后补跑再直合、原样重新入队时复用同一 `H_mg` 的旧 check、同次入队内被替换的旧 queue head、合并后 rerun 或旧队列结论；不要求本地抓取已删除的临时 ref 或执行逐补丁形式化重放。
- 全部关联 PR 都早于 boundary 合并的历史 G4 可继续无 record 重放；若一个 Issue 同时包含 activation 前后 PR，则 record 仍须覆盖全部成员，历史成员使用 `pre_activation`、保存 `H_pr/H_main` 与非空 reason，禁止补造 `H_mg`。boundary 后的非队列 merge 默认失败关闭。唯一已冻结迁移是 #451 Related PR #452：它在 #451 G1/G2 明确授权下先部署 advanced CodeQL workflow、且早于 live Merge Queue transaction 合入，记录必须使用 `activation_bootstrap`、精确 reason 与 `bootstrapEvidenceUrl=https://github.com/illusion-tech/laneflow/issues/451#issuecomment-5352404197`，不得填写 `H_mg` 或 queue checks；validator 同时固定 repository、Issue、PR、role、`mergedAt` 与未编辑授权 comment。该一次性记录不开放其他直合例外；新的例外仍必须先通过独立 Issue 扩展结构化 record 与 validator，不能只改 `- 合并：` 文本。
...
activation 后已经合并、但无法证明普通 Merge Queue 成功的 PR，只能使用通用历史失败接受协议：

- 历史合并失败记录（historical merge failure record）的 `mode` 为 `historical_failure`，如实保存 `H_pr/H_mg/H_main`、非空 `reason`、`failureEvidenceUrl/recoveryEvidenceUrl`，省略全部 checks / inclusion 成功字段；`H_mg` 必须绑定该 PR 最终队列事务的 live `merge_group` identity，但对应 run 无需成功，原失败不得追授 Pass。
- 同一未编辑 G4 comment 必须包含唯一 `- Historical queue recovery：` 行与历史失败接受记录（historical failure acceptance record）`merge-queue-g4-recovery:v1`；其 `schemaVersion: 1`、`decision: accepted_exception`，`failures` 与全部 `historical_failure` records 一一对应，逐项记录 `failedPr/failedRole/failureEvidenceUrl/remediationIssue/remediationG4Url`，并提供非空 `risk/acceptanceBoundary/cleanupOwner/authorizedBy`，其中 author 与 `authorizedBy` 必须是同一 trusted G3 Owner。
- failure evidence 必须可信、未编辑，并以唯一 `Historical failed PR/H_mg/reason` 行精确绑定失败身份与原因；独立 remediation Issue 必须已有 trusted 未编辑 G4 记录、Gate Ledger 回链、Project `Done` 和手动关闭，其 G4 以唯一 `Historical failure evidence` 行精确回链。时序严格为 failed merge < failure evidence < remediation G4 < accepted_exception；全部引用、身份与时序由 validator 对照 live GitHub 失败关闭。

remediation 的 PR 数量、角色、push 次数与排队方式属于对应 Issue 的动态验收，不进入永久协议。
