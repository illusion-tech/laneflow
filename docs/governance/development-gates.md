...
- G4 live validation 还须从 GitHub API 读取 `merge_group` workflow runs、PR timeline 与目标分支全部当前生效的 required-status-check rules；合并前最后一个 queue event 不得是 dequeue，最后一次入队到 merge 之间对应 PR 的最后一代 queue head 必须等于记录的 `H_mg`，并存在 trusted success run；每个 live required check 只按 merge 时刻前已完成的最后一个同名 check run 判定，且必须在该 `H_mg` 上 `completed/success`。该边界用于排除任意自报 SHA、同次入队内被替换的旧 queue head、合并后 rerun 或旧队列结论；不要求本地抓取已删除的临时 ref 或执行逐补丁形式化重放。
