...
规划状态：2026-07-18，里程碑跟踪议题（Issue）为 #120。#122 的 ADR 0012 继续描述迁移前当前 `f64` 生产实现；#140 推翻旧补偿候选和无性能收益前提后，#141 已通过 G1，并由 ADR 0014 接受下一目标契约：单 edge 10 km 硬上限，`EdgeLength` 与单值热状态使用经过检查的 `f32`，`EdgeProgress` 使用补偿残差感知的高位/残差表示，route 距离由完整证据选择布局，Spatial 标准坐标继续使用 LaneFlow 自有 `f64`。生产切换仍受 #125/#126、独立原子转换、#127 的正确性/完整内存/性能闸口和 #128 收口约束；目标已接受不等于当前实现已迁移。#123 已通过 G1，接受 ADR 0013、独立空间层、配套空间制品、LaneFlow 自有类型和首版无生产几何依赖；Spatial 生产实施继续按自己的闸口推进。
