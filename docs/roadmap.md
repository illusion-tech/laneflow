...
规划状态：2026-07-20，里程碑跟踪议题（Issue）为 #120。#122 的 ADR 0012 继续描述当前 `f64` 生产实现；#140 推翻旧补偿候选和无性能收益前提后，#141/ADR 0014 接受下一目标契约：单 edge 10 km 硬上限，`EdgeLength` 与单值热状态使用经过检查的 `f32`，`EdgeProgress` 使用补偿残差感知的高位/残差表示，route 距离由完整证据选择布局，Spatial 标准坐标继续使用 LaneFlow 自有 `f64`。#125 拆分 current-f64 九个领域数值 owner，#126 审计 API/Data 边界，#127 完成 target-f32 离线标定；#144 的完整原子生产候选通过正确性与内存护栏，但稳态性能只提升 `4.257%`、未达到 `5%` 门槛，已完整回退 current-f64。#128 按该不迁移（no-go）事实收口；未来若重启数值迁移，必须新建议题并重新进入 G1。#123 已通过 G1，接受 ADR 0013、独立空间层、配套空间制品、LaneFlow 自有类型和首版无生产几何依赖；Spatial 生产实施继续按自己的闸口推进。
