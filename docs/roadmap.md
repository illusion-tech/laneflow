...
规划状态：2026-07-19，里程碑跟踪议题（Issue）为 #120。#122 的 ADR 0012 继续描述迁移前当前 `f64` 生产实现；#140 推翻旧补偿候选和无性能收益前提后，#141 已通过 G1，并由 ADR 0014 接受下一 Core 数值契约：单 edge 10 km 硬上限，`EdgeLength` 与单值热状态使用经过检查的 `f32`，`EdgeProgress` 使用补偿残差感知的高位/残差表示，route 距离由完整证据选择布局。#125 只拆分 current-f64 九个领域数值 owner，#126 审计 API/Data 边界，#127 离线标定 target-f32 并完成正确性/完整内存/性能闸口，#144 负责原子生产迁移，#128 负责收口；目标已接受不等于当前实现已迁移。#123 已通过 G1，接受 ADR 0013、独立空间层、配套空间制品、LaneFlow 自有类型和首版无生产几何依赖；#133/ADR 0015 后续把 Spatial runtime 精度修订为每轴 `±16_384 m` 的有界 canonical `f32` frame，并保留其他分层与权威决策。Spatial 生产实施继续按自己的闸口推进。
