...
**最后更新**: 2026-09-06
**适用范围**: LaneFlow 版本路线图与交通基础设施的长期演进

本文记录 LaneFlow 的稳定路线图和已接受长期目标。GitHub Project 负责当前执行
状态，本文负责已接受版本边界与仍需各后继 Issue 独立完成的实施 Gate。

ADR 0021 定义可嵌入、引擎无关、确定性的道路交通运行时与工具链，以及宿主应用、
出行编排、路径规划和交通运行时的职责边界。局部走廊、园区和背景车流是渐进验证
场景，单世界规模演进需要独立研究与证据；中国交通规则和城市工作负载保留为通用
技术验证场景。城市经济、出行需求、土地利用与游戏规则继续属于宿主。长期方向
不表示当前已实现能力，也不构成交付时间承诺。
...
4. 城市拓扑/需求/运行时工作负载 `LF-CN-URBAN-v1`，以
   [首版交付合同](design/chinese-style-city-workload.md)为准：七项有限场景组合已有
   多阶段信号、左转待转、干支路让行、方向性需求与停车能力，两档完成后关闭 #304。
   公交运营、出租派单、上下客与动态路侧摩擦不属于本版交付；是否增加这些宿主业务的
   技术验证，由 #21 的后续范围规划决定并单独立项。非机动车/步行仍由 #236 提供独立
   产品与研究输入，不作为 #304 的隐含验收项。
