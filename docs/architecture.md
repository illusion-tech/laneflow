...
**文档状态**: Accepted  
**最后更新**: 2026-07-22<br>
**适用范围**: LaneFlow 分层、Rust crate 依赖方向、Traffic Data、Signals、Parking、场景人口与 Core/Adapter 边界
...
v0.8 的场景人口与回流采用 ADR 0016 的 caller-owned engine-neutral controller，不进入 `CoreWorld::step` 隐藏状态，也不由 Bevy ECS 选择 route。Core 只拥有原子 old-handle/new-handle replace 与交通 invariant；Adapter 以 typed transaction 原子切换同一 Entity 的 handle binding；Population Controller 按 fixed-step input sequence 拥有 seed、portal/lane 决策和 blocked retry。Traffic/Spatial/Manifest 继续是静态制品，不持久化目标人口、runtime handles 或 Entity。完整目标见 `design/example-scenarios.md`；production 实施仍待 #185–#189。
