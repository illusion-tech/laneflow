...
#30 曾将 v0.2/v0.3 序列化字段冻结为 `laneGraph.edges[].connections[].to`；current v0.4 已按 #94 直接迁移为 `toEdgeId`。其 directed connection 语义不变，terminal edge 继续使用空 `connections` 数组。
