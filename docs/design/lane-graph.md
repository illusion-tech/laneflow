...
- edge length：编制米制先量化再检查；已提交值为 `u32` mm，`100..=10_000_000`（ADR 0028）。
- edge speed limit：编制米制先量化再检查；已提交值为 `u32` mm/s，
  `1..=100_000`（ADR 0028）。
