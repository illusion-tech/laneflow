...
#126 冻结下一唯一有效的交通数据（Traffic Data）版本为 `formatVersion: "0.6"`。当前 v0.5 在 #144 合入前继续是唯一生产加载器，只为保持主干内部一致；项目当前没有外部用户、已发布数据资产或支持窗口，因此不建立 v0.5 兼容或迁移责任。
...
#144 必须在同一交付 PR（Delivery PR）中原子切换 `CURRENT_FORMAT_VERSION`、`laneflow-data-v0.6.schema.json` 及其 `$id`、`schemas/publication.json` 的当前指针、私有 DTO、规范化、标准固定样例、加载器/模式/端到端测试与当前文档。切换后只接受 v0.6；不保留 v0.5 分派、有效样例/测试、弃用别名、双精度开关、自动拆边（edge）、静默截断或迁移工具。仓库内资产直接随该变更更新，历史由 Git 保存；ADR 0011 已要求的不可变模式（schema）发布制品不进入有效加载器。

#127 负责在 #144 开工前冻结九个目标 `f32` 固定绝对阈值、`EdgeProgress` 运算链和路线距离（route-distance）生产布局；#144 不得用当前 `f64` 的 `1.0e-9`、动态相对误差、运行时末位单位（ULP）或通用近似比较辅助函数代替该输入。
