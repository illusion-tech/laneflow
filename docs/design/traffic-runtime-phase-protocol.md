...
[单世界并行设计](traffic-runtime-parallel-execution.md)以本协议为串行语义基线，
细化首版独立计算任务、P4 规范串行、逻辑首错与完整 join；该文已 Accepted，
不改变本文现行串行合同，也不表示已经有生产多 worker 实现。
...
| `live_order_index`                                                                                                      | Derived                                  | `live_order` 已登记前缀的 slot 到位置缓存，按完整句柄复核；只追加时补齐，删除/替换/重建时失效；停车命令预留失败回退全量投影，不进入快照或日志                      |
| `spawn_overlap`                                                                                                         | Derived                                  | 道路准入/切换重验证候选缓存；命令与生命周期增量维护，车辆 tick/候选整值改写后失效，查询时重建；晋升随候选交换，不进入快照与摘要                                    |
| `parking`                                                                                                               | Committed resources                      | 显式/虚拟占用和 binding 的唯一停车聚合。binding 按车辆槽位下标存放并核对句柄代际；安装时按 `vehicle_capacity` 建槽，更高下标只在写入预检或迁移暂存受检扩容，查询不分配。step 只检查预约和产生到达观察，不暗中执行 park/unpark 命令 |
...
| `frontier_maintenance`                                                                                                  | Workspace                                | #740 近门名单、冲突距离缓存和生命周期增量。待发布名单由本拍运动结果分类。已发布名单只在 P7 替换；失败不丢弃上一份已发布名单。不是已提交权威，不进快照。合同见 `traffic-runtime-near-gate-frontier.md` |
...
exhausted、余量与速度。停止距离不参与下一速度求解；其后的降速边界截断只会
取更小的位移，新增上限不小于原最终位移时结果仍相同。`BeyondFinite` 不缩短本拍
有限位移。缓存结果还不能已经越过该屏障 hop。SI 比较覆盖大距离的浮点舍入，
...
活动车辆仍每拍计算运动。冲突准入、等待入口和信号门的停止查询，只在该位置有可能收紧本拍位移时执行。跟车前视窗不使用这条上界：远处前车仍可能改变本拍加速度。

上界取拍初速度、profile 最大加速度和本拍固定步长：

```text
reach_mm = speed_mm_s × Δt + 500 × max_accel × Δt² + 50
```

`500` 是 `0.5 × 1_000`，把米制加速度项换成毫米。末尾 50 mm 覆盖积分误差、微米舍入和 `carry_um`（`0..=999`），车辆仍停在屏障前。比较使用浮点上界对整数毫米的严格大于；等于上界时仍查询。

已证明范围是速度 `0..=100_000` mm/s、最大加速度 `0.5..=50` m/s²、步长 4–1_000 ms。上界非有限或超出该范围时，本拍做完整查询。

路线编译在 `conflicts` 与 `waiting` 完成之后，为每个路线 occurrence 保存最近冲突准入和最近等待入口。距离从该 occurrence 起点量到准入门或等待入口的后继 occurrence 起点，不加车长，也不使用 passage 入口或车尾净空。`None` 表示后面没有该类屏障。有限距离和 `BeyondFinite` 分开表示。`BeyondFinite` 只提供 `u32::MAX - progress` 这个保守下界，用来回答“是否肯定够不着”；它不是停止距离，也不能充当“没有屏障”的哨兵。索引缺行同样不是“没有屏障”，本拍改做完整查询。循环路线里重复出现的边各有一行。这列只活在编译路线的内存热表里，不进入快照、公开 API 或 Adapter。

`progress_mm` 和 `carry_um` 同时为 0 时，冲突查询保持原路径，包括它里面的等待入口搜索。这个条件处理的是正好站在 occurrence 边界上的车，不是所有速度为 0 的车。

三类查询独立：

- 冲突准入远于上界时，跳过冲突出现项搜索；等待入口仍按自己的距离决定。
- 等待入口远于上界时，跳过 `compiled.waiting` 里的未授权入口搜索。这不跳过 `waiting_stop_for`：本拍 Waiting 计划的校验和停止约束仍执行。
- 信号沿 `next_controlled` 推进。解释一扇门之前，先看从当前进度到该门的距离。够不着就结束本拍信号停止查询，不再解释更远的门。近处绿灯、远处红灯时，只解释上界内的门。
- `hard_room` 仅在当前边剩余长度远于上界时，不再为这一次包络调用 `hop_permitted`。`apply_travel_mm` 真正跨 hop 时的 `hop_permitted` 保留。

最近几何门已经获准，不能据此跳过后面的未授权门。只要最近几何位置仍在上界内，就继续原来的授权搜索；下一扇未授权门在上界内时仍然停车。两类都远于上界时，可以不读取本拍 grant、已有 reservation 和 Waiting membership。

上界读取失败不得抢在 Parking、Waiting 计划或 Conflict 不变量错误之前制造新的 `NonFiniteMotion`。MotionPreview 的复用证明保持原条件：P2 里某次“够不着”不能让 P5 无条件接受缓存。P2 与 P5 的运动内核使用同一个上界计算来裁剪信号门和 `hard_room` 许可读取。冲突准入与等待入口的裁剪在 P5 的生产查询里完成。

判断只使用本拍初始状态。本拍够不着，不记录到下一拍。不改 frontier 名单、跟车公式、红灯到达下界、公开 API、数据格式或 Adapter。产品不保留研究开关。
...
零输出判定只允许省略已证明无结果的生成工作，不撤销完整性证明。对当前计数与
生成分开的输出路径：

- 零转移事件仍先完成首遍完整校验、计数与容量处理，才可省略第二遍生成；成功
  提交仍以空批次替换上一拍事件，失败仍保留上一成功批次。
- 零非入口 Gate 只说明没有这类决策，不能推导没有 Waiting 决策或转移事件。
  首遍计数、溢出检查与容量处理不省略；已有决策的规范排序和后续转移事件暂存
  仍须执行，不能提前返回整个输出收尾。

这些约束不强制未来实现采用两遍扫描；遍历融合或其他等价改写仍须保持现行
校验覆盖、首错、事件内容和规范顺序，以及失败零提交与重试结果。
...
- 迁移日志未开启时，可以省略日志专用的状态比较、路线定位和 delta 物化；车辆
  状态写回、资源提交及 tick/time 推进不依赖日志是否开启，不得一并跳过。
...
完整 step 在进入可授权的准备阶段前保存 Conflict 私有序号基线；失败时先撤销本次
暂存并销毁全部 grant，再恢复基线，避免失败 attempt 消耗临界序号额度。成功提交
保留序号推进。普通资源 discard / expire 不承担序号回退，因为它们也用于正常提交
和恢复；序号恢复仅属于已回收全部本次凭证的 step 失败出口。
