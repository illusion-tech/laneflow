...
- Traffic v0.7 target、SpatialPackage v0.1 和 ScenarioManifest v0.1；
...
本文件冻结设计目标，不声称 production schema、Core API、Adapter API 或 native example 已经实现；实施由 #185–#189 承担，#195 独立收口。
...
Traffic v0.7 target 在每个 lane edge 上要求严格正、有限的 `speedLimit`，单位为 m/s：
...
- Traffic package v0.7 target：lane graph、14 routes、vehicle profiles、Signals、Parking 空集合和 per-edge speed limit；
...
- 下游 #185–#189 只有分别通过 G2 后才能开始 production 实施；
