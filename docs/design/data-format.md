...
**文档状态**: Accepted  
**最后更新**: 2026-07-25<br>
**适用范围**: 当前 Traffic v0.8、SpatialPackage v0.1、ScenarioManifest v0.1 与保留的 Data v0.6 数值研究输入
...
本文定义 LaneFlow 当前唯一 active 的 v0.8 external package。它是 checked-in schema、
...
三类 topology arrays、`signals`、`parking` 及其全部子数组均必填，可以为空。
当前 canonical fixtures：

- `examples/data/v0.8-parking-signals-baseline.laneflow.json`：完整 topology/Signals、
  area-owned 与 standalone spaces。
- `examples/data/v0.8-empty-signals-and-parking.laneflow.json`：显式空 static arrays，
  承接 route/profile/repeated-edge 行为回归。
- `examples/data/v0.8-signalized-corridor.laneflow.json`：2 Junction、8 Movement、
  20 ManeuverPath/Gate 的 generator artifact。
...
- edge length 必须 finite 且严格大于 current v0.8 继承的 `1.0e-9 m` exclusive minimum；Data 不导入 Core 私有数值策略。
...
- entry/exit anchor edge 必须存在；progress 必须 finite，并严格满足 `1.0e-9 m < progress < edgeLength - 1.0e-9 m`。该值是 current v0.8 继承的 anchor 数值事实。
- geometry 以 entry edge 的正向切线为局部基准；`abs(lateralOffset) > 1.0e-9 m`，heading 位于 `[-PI, PI)`，length/width 严格大于 current v0.8 继承的 `1.0e-9 m` exclusive minimum。lateral offset 与 extent 在测试中分别拥有语义，不从 Core 公共常量导入。
- External package 不持久化 reservation、occupancy、initial parked vehicles、runtime handles、maneuver path 或 world transform。

停车场、专用路边停车区和 standalone 路边泊位复用同一 `ParkingSpace` 模型；current v0.8 static data 不加入影响 Core 行为的 lot/curbside kind。完整 runtime/lifecycle 契约见 [`parking-system.md`](parking-system.md)，已由 #108/#109 交付并由 #110 完成端到端验证。
...
- `laneflow-data`：version header、private v0.8 DTO、JSON/units/path 和 external-to-Core 转换。
...
- `LoadedPackage` 只表示 current v0.8，并持有已验证的 `InitialTrafficData`。
...
- 当前 v0.8 继续拥有交通边外部 ID、Core 边长、基础限速、拓扑、路线、信号与停车边相对数据。
...
（no-go）结论，v0.6 从未成为 current/published production format。当前 v0.8
继续使用既有 `f64` 数值域；以下内容仍是未来数值迁移输入：

- 当前 v0.8 的线格式 DTO、模式范围、加载器诊断和 `f64` Core 规范化在未来数值迁移前保持当前实现行为；不增加逐字节、旧范围或旧诊断兼容证明；
...
- 有效代码树仍只维护一个当前加载器。未来切换后不叠加 v0.8 运行时兼容分支，不自动拆 edge、不静默截断；仓库内资产随迁移直接更新，不实现离线迁移工具；
