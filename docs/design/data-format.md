...
ADR 0014 接受了下一 Core/Data 数值契约；#126 进一步冻结下一唯一有效的交通数据（Traffic Data）版本为 `formatVersion: "0.6"`。当前 v0.5 在 #144 合入前只负责维持主干内部一致，不形成外部兼容承诺：
...
- 模式（schema）文件名/`$id`、发布目录的当前指针、私有线格式 DTO、加载器版本闸口与路径诊断、标准固定样例、Core 构造器、测试和当前文档必须由 #144 在同一交付 PR（Delivery PR）中更新；
- 有效代码树仍只维护一个当前加载器。切换后不叠加 v0.5 运行时兼容分支，不自动拆 edge、不静默截断；仓库内资产随 #144 直接更新，不实现离线迁移工具；
- 规范化和批量命令继续执行“先计算、后提交”，任一范围、转换或引用错误不得留下部分 `InitialTrafficData` 或 world 状态。

#127 拥有九个目标 `f32` 固定绝对阈值、`EdgeProgress` 运算链和路线距离（route-distance）布局证据；#144 只消费其 G4 后的结果并实施生产转换，不重新发明阈值。已公开且受 ADR 0011 约束的历史模式（schema）可以作为不可变静态制品保留，但不进入有效加载器、固定样例或规范化测试；其他 v0.5 历史由 Git 保存。

因此，#126 的文档合入只建立直接迁移输入；只有 #144 的原子 Data/Core 迁移通过正确性、完整内存、性能与 G0-G4 闸口后，本文件的“当前版本”标题和模式链接才切换到 v0.6。
