...
上表是停车资源聚合的目标边界，不表示当前完整生命周期命令已达到相同复杂度。
#678 检查基线中，成功 park/leave 还调用 `rebuild_active_order`，从全部 live 车辆
保序重建 Active 向量；leave 的后车安全检查另扫描 Active。连续 leave 成功会改变
`ObservationStateSequence`，使下一次到达占用查询的命令重建索引。重复拒绝不改变
该序号，因而复用同一索引；不能把重复拒绝的优化当成连续成功的优化。

对只包含 leave、且到达安全检查的批次，设 `L` 为 live 数、`A_i` 为第 i 次命令前
Active 数、`S` 为成功数、`J` 为成功完成的占用重建次数（不含分配失败重试）。该基线
成本包括逐命令的 Active
访问及其路线出现项/leader 查询、`J` 次完整占用构建，以及 `S × L` 次 live 访问和
相应 Active 写入。冷索引下 `J <= S + 1`；最后一次成功之后没有读取就不会额外
重建。占用构建按边数、实际车身片段数和桶内排序计成本，不能总以车辆数代替片段数。
调研入口、有限结构比较和测量口径见
[#678 命令研究](../../research/issue-678-parking-commands/README.md)。

内部等价优化须保持以下约束，不改变 §5.5 的准入判据：

1. Active 仍是 `live_order` 的保序投影。局部插入/移除不能以 slot 下标、hash 遍历或
   公共调用之外的批量延迟替代正式顺序；完整句柄世代和首错顺序必须保留。
2. 顺序定位缓存只属于 Derived，可由当前 live 顺序重建；spawn、despawn、replace、
   恢复和修订切换必须同步维护或失效。park/leave 仅改变状态时可以复用定位。
   所有可能失败的预留先于提交；可选缓存失败可以回退既有计算，不能增加部分提交。
3. 局部占用更新或覆盖层必须在每次成功命令返回前建立下一命令可见的完整状态；
   删除、槽位复用和 route/footprint 改变不能留下旧记录。占用记录中的顺序键、
   suffix minima、同车排除和跨边片段都属于等价性证明，不能只更新出口所在桶。
   来源标记与覆盖层只能在成功提交后一起前进；失败、step、restore/cutover 的失效
   边界须在实施设计中逐项闭合。固定 step 的正式索引仍消费唯一提交态。
4. 后车候选剪枝必须证明覆盖上游、所有合流分支、循环/重复出现项和跨边车身，并
   按原稳定顺序执行精确谓词。仅查出口边、固定一层 predecessor 或仅按物理边去重
   不能作为完整性证明；没有证明时保留现有全扫描。

#696 保留 Active 向量，在成功 park 时保序移除，在成功 leave 时按 live 位置二分
定位后插入。定位表属于 Derived：slot 对应 `u32` live rank，查询同时核对完整句柄，
不以 slot 顺序替代 live 顺序。第一次成功 leave 在所有领域检查通过后、提交前按
实际槽位长度可失败预留；失败退回原有全量重建，不新增公共错误或改变首错。
仅追加 live 时复用旧前缀并补录新增成员；删除、replace 或整体重建使定位失效。
park/leave、普通 step 和 Completed 不改变 live 顺序，定位仍有效。恢复与跨修订
候选从空定位开始；同修订原地切换保留 live 句柄和顺序，因此可保留定位。

设 `H` 为实际槽位高水位。首次定位含 `O(H + L)` 初始化，热定位为 `O(log A_i)`；
Active 插入/删除仍有 `O(A_i)` 查找或连续内存移动。位置表保留容量按实际槽位增长，
不按声明 vehicle capacity 分配；堆成本为 `4 × positions.capacity()` 字节，另有
向量头和前缀长度的内联成本。连续 leave 成功不再各自扫描全部 live，但后车安全
扫描和 Occupancy 重建未改变。冷首次成功、热成功、全拒绝、高声明容量及高槽位
水位的实际 A/B 见 [#696 测量](../../research/issue-696-active-order/README.md)。
位图、分块顺序和占用覆盖层仍未升级为生产实现方案，不据此新增格式、公共 API
或并行路径。
