...
**文档状态**: Accepted  
**最后更新**: 2026-07-17
**适用范围**: v0.5 Parking 的 current 静态领域/data contract，以及 planned 占用、车辆生命周期、ParkingStop/route 集成、确定性、失败原子性与性能边界
**实现状态**: #105 已冻结设计与 ADR 0010；#106 已交付 lifecycle/performance substrate；#107 已交付 static Parking 与 current 0.5 data；#108-#110 继续实施 runtime、activation 与全面验证
...
不得把 #108-#110 的 planned runtime API/行为描述成已实现事实；#107 的 static registry、loader、schema、fixtures 和 current docs 已是 production 事实。
...
- Core API：#107 已以 pre-1.0 breaking change 新增 static Parking types/handles/registry/errors；runtime snapshot、commands/records/events、`VehicleStatus::Parked` 和 despawn record extension 由 #108/#109 交付。
...
- 已由 #107 原子切换 production current v0.5，同时保持 #108-#110 runtime 未实现边界。
