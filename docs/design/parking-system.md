...
Active 数、`S` 为成功数、`J` 为成功完成的占用重建次数（不含分配失败重试）。当前
成本包括逐命令的 Active
...
最小实施方向是先保留 Active 向量、增量维护停车状态变化带来的成员变动；顺序定位
的构建、失效及额外容量也须进入实际 A/B。位图、分块顺序和占用覆盖层尚未由结构
微测量升级为生产实现方案，不据此新增格式、公共 API 或并行路径。
