...
**文档状态**: Accepted  
**最后更新**: 2026-07-17
**适用范围**: v0.5 Parking 的 current 静态领域/data contract、runtime authority/commands，以及 planned ParkingStop/route 集成、确定性、失败原子性与性能边界
**实现状态**: #105 已冻结设计与 ADR 0010；#106/#107 已交付 substrate 与 static/current data；#108 已交付 runtime/commands/transitional guard；#109/#110 继续实施 activation 与全面验证
...
- 冻结 planned current data format 0.5、migration、canonical fixtures 和验证/性能门禁。

本文是 #105 的 Accepted 实现输入，不是完整 Parking runtime 完成声明。分阶段事实必须保持：
...
4. #108 已交付 runtime commands/query，并用窄 capability guard 保护 Reserved step。
...
不得把 #109/#110 的 moving runtime/全面验证描述成已实现事实；#107 的 static data 与 #108 的 runtime authority/commands 已是 production source 事实。
...
它没有增加 `ParkingArea` / `ParkingSpace` / `Parked`、schema 0.5、loader、reservation/binding、ParkingStop、leave/commit command 或 Adapter API。#107 随后补齐了 static types/data，但停车场或路边停车位仍未成为 production runtime 行为；对外 lifecycle 由 #108-#110 完成。
...
### 9.7 I3 transitional guard

在 #108 已公开 runtime commands/query但 #109 未交付 moving capability 时：
...
- #109 激活后保留 legacy variant，但合法 production world 不再返回；注释与测试必须证明不可达。
...
- `VehicleStatus` planned 增加 `Parked`。
...
- `VehicleDespawnRecord` planned 增加 `parking_release: Option<ParkingReleaseRecord>`。
...
这些是 planned pre-1.0 breaking API；具体交付由 #108/#109 承担。
...
- Core-only `parking_lifecycle` example：load/register -> reserve -> approach/arrival -> commit -> leave -> resume。
- Example/fixture assertions由integration tests复用；benchmark builder复用相同topology generator。
- #110新增 `docs/reference/v0.5-parking-validation.md`，记录commit/dirty status、CPU/OS/rustc/LLVM/target/profile/power mode、workload、commands、三轮raw results、confidence interval、outliers、noise、profile与remaining risks。
...
- Core API：#107 已以 pre-1.0 breaking change 新增 static Parking types/handles/registry/errors；#108 新增 runtime snapshot、commands/records、`VehicleStatus::Parked` 和 despawn record extension；step events 由 #109 交付。
...
- 已由 #107 原子切换 production current v0.5，并由 #108 交付 runtime authority/commands，同时保持 #109/#110 activation/全面验证未实现边界。
