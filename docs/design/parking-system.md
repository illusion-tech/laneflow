...
## 12. Planned current data format 0.5

### 12.1 Staged truth

本节冻结 #107 的实施输入，不改变当前 production v0.4。#107 必须在同一个Delivery PR中原子更新 schema、private DTO、loader、Core normalization、fixtures、examples、format constant和current data docs后，才能把本节改写为已实现 current事实。

### 12.2 Package shape

Planned唯一 current version：
...
Planned production fail-fast order：
...
2. 新增0.5 schema/fixtures/DTO/tests并删除active0.4 schema/fixtures。
3. 仓库active examples同一交付迁移。
4. Current docs在#107中改写；#105只保留planned说明。
5. v0.4输入返回 `UnsupportedFormatVersion`，不自动补empty Parking、不提供shim/converter。
...
- Core API：planned pre-1.0 breaking，新增static/runtime Parking types、handles、snapshot、commands/records/events/errors、`VehicleStatus::Parked`和despawn record extension。
- Data format：planned breaking `0.4 -> 0.5` current-only replacement；无production compatibility shim。
...
- 对齐ADR0003/0005/0006/0007/0008/0009与current v0.4实现边界；
- 明确Core/data/Adapter影响、determinism、error/event order、tests、10k/100k、allocation/memory与activation chain；
- 保持production current v0.4事实，直到#107原子切换。

若实施发现authority矛盾、局部lookup无法与full-scan语义等价、sparse atomicity必须退化为full-S/V hot path、guard无法保持安全中间主线，或planned0.5数据无法按ADR0008原子切换，必须回到本设计/ADR或拆follow-up；不得用private实现静默改变Accepted语义。
