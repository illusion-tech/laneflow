...
Parking 静态几何与锚点由编译器写入 Canonical LIR / LFCA，再进入共享静态路网。
`formatVersion: "0.10"` 与 `schemas/laneflow-data-v0.10.schema.json` 已删除，不是
现行契约。占用权威在 `TrafficWorld`，不回写静态制品。

历史 JSON package shape 与 loader 顺序只存在于 git 历史。
