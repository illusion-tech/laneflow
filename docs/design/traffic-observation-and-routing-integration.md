...
与具体容器布局才由 G2 落定，且不得改变本文的权威分层、绑定集、时点、失败关闭和
预算口径。
...
度量沿 `LF-P100-REF-01` 同机描述性基线；G2 登记首轮数值，1.0 前只许按实测收紧。
所有条目数、exact bytes、累计分配、峰值 retained bytes、墙钟与 tick 间隔干扰同时
报告，不能用单个平均耗时代替。
...
provenance，或无法复用唯一 route 编译器，必须停止并返回 G1。
