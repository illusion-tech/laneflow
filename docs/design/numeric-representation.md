...
| 数值域                                      | 标量                                                 | 权威归属                                 | 规则                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------- | ---------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LFRE wire 几何                              | `f64`（`Vec3F64` + corridor station/宽度标量）       | 私有 wire / schema                       | FlatBuffers `double`（含 `RoadCorridor` `start/end_station_meters`、`LinearWidthProfile` `start/end_width_meters`），format v3 权威；其它版本失败关闭；`f32` 量化发生在 geometry compile 边界，不在 wire/lowering 层                                                                                                          |
...
| LFRE enum / union 判别值                    | `u8`（wire `ubyte`）                                 | 私有 wire / schema                       | Road Editing v3 合法代码由 exact schema 冻结；所有 zero/unknown 均无效，新增或改义判别值必须提升来源格式版本                                                                                                                                                                                                                  |
...
| compiler 来源描述符版本                     | `u32`                                                | compiler 来源描述符                      | SyntheticDsl `frontendVersion = 4`；RoadEditingSource `frontendVersion = 3`（ADR 0029）；`SOURCE_DOCUMENT_SET_DIGEST_VERSION = 1`。前端版本与文档集摘要版本是不同版本轴，均精确匹配，不并存旧读法                                                                                                                             |
...
| LFRE format_version                         | `u32`（wire `uint`）                                 | 私有 wire / schema                       | `RoadEditingSource.format_version` 为 FlatBuffers `uint`；精确值为 `3`（ADR 0029），其它值失败关闭                                                                                                                                                                                                                            |
...
| 标识 v1（Identity v1）固定域                | `[u8; 16]` / `[u8; 4]` / `u16` / `u32`               | 静态契约（static contract）/ compiler    | `StableId128([u8; 16])`、`IDENTITY_MAGIC: [u8; 4] = *b"LFID"` 与固定 `STABLE_ID_DOMAIN_PREFIX`；encoding/registry version 均为 `u16 = 1`，`EntityKind` / `FieldTag` 为 `#[repr(u16)]`；前像字段数使用 little-endian `u16`，每个字段字节长度使用 little-endian `u32`                                                           |
