...
**最后更新**: 2026-07-18（#125 current-f64 领域策略同步）
...
最小尺寸属于输入语义，不是算术误差阈值；精确零状态和有符号零规范化也不使用 near-zero predicate。production 不提供通用近似比较 helper，不使用运行时相对容差或动态 ULP。#127 可以把相对误差和 ULP 用作 target-f32 离线标定工具，但只能产出固定、带单位、领域化的绝对阈值，并由 #144 原子启用。
...
| edge 长度 / edge 进度             |        `min_edge_length < length <= 10_000 m`；进度在 edge 内 |               `0.0009765625 m` | #125 已分离 current owner；目标值由 #127 标定 |
...
current-f64 的九个 owner 已由 #125 按第 2.2 节拆分，不得在未来的局部 `f32` 路径中直接转换后复用。target-f32 阈值必须由 #127 使用硬范围的最大 ULP、真实运算次数、连续误差上限和 exact/adjacent oracle 离线标定。首轮 `4 * max ULP` 只作为加法、减法和吸附的研究下限：在 `10_000 m` 处约为 `0.00390625 m`，低于 `0.01 m` 上限；它不是 production 常量，不能直接复制进 #144。
...
后续仍需独立交付：#126 Core API/current Data 审计，#127 target-f32 九领域离线标定、长期判定基准、性能与扩展常驻内存账本，#144 原子生产转换，以及 #128 收口审阅；Spatial/v0.7 分别交付真实几何与 Adapter 转换。任一生产闸口失败时保留 current `f64`，不放宽预算或保留双精度开关。
