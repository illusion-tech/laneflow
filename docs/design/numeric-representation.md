...
- 当前生产 Core/Data v0.5 继续使用现有 `f64` 权威和接受范围；#144 的首次完整生产迁移仅获得 `4.257%` 稳态提升，未达到 `5%` 门槛，已按 ADR 0014 完整回退；
...
下一目标交通数据（Traffic Data）版本仍冻结为 `formatVersion: "0.6"`，但 #144 no-go 后没有成为当前有效版本；当前唯一加载器和样例仍是 v0.5。未来原子迁移必须在同一交付 PR（Delivery PR）中切换模式（schema）文件名/`$id`、发布目录的当前指针、版本闸口、私有 DTO、规范化、固定样例、测试和当前文档；不得保留 v0.5 运行时分支、双精度开关、弃用别名或迁移工具。
...
#126 已冻结公开 Core API、Parking 锚点、分层错误载荷、Data v0.6 准确版本与原子替换矩阵；#127 已完成目标 `f32` 九领域离线标定、长期判定基准、性能与扩展常驻内存账本。#144 的原子生产转换因性能门槛失败而 no-go，当前保持 `f64` 且不保留双精度开关。#128 收口必须消费这一真实结论；未来若重启数值迁移，应新建议题并重新进入 G1。空间层（Spatial）/v0.7 仍分别交付真实几何与适配器（Adapter）转换。
