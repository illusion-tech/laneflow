...
| geometry semantics version                  | `u8`                                                 | compiler frontend options        | `ROAD_EDITING_GEOMETRY_SEMANTICS_VERSION = 1`；先于两个 profile code 写入 `frontend_options_digest`，冻结算术图变化时必须提升                                                                      |
...
| canonical 几何硬阈值                        | `f32`                                                | compiler / static contract       | 点分量范围 `[-16_384, 16_384] m`、最短 segment `0.1 m`、source join gap 上限 `0.005 m`、最小 projected-up 长度 `0.008_726_535`                                                                     |
...
| LIR semantic fingerprint                    | `[u8; 32]`                                           | compiler 输出                    | `Lir::semantic_digest` 与 `CompilationMetrics::semantic_fingerprint()` 固定为 32 bytes；#298 的 portable artifact 字节完整性 digest 归 #298                                                        |
