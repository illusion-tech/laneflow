...
- compensated f32 修复 constant-addition，但没有修复整个 f32 控制链；
- `f32 compute + f64 progress` 是唯一通过 10k/100k 差分 ceiling 的候选，但在当前 Criterion matrix 中比同布局 f64 慢 `3.79%..38.65%`；
- candidate layout 可从 `128 B/vehicle` 缩到 `80–88 B/vehicle`，但这是 research-only vehicle/motion vectors，不是完整 Core 的已实现总内存收益；
- 因此 v0.6 没有足够正确性、性能和迁移收益支持 production scalar 改动。
...
3. 同一代表性 10k/100k workload 的 f64/raw-f32/compensated/mixed 时间与 candidate memory；
