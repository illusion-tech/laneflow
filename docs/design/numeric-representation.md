...
| compiler 几何配置档代码                     | `u8`                                                 | compiler frontend options              | Accuracy `Fine/Balanced/Compact = 1/2/3`，Direction `Smooth/Balanced/Compact = 1/2/3`；`frontend_options_digest` 输入顺序固定为前导 `ROAD_EDITING_FRONTEND_OPTIONS_DOMAIN = b"laneflow.road-editing.frontend-options.v1\0"`、semantics version、accuracy code、direction code、`generator_options_digest: [u8; 32]`                        |
