...
#296 引入的 compiler 前端遵循已注册管线 LFRE wire → 有类型抽象语法树（typed AST）→ 高层中间表示（HIR）→ topology/geometry 中层中间表示（MIR）→ 已验证规范低层中间表示（validated canonical low-level intermediate representation，canonical LIR），并原子产出已验证源映射输入（validated source-map input）。该管线沿 ADR 0022 五层几何表示计量，不在制品/运行时整数毫米与 Spatial `f32` 之外新增第四种权威；canonical 折线自 geometry compile 起以有界 `f32` 贯穿高层中间表示（HIR）内部 `SpatialHir` 子表示、中层中间表示（MIR）与 LIR。`f64` 只保留在 road-editing analytic/reference 子表示，以及准入前的 LFRE 非几何交通/静态 wire。提案中（#500）：准入后交通一维在 Typed AST / HIR / MIR / LIR 存整数毫米或受检 `f32` SI。当前 IR 仍存编制 SI 米。每一域显式归属为：
...
| LFRE 非几何交通/静态标量                    | wire `f64`；准入后提案中为 mm / 受检 `f32`           | 私有 wire / schema；准入后为 compiler IR | LFRE wire 的限速、Parking、`VehicleProfile` 等为编制 `f64`（限速有限且严格 `> 0`）。准入先量化再闭包。提案中（#500）：之后 Typed AST / HIR / MIR / LIR 存整数毫米或受检 `f32` SI，发射只写已提交值。当前 IR 仍存编制 SI 米。几何路径的「禁用后端窄化」不适用于交通一维准入量化。                                              |
