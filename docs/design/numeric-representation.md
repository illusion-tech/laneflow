...
| compiler 来源描述符版本                     | `u32`                                                | compiler 来源描述符                      | SyntheticDsl `frontendVersion = 3`（准入后交通一维以整数毫米 / 受检 `f32` SI 写入 `LFSOURCE`）；RoadEditingSource `frontend_version = 1`；`SOURCE_DOCUMENT_SET_DIGEST_VERSION = 1`。前端版本与文档集摘要版本是不同版本轴，均精确匹配，不并存旧读法                                                                            |
