...
**文档状态**: Accepted（已提交一维几何为整数毫米；Road Editing 3 / 来源编号修订已纳入现行权威）
...
- 现行合同是 ADR 0028：已提交一维几何为整数毫米 + 微米余数，速度为 `u32` mm/s，步长 `4..=1000` ms，有折线边长由规范 `f32` 弧长派生后量化并写回 IR，`max_accel` / 舒适与紧急减速度均 `>= 0.5 m/s²`，不另做速度余数。对照门是该契约自洽，不是与旧 `f64` tick 零分歧，也不是 `5%` 墙钟。准入后 Typed AST / HIR / MIR / LIR 交通一维与制品同一套整数，公开 `Canonical*View` 删除米制访问器。
...
§2.1 起的表是 **历史 current-f64 Core** 基线，不是现行 `TrafficWorld` 合同。现行
已提交一维几何见 ADR 0028。历史实现当时不是单一字段上的 `f64` 选择，而是贯穿多个边界：
...
当前一万/十万车辆工作负载中存在把全部车辆铺在单条超长边（edge）上的规模固定样例。十万车辆自由流场景的边可达到约 `25_001_000 m`，该数量级的 `f32` 末位单位（ULP）为 `2 m`，已经大于常见固定 tick 的单步位移。它应继续作为迁移前范围的差分/规模探针，但不能单独代表局部坐标产品工作负载；性能比较还需要等车辆数、等密度、但保持边/局部坐标在验证包络内的配对固定样例。
...
| canonical 几何与位姿               | LaneFlow 自有的有界 `f32`              | Spatial            | 每轴 `±16_384 m`，加入独立 Core 长度量化余量            |
| frame 到宿主位姿                   | 宿主选择的 f32/double placement        | 派生快照，不是权威 | Adapter 显式映射 frame，不反写 Core/Spatial             |
| 宿主向量/`Transform`               | 宿主专用                               | Adapter 表现       | 只能出现在 Adapter 末端                                 |
| `f16` / 量化整数                   | 仅用于显式编码/缓存                    | 非权威             | 独立缩放/原点/范围/溢出/错误契约                        |

#### compiler 前端数值权威（#296 / #378）

#296 引入的 compiler 前端遵循已注册管线 LFRE wire → 有类型抽象语法树（typed AST）→ 高层中间表示（HIR）→ topology/geometry 中层中间表示（MIR）→ 已验证规范低层中间表示（validated canonical low-level intermediate representation，canonical LIR），并原子产出已验证源映射输入（validated source-map input）。该管线沿 ADR 0022 五层几何表示计量，不在制品/运行时整数毫米与 Spatial `f32` 之外新增第四种权威；canonical 折线自 geometry compile 起以有界 `f32` 贯穿高层中间表示（HIR）内部 `SpatialHir` 子表示、中层中间表示（MIR）与 LIR。`f64` 只保留在 road-editing analytic/reference 子表示，以及准入前的 LFRE 非几何交通/静态 wire。准入后交通一维在 Typed AST / HIR / MIR / LIR 存整数毫米或受检 `f32` SI。每一域显式归属为：

| 数值域                                      | 标量                                                 | 权威归属                                 | 规则                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------- | ---------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LFRE wire 几何                              | `f64`（`Vec3F64` + corridor station/宽度标量）       | 私有 wire / schema                       | FlatBuffers `double`（含 `RoadCorridor` `start/end_station_meters`、`LinearWidthProfile` `start/end_width_meters`），format v4 权威；其它版本失败关闭；`f32` 量化发生在 geometry compile 边界，不在 wire/lowering 层                                                                                                          |
...
| LFRE 非几何交通/静态标量                    | wire `f64`；准入后为 mm / 受检 `f32`                 | 私有 wire / schema；准入后为 compiler IR | LFRE wire 的限速、Parking、`VehicleProfile` 等为编制 `f64`（限速有限且严格 `> 0`）。准入先量化再闭包。之后 Typed AST / HIR / MIR / LIR 存整数毫米或受检 `f32` SI，发射只写已提交值。几何路径的「禁用后端窄化」不适用于交通一维准入量化。                                                                                      |
| Parking / VehicleProfile 硬范围             | 编制 `f64` → 量化毫米 / `f32`                        | compiler / 静态契约（static contract）   | 先量化再闭包：停车锚点相对 **提交后** 边长 `L` 为 `1 <= p <= L-1` mm，横向绝对值 `1..=128_000` mm，长/宽与车长 `100..=128_000` mm，朝向量化并折叠后 `-π <= x < π`；期望车速 `1..=100_000` mm/s，时距 `(0, 60]` s，三项加减速 `0.5..=50` m/s²，min gap `0..=128_000` mm，emergency ≥ comfort                                   |
| LFRE enum / union 判别值                    | `u8`（wire `ubyte`）                                 | 私有 wire / schema                       | Road Editing v4 合法代码由 exact schema 冻结；unknown 均无效，零值按枚举定义判断。Straight、Uncontrolled、GateProhibition::None 的显式零值合法并保留字段存在性；Unspecified 与 union NONE 仍无效，新增或改义判别值必须提升来源格式版本                                                                                        |
...
| compiler 来源前端选择器                     | `u16`（`SourceLanguage`）                            | compiler 来源描述符                      | `SourceLanguage` 连续登记为 `1=SyntheticDsl, 2=RoadEditingSource`，不得与 LFRE wire format version 混同                                                                                                                                                                                                                       |
| compiler 来源描述符版本                     | `u32`                                                | compiler 来源描述符                      | SyntheticDsl `frontendVersion = 5`；RoadEditingSource `frontendVersion = 4`（路权策略实施合同）；`SOURCE_DOCUMENT_SET_DIGEST_VERSION = 1`。前端版本与文档集摘要版本是不同版本轴，均精确匹配，不并存旧读法                                                                                                                     |
| compiler 来源记录字节长度                   | `u32`（受检）                                        | compiler 来源文档描述符                  | `SourceDocumentDescriptor::source_record_byte_len` 为精确 `u32`；超出范围时前端失败关闭                                                                                                                                                                                                                                       |
| compiler 来源文档/文档集摘要                | `[u8; 32]`                                           | compiler 来源描述符                      | 单文档为 source record 的 SHA-256；文档集 SHA-256 前像依次为 `b"LFSOURCE-DOCUMENT-SET"`、little-endian `u32` version、little-endian `u32` document count，再按 key bytes 排序写入 little-endian `u32` key length、key、little-endian `u32` source-record byte length、document digest                                         |
| 几何语义版本（geometry semantics version）  | `u8`                                                 | compiler frontend options                | `ROAD_EDITING_GEOMETRY_SEMANTICS_VERSION = 1`；先于两个 profile code 写入 `frontend_options_digest`，冻结算术图变化时必须提升                                                                                                                                                                                                 |
| compiler 几何配置档代码                     | `u8`                                                 | compiler frontend options                | SHA-256 `frontend_options_digest` 输入顺序固定为前导 `ROAD_EDITING_FRONTEND_OPTIONS_DOMAIN = b"laneflow.road-editing.frontend-options.v1\0"`、semantics version、Accuracy `1/2/3`、Direction `1/2/3`、`generator_options_digest: [u8; 32]`                                                                                    |
| `SourceLocation` 闭合数值族                 | `u32` / `u16` / `u8`                                 | compiler 来源位置                        | `SourceSpan` 一基 line/column、relation occurrence、context ordinal、wire physical index 与 checked byte-range start/length 为 `u32`；table field ID 为 `u16`；struct member ID 与 union discriminant 为 `u8`；byte range 加法或输入范围越界时 fail closed                                                                    |
| LFRE format_version                         | `u32`（wire `uint`）                                 | 私有 wire / schema                       | `RoadEditingSource.format_version` 为 FlatBuffers `uint`；精确值为 `4`（ADR 0029 与路权策略实施合同），其它值失败关闭                                                                                                                                                                                                         |
| 信号 offset/duration 计时器                 | `u64`（wire `ulong`）                                | 私有 wire / schema                       | portable 上限为 `9_007_199_254_740_991` ms；offset 为 `0..=max`，duration 为 `1..=max`，cycle 累加不得超过 max 且 offset 必须小于 cycle                                                                                                                                                                                       |
| 来源沿袭（provenance）随机种子              | `u64`（wire `ulong`）                                | 私有 wire / schema                       | `OptionalU64.value` / `Provenance.random_seed` 为精确 `u64`，compiler 模型保留为 `Option<u64>`；仅 Generated 来源沿袭接受可选完整 `u64` 范围，Direct 来源沿袭必须为 `None`，不得无条件收窄或放宽                                                                                                                              |
| transition index / occupancy 计数           | `u32`（wire `uint`）                                 | 私有 wire / schema                       | `max_occupancy` 必须非零；`transition_index` 必须严格小于引用机动路径的派生 transition count（`path.edges.len() - 1`）                                                                                                                                                                                                        |
...
| 标识 v1（Identity v1）固定域                | `[u8; 16]` / `[u8; 4]` / `u16` / `u32`               | 静态契约（static contract）/ compiler    | `StableId128([u8; 16])`、`IDENTITY_MAGIC: [u8; 4] = *b"LFID"` 与固定 `STABLE_ID_DOMAIN_PREFIX`；encoding version 为 `u16 = 1`，registry revision 为 `u16 = 4`，`EntityKind` / `FieldTag` 为 `#[repr(u16)]`；前像字段数使用 little-endian `u16`，每个字段字节长度使用 little-endian `u32`                                      |
| 有类型抽象语法树（typed AST）解析曲线控制点 | `f64`（`AuthoringPoint3F64`）                        | compiler                                 | ADR 0022：编制解析曲线（Line/Cubic Bézier）控制点在前端以 `f64` 求值                                                                                                                                                                                                                                                          |
| canonical 折线几何                          | `f32`（`CanonicalPoint3F32Input`）                   | compiler                                 | geometry compile 经 `quantize_point` 把 `f64` evaluator 输出受检量化为有界 `f32`；每个零分量经 `canonical_f32` 规范化为 bit-exact `+0.0f32`，禁止 `-0.0` 进入 LIR 语义指纹                                                                                                                                                    |
| 道路里程站位（Stationing）reference 基表    | `f64` 累计弦长                                       | compiler                                 | 配置档无关、按 source curve segment 组织的累计弦长；固定使用 `Fine2Cm + Smooth1Deg`，即 position `0.01 m` / direction half-angle `0.5°` 的 B1 policy；不按 edge 序列累计                                                                                                                                                      |
| 公开几何配置档目标                          | `f64`                                                | compiler                                 | Accuracy `0.02/0.05/0.10 m`；Direction full-angle `1°/2°/5°` 及其冻结 binary64 `cos²`；用于总目标、诊断与最终 canonical direction 检查                                                                                                                                                                                        |
| B1 细分接受阈值                             | `f64`                                                | compiler analytic/reference              | position `0.01/0.025/0.05 m`；direction half-angle `0.5°/1°/2.5°` 及其冻结 binary64 `cos²`；不得用公开 profile target 替代                                                                                                                                                                                                    |
...
| 高层中间表示（HIR）内部 `SpatialHir` 子表示 | canonical 折线 `f32`                                 | compiler（owned）                        | 私有 `SpatialHir` 由 `build_spatial_hir` 在高层中间表示（HIR）构造期间建立；`HirCanonicalPoint3F32`/`HirSpatialSegment` 经 `freeze_spatial_polyline` 冻结。长度校验用 `length_mm / 1000` 作观察值对账弧长                                                                                                                     |
...
| 已验证规范低层中间表示（canonical LIR）折线 | `f32`（`LirCanonicalPoint3F32`/`LirSpatialSegment`） | compiler 输出                            | 运行时规范权威几何。交通一维在制品/运行时与 LIR 均为整数毫米 / 受检 `f32`；发射只写已提交值，不再量化                                                                                                                                                                                                                         |
...
- 当前已提交交通一维为整数毫米 + `carry_um` + `u32` mm/s（ADR 0028）。ADR 0014 补偿残差感知 `f32` 进度是 **#144 历史目标**，经 no-go 回退。compiler 编制曲线保持 `f64` 求值，canonical 折线仍为 `f32` 米。准入后 compiler IR 交通一维不再存编制米列。
...
- road-editing analytic/reference 子表示（编制解析曲线、道路里程站位 reference 基表、配置档阈值、corridor station/宽度）均以 `f64` 存续；canonical 折线自 geometry compile 起量化到有界 `f32` 并贯穿高层中间表示（HIR）内部 `SpatialHir` 子表示、中层中间表示（MIR）与 LIR（ADR 0022 五层表示）。非几何交通/静态标量：LFRE wire 为编制 `f64`；准入后 IR 为整数毫米 / 受检 `f32`。
- 道路里程站位（Stationing）是 authoring 曲线的配置档无关累计弦长基表，按 source curve segment 组织，不属于运行时 edge 序列里程。
- 受检的几何 `f64 → f32` 量化只发生在 geometry compile 的 `quantize_point`（`geometry.rs`）：对 evaluator 输出分量做 canonical 范围检查后再 `as f32`，并由 `canonical_f32` 把每个 signed zero 规范化为 bit-exact `+0.0f32`；不允许在 wire/lowering 以隐式 `as`/截断提前量化折线点，也不允许在高层中间表示（HIR）内部 `SpatialHir` 子表示、中层中间表示（MIR）或 LIR 中对折线点二次量化（ADR 0022）。交通一维的毫米 / 受检 `f32` 量化发生在编制准入，不是这条几何路径。
- offset horizontal-regularity proof 在细分前对 binary64 derivative controls 执行独立 interval walk，证明水平导数在整个 source segment 不为零；该检查不产生 canonical point，也不能由端点 distance/direction test 替代（ADR 0022 220–244）。
- subdivision acceptance 与最终 canonical direction 检查把已量化的 `f32` 弦端点经 `promote_point` 无损提升回 `f64` 后做 distance/direction test（ADR 0022 246–280），保证判定阈值与 authoring numeric freeze 共享同一 binary64 `cos²`；这不授权在 road-editing analytic 子表示之外使用 `f32` 权威或未受检窄化。
- corridor station 端点（`s0/s1`）与宽度端点（`w_i`）从 wire 到有类型抽象语法树（typed AST）全程保持 `f64`（ADR 0022 166–190）；offset 求值 `O(t) = B(t) + d(s(t))·L(t)` 在 `f64` 完成后才经 `quantize_point` 量化，不在 offset 中间量提前窄化。

研究结果解释：

- 直接 `f32` 的 36,000 tick 进度漂移和密集/走走停停控制误差超过预算；
- #122 的旧补偿 `f32` 只在写入时维护残差，读取前车间距、edge 剩余距离、边界和快照时仍只使用高位分量；它没有实现完整的补偿进度语义；
- #140 的补偿残差感知 `f32` 补全读取语义后，通过 5 种 edge 布局、3 种一万车辆场景和十万车辆密集观测的原严格上限，且无离散分歧；
- “`f32` 计算 + `f64` 进度”不再是唯一通过候选。#140 同轮研究模型中，补偿残差感知 `f32` 相对 `f64` 在 10 km 上界的一万车辆场景快约 2.4%–5.8%，十万车辆密集场景快约 7.1%；该结果不足以直接外推到生产；
- 候选布局可从每辆车 `128 B` 缩到 `80–88 B`，但这是仅供研究的车辆/运动向量，不是完整 Core 已实现的总内存收益；
- ADR 0014 已接受上表作为目标契约；#144 完成真实生产实现后因性能门槛失败而回退，所以当前生产仍使用 `f64`。目标已冻结不等于生产已经切换或已安排再次迁移。

### 4.1 #126 冻结的公开表面与转换边界

| 表面                                  | v0.6 公开/存储类型                                            | 转换与诊断规则                                                               | 已完成的研究/实施     |
| ------------------------------------- | ------------------------------------------------------------- | ---------------------------------------------------------------------------- | --------------------- |
| `EdgeLength`、`Speed`、`Acceleration` | 主要构造器和 `value()` 为 `f32`                               | 现有 `f64` 生产者使用受检 `TryFrom<f64>` 或命名转换；禁止未检查的 `as f32`   | #144 实施后 no-go     |
| `EdgeProgress`                        | 私有 `high: f32`、`residual: f32`；构造和有效值观察为 `f64`   | 所有差值、排序、边界、快照和事件判定都读有效值；分量不公开、不序列化         | #127 标定；#144 no-go |
| `IidmProfileSpec`、Parking 几何值     | 单值 `f32`                                                    | 不保留 `f64` 镜像；Data 原始值在受检转换前仍可用 `f64` 诊断                  | #144 实施后 no-go     |
| Parking 入口/出口锚点                 | Core 内保存 `EdgeProgress`，观察为有效 `f64`                  | 线格式仍是一个 `progress` JSON 数值；不新增静态位置标量类型                  | #144 实施后 no-go     |
| 路线（route）派生距离                 | #127 选择的 `f64` 前缀或分块局部 `f32` 布局                   | 对外只暴露真实采用的派生值；不把原始边长留作第二权威                         | #127 选择；#144 no-go |
| `CoreError` / `DataError` 数值载荷    | 原始转换值 `f64`；规范化单值 `f32`；有效进度/路线派生值 `f64` | 错误类型按发生阶段保存真实值；错误显示使用领域化中文范围，不引用已删除常量名 | #144 实施后 no-go     |
| Data v0.6 私有线格式                  | `f64` 或等价高保真暂存                                        | 只用于转换前校验和原始输入诊断；成功后只保留 Core 权威                       | #144 实施后 no-go     |

`formatVersion: "0.6"` 是 #144 已回退的历史数值候选，从未成为 current/published production format；当前唯一加载器和样例是 v0.10。未来重启数值迁移必须分配新的 format version，并在同一 Delivery PR 中切换 schema 文件名/`$id`、publication current source、版本闸口、私有 DTO、规范化、固定样例、测试和当前文档；不得保留 v0.10 运行时分支、双精度开关、弃用别名或迁移工具。
...
| Unity      | `Vector3(float x, float y, float z)`                     | 直接映射 canonical f32           |
| Unreal     | `FVector` 为 `TVector<double>`                           | Adapter 在末端无损升宽 f32       |
| Godot      | 默认 32-bit，支持 double-precision build                 | Adapter 按 build/ABI 做末端映射  |
...
### 6.2 Spatial frame 映射与失败语义

ADR 0015 接受的 runtime 顺序是：

```text
Core 的有效进度
  -> Spatial 有界 canonical f32 采样
  -> frame/有限性/点范围/基向量校验
  -> canonical f32 位姿批次
...
| 当前 Data v0.10     | JSON 结构、static topology、per-edge limit、加载器范围/诊断保持当前实现 | 当前不变；下一有效格式按 ADR 0008 原子替换      |
| 历史 Data v0.6 候选 | 硬范围、规范化、诊断与 Core 同步                                        | #144 no-go；未来重启必须使用新的 format version |
...
3. 同一代表性一万/十万 workload 的 f64/raw-f32/旧 compensated/residual-aware/mixed 时间与 candidate memory；
