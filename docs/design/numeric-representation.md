...
- 当前生产 Core/Data v0.9 继续使用现有 `f64` 权威和接受范围；#229 的 static-domain clean-break 与 #262 的横断面/准入迁移均未改变该数值契约。#144 的首次完整生产迁移仅获得 `4.257%` 稳态提升，未达到 `5%` 门槛，已按 ADR 0014 完整回退；
...
#122 的 `16_384 m` 边（edge）范围是历史研究输入，不是模式/API 的硬上限。#140 的边上界证据与 #141 G1 已把以下范围冻结为下一数值契约；当前 v0.9 在新的原子数值迁移前保持现有 `f64` 行为：
...
`formatVersion: "0.6"` 是 #144 已回退的历史数值候选，从未成为 current/published production format；当前唯一加载器和样例是 v0.9。未来重启数值迁移必须分配新的 format version，并在同一 Delivery PR 中切换 schema 文件名/`$id`、publication current source、版本闸口、私有 DTO、规范化、固定样例、测试和当前文档；不得保留 v0.9 运行时分支、双精度开关、弃用别名或迁移工具。
...
| 当前 Data v0.9      | JSON 结构、static topology、per-edge limit、加载器范围/诊断保持当前实现 | 当前不变；下一有效格式按 ADR 0008 原子替换      |
