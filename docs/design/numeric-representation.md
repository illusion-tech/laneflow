...
| 标识 v1（Identity v1）固定域                | `[u8; 16]` / `[u8; 4]` / `u16`                       | 静态契约（static contract）/ compiler  | `StableId128([u8; 16])`、`IDENTITY_MAGIC: [u8; 4] = *b"LFID"` 与固定 `STABLE_ID_DOMAIN_PREFIX`；`IDENTITY_ENCODING_VERSION: u16 = 1`、`IDENTITY_REGISTRY_REVISION: u16 = 1`，`EntityKind` / `FieldTag` 均为 `#[repr(u16)]`，进入规范 BLAKE3-128 标识前像                                                                                   |
