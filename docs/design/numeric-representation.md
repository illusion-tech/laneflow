...
**文档状态**: Accepted（已提交一维几何为整数毫米；编制 `f64` 与 Spatial canonical `f32` 仍在量化之前；#500 准入后 compiler IR 交通一维为整数毫米）
...
| 数值域                                      | 标量                                                 | 权威归属                                 | 规则                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------- | ---------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LFRE wire 几何                              | `f64`（`Vec3F64` + corridor station/宽度标量）       | 私有 wire / schema                       | FlatBuffers `double`（含 `RoadCorridor` `start/end_station_meters`、`LinearWidthProfile` `start/end_width_meters`），format v2 权威（ADR 0029，无 `static_routes`）；`format_version = 1` 失败关闭；`f32` 量化发生在 geometry compile 边界，不在 wire/lowering 层                                                             |
...
| compiler 来源描述符版本                     | `u32`                                                | compiler 来源描述符                      | SyntheticDsl `frontendVersion = 3`（准入后交通一维以整数毫米 / 受检 `f32` SI 写入 `LFSOURCE`）；RoadEditingSource `frontendVersion = 2`（ADR 0029）；`SOURCE_DOCUMENT_SET_DIGEST_VERSION = 1`。前端版本与文档集摘要版本是不同版本轴，均精确匹配，不并存旧读法                                                                 |
...
| LFRE format_version                         | `u32`（wire `uint`）                                 | 私有 wire / schema                       | `RoadEditingSource.format_version` 为 FlatBuffers `uint`；现行精确为 `2`（ADR 0029）；reader 以精确 `u32` 比对，`1` 失败关闭                                                                                                                                                                                                  |
