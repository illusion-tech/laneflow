...
- 受检的 `f64 → f32` 量化只发生在 geometry compile 的 `quantize_point`（`geometry.rs`）：对 evaluator 输出分量做 canonical 范围检查后再 `as f32`，并由 `canonical_f32` 把每个 signed zero 规范化为 bit-exact `+0.0f32`；不允许在 wire/lowering 以隐式 `as`/截断提前量化，也不允许在高层中间表示（HIR）内部 `SpatialHir` 子表示、中层中间表示（MIR）或 LIR 中二次量化（与 ADR 0014/0015/0022 一致）。
