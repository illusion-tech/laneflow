...
| Parking / VehicleProfile 硬范围             | `f64`                                                | compiler / static contract       | Parking anchor 距 edge 两端严格 `> 1.0e-9 m`，lateral offset 绝对值和长/宽严格 `> 1.0e-9 m`，heading offset 为 `[-π, +π)`；vehicle length 严格 `> 1.0e-9 m`                                        |
...
| compiler 几何配置档代码                     | `u8`                                                 | compiler frontend options        | Accuracy `Fine/Balanced/Compact = 1/2/3`，Direction `Smooth/Balanced/Compact = 1/2/3`；摘要顺序固定为 semantics version、accuracy code、direction code                                             |
