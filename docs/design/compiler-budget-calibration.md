...
工作负载、当前固定样例研究投影、基础规模发现和两批五轮正式规模阶梯已经实现。本文
已另有朴素 Rust 子命令从基础规模原始样本重算冷实例临时性能预算；完整 Evidence v1、
候选矩阵和详尽失败恢复不阻塞首轮预算执行。本文不实现生产编译器、不冻结公共应用程序
接口（API），也不形成产品服务等级协议（product SLA）
