...
是否复用子进程不改变该规则。非清理样本仍显式记录 `phase = not-applicable`，但其
...
- `passed` 当且仅当六条引用运行全部有效且全部闭合检查为真。异常终止或语义负面结果
  ...
  补足对应角色后才能形成资格汇总；
- `noise-no-difference`、`repeatable-improvement`、`repeatable-regression` 和
  `rejected-correctness` 必须各自绑定同候选的唯一资格；`rejected-safety` 或
  `insufficient-evidence` 可以在未进入正确性/性能求值时省略资格汇总。已经执行的
  负面资格仍必须保留，不能用省略规则删除；未执行时，候选比较的资格引用分别使用
...
- 三个内部快速哈希候选的 `noise-no-difference`、`repeatable-improvement` 或
  `repeatable-regression` 必须引用同候选且 `passed = true` 的恒定哈希资格；以
  `rejected-correctness` 结束时必须引用可重算的失败资格。安全预先淘汰或证据不足
  可以不运行该资格，但不得形成性能结论；其他候选必须显式记录不适用原因；
...
不得充当六条资格运行之一。验证器随后逐条核对候选比较：性能结论要求通过资格，
`rejected-correctness` 要求可重算的失败资格；安全预淘汰或证据不足可以没有资格，
但若已经执行仍必须完整验证并保留；未执行时，资格引用的空值原因必须等于清单按决策
登记的唯一原因。未执行恒定哈希构建器却制造汇总、重复候选、跨候选复用资格、只引用
性能运行、缺少必需资格、自由省略原因或失败资格仍形成性能结论都必须失败。
