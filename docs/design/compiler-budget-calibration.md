...
- `core-runtime-performance-baseline.md` 已登记 R0 固定研究环境、P100 推荐硬件角色及
  研究证据声明纪律；两者复用同一台物理机器，但不复用认证结论。
...
- 从 R0 结果推导最低产品硬件、等价/替代推荐机型、玩家编辑体验或中国特色城市
  工作负载 SLA；P100 是产品负责人独立作出的硬件选择，不是本研究从性能结果外推的
  结论；
...
| 合成规模曲线 | P100 同机 R0 环境中特定内核、候选和记录组合的成本/内存增长 | 对应真实城市容量或交通参与单元规模                |
| 容器候选比较 | 相同输入与输出下的平衡顺序相对成本                         | 未测键域、平台或攻击面的全局最优                  |
| 研究预算建议 | #292 G1 可审阅的 P100 同机 R0 研究预算输入                 | 产品通过（Product Pass）、玩家体验 SLA 或公共契约 |
| 硬件对照锚点 | 后继生产实现可在同一 P100 配置上与 R0 研究成本作分层对照   | R0 已认证生产编译器、Traffic Runtime 或完整游戏   |
...
- 预言机同样受研究停止护栏约束；若某级别无法完成预言机验证，该级别不能形成有效
  性能证据；
- 预言机时延和内存只作诊断，不进入候选性能比较或 #292 预算。
...
2026-08-10 后该标识符只保留为 #292 历史证据名称；当前投影测试已改用编译器原生
夹具，不再维护 current JSON 端到端等价 workload。
...
整份权威证据还必须登记正式研究处置（formal study disposition）：

- `no-reliable-base-scale`：所有适用 `baseScales[]` 都在找到 `B` 前由真实终止护栏
  结束；正式轮次/阶梯汇总、相邻比值、拐点、重复性包络、候选比较和预算
  建议必须全部为空，但试运行、护栏和已执行的独立正确性资格验证仍须保留；被安全
  预先淘汰且未执行资格验证的候选按第 9.2 节保持缺席，不得伪造运行；
- `insufficient-formal-ladder`：至少一个自然身份已找到 `B`，但护栏前没有完成冻结
  校准/压力规模所需的正式阶梯；允许保留已完成级别及诊断性相邻比值，但不得生成
  重复性包络、候选性能比较或预算建议；
- `formal-analysis-available`：至少一个自然身份已找到 `B`，并已有可解析的轮次指标
  汇总和正式阶梯批次汇总；后续派生项仍各自受本节的证据充分性约束，不能仅凭状态名
  自报完成。

独立验证器必须从完整 `baseScales[]`、正式级别覆盖和停止护栏重算该处置；不得用一个
不相关工作负载的汇总伪造“有正式结果”，也不得在全部 `B` 缺失时填充任意汇总来满足
Schema。

### 5.3 正式规模阶梯

每个 `(workloadId, graphProfile)` 的基线候选在本次 R0 固定运行五级正式阶梯。
正式实验包含五个新进程轮次
...
父协调进程必须从子进程启动时并发排空标准输出和标准错误，不得等子进程退出后才读取；
否则包含三十五条清理观察等较大结构化结果会填满 Windows 匿名管道并造成父子互等。
排空发生在父进程侧，不进入子进程内部计时，但其完成时间仍属于完整子进程墙钟观察。

每个新级别进入正式轮次前，先运行一次插桩护栏预检、一次不进入预算的非插桩时延
护栏观察和一次计时区外预言机。后一级启动前墙钟预测只能使用前一级非插桩时延护栏
观察中**完整计时子进程**的父进程监控墙钟，不得使用单个冷实例/稳定容量复用样本、
正式轮次样本或插桩进程的诊断时长；预测值和硬上限约束的对象必须同为完整子进程。
该观察一经用于后一级预检便不得被后续正式轮次覆盖，以便独立验证器重算同一前像。
正式轮次中，偶数轮先运行插桩内存进程，奇数轮先运行非插桩时延进程。两种进程使用相同
...
五轮平衡轮转。若五级内确认拐点，按第 5.5 节选择其前一级和拐点级；若没有确认拐点，
按既有 `first-confirmed-knee-or-max-complete-v1` 规则选择 `8B` 与 `16B`，并明确报告
“测量窗口内未观察到拐点”。执行器不得仅为寻找更高拐点或资源上限而启动 `32B` 及
以上级别，也不得清空五级有效结果后扩大集合重测；这类极限探索不影响 #292 所需预算，
只能由后继研究另行授权。

稳定容量复用只复用暂存容器容量，不得复用符号解析、身份结果、规范排序结果或其他
语义结果；否则该样本属于增量编译，不得混入本研究。

冷实例编译、三次不计时预热和随后七次稳定容量复用必须使用同一个仍存活的编译器
实例：实例在冷实例编译前构造，在第七次复用完成后才允许销毁。每条冷实例/复用原始
运行都保存同一运行级 `compilerInstanceId`；样本间销毁并重建实例、即使仍在同一
`childPid` 或同一轮次尝试中，也属于协议违例。新构造实例必须获得新身份，不得复用
已经销毁实例的身份。
...
每条原始运行必须携带轮次尝试身份（round-attempt identity）：稳定尝试 ID、从零开始
的重试序号和闭合范围 `single-experiment`、`formal-ladder-round` 或
`candidate-comparison-round`。同一 `(purpose, batch, round, 完整轮次分层)` 的重试
必须获得新的尝试 ID 并递增序号；`runId` 只标识单一样本，不能替代轮次尝试身份。
正式阶梯同一模式整轮、或平衡候选同一整轮中的全部样本必须共享尝试身份。

若一个整轮尝试中的任一样本触发预声明作废事件，执行器必须把该尝试内已经取得的全部
样本统一改为 `status = invalid`，并传播同一作废原因集合；尚未启动的位置可以缺失，
但不能从另一个尝试补齐。重跑使用新尝试身份。独立验证器按尝试分组核对批次、轮次、
位置集合、候选/级别覆盖、状态与原因传播；任何仍标为有效的早期样本都使整个尝试无效。
...
序号集合完整、无重号；汇总还必须显式绑定这些运行共同的 `roundAttemptId`。轮次
...
若固定五级始终没有确认拐点，压力规模取最大完整级别 `16B`，校准规模取其
前一完整级别 `8B`；两者都必须标记“测量窗口内未观察到拐点”，不得伪造窗口外的
增长形态或资源边界。任一固定级别在启动前或运行中触发研究停止护栏时，必须把预检或
受监控子进程事实保存为无效运行；五级不完整时不得冻结规模，也不得继续下游资格。
父进程为执行墙钟、私有字节或可用物理内存护栏而终止子进程时产生的非零退出状态，
只是该受检终止的伴随事实；只有脱离具名资源护栏单独出现时，才按子进程异常退出作废。
...
基线配置的全部正式级别必须由同一次协议内、使用全新子进程的 batch 1 完整复测，不能
只复测 batch 0 发现的疑似拐点；这里的“另一独立执行批次”不要求第二次启动完整协议。
每批仍包含第 5.3 节的五轮。下式中的指标 `m` 只包含严格大于零的时延、
...
R0 不发布对数增长斜率。正式阶梯已经保存并独立复算每个相邻级别的精确整数比值；
这些比值直接支撑拐点、规模选择和预算建议，增长斜率不参与任何 R0 决策。若后继研究
需要跨多级的复杂度拟合，必须在独立 Issue 中定义版本化算法和证据用途，不得给本次
Evidence v1 增加昂贵且未被消费的派生字段。

上述公式只生成供 #292 审阅的 R0 研究预算建议。#292 G1 已按机器可读自然身份接受
首轮建议；生产实现若需收紧或因必要语义重新测量，必须记录证据和理由，不能把停止
护栏或未解释整数替代该来源。
...
2. 分别预测编译器控制峰值存续字节、进程私有字节和完整管线墙钟。有上一完整级别时，
   三者都按主记录数比例线性外推并乘以 `5 / 4` 安全因子，精确公式分别为
   `ceil(previousPeakLiveRequestedBytes * nextPrimaryRecordCount * 5 /
   (previousPrimaryRecordCount * 4))`、
   `ceil(previousPrivateBytes * nextPrimaryRecordCount * 5 /
   (previousPrimaryRecordCount * 4))` 与
   `ceil(previousWallTimeNs * nextPrimaryRecordCount * 5 /
   (previousPrimaryRecordCount * 4))`；编译器控制字节最终预测还要取其历史外推项与
   清单单缓冲区下界的最大值，后两项不引入额外下界或自由固定开销；
...
压力规模触发护栏只说明 R0 不应继续扩大。#292 G1 冻结生产资源上限时已经结合：

- 已确认的线性区间和成本拐点；
- 实际生成记录与字节；
- 失败关闭能够在分配前执行的限制维度；
- 生产需求、P100 正式测量、P10 与替代硬件的后续证据。
...
首轮正式研究复用 `core-runtime-performance-baseline.md` 当时的 R0 **研究环境**；
...
其中硬件字段标识 P100，操作系统、工具链、电源计划和固件仍是该次 R0 的历史运行
环境。图形硬件只用于完整机型识别，不进入编译器预算。P100 的选择使 #292 可以在同一
推荐配置上比较研究内核与生产实现，但不复用运行时规模、tick/frame 预算或产品认证，
也不把本报告追溯改写为 Product Pass。每次正式执行仍须重新记录 CPU、物理内存、OS build、AC/电池、
...
摘要计算。摘要成本若需要归因，只能作为独立指标报告，不进入候选性能比较、`B`、拐点或
#292 预算建议。
...
失败输入摘要（failure input digest）使用清单冻结的
`LANEFLOW-COMPILER-CALIBRATION-INPUT-V1\0` 域和固定二进制字段顺序，绑定工作负载清单
SHA-256、完整工作负载/配置档/`N`/`B`/规模角色、二十三项规范聚合计数、输入变体、
值来源、基准运行 ID 与全部私有限制。规范来源值及其载荷只由已认证清单、完整规模身份、
规范计数和变体规则定义；封套绑定这套唯一来源定义，不为证据摘要再次物化城市级输入
字节。独立验证器必须从受信任清单和原始运行字段重建同一封套，不能调用写出器的摘要
函数或回抄 `inputDigest`。
...
本节只验证完整研究管线基线
`baseline-std-randomstate-stable-vec-v1` 的失败后生命周期恢复，不承担私有候选准入。
基线按机器清单 `cleanupExperimentBindings[]` 顺序、再按每项 `scaleRoles[]` 顺序，
在校准规模和压力规模执行全部六组实验。候选性能分类仍由第 9.2 节的原始性能证据
独立产生；清理观察不得把性能分类改写成候选淘汰或实现建议。若 #292 后续拟采用某个
私有候选，应在生产实现 Gate 中按其真实生命周期另行定义资格测试，不能把本节的基线
研究观察外推为候选已经合格。
...
每组必须在同一进程内保持第 1–4 项的实例生命周期，因此它不是一条单次完整管线墙钟
样本。父进程仍持续监控私有字节和系统可用物理内存，聚合进程墙钟上限按执行器冻结的
`cleanup-group-size-times-single-pipeline-wall-time-v1` 规则派生：只把单次完整管线的
`60 s` 上限乘以冻结 `groupSize = 35`，其他护栏阈值保持不变。该派生只允许
完成同进程失败恢复资格，不产生性能样本，也不得进入正式轮次、预算或候选比较。执行
检查点必须保存实际聚合监控阈值，验证器按单次阈值和组大小重算；直接复用 `60 s` 或
自由填写更大值都使清理资格无效。

失败预热后的三十二次重复中，每次结束时
`metrics.liveRequestedBytes.value` 必须为零；第 `1` 次失败建立允许的保留容量上界，
第 `2..=32` 次的 `metrics.retainedCapacityBytes.value` 都必须小于或等于该上界。
这允许容量复用或回收，但禁止失败次数推动保留容量继续上升。任一观察缺失、出现台阶
增长或存续字节非零时，该组失败；必须保存全部已执行原始容量，不能用最终进程退出
掩盖泄漏。

同一清理实验使用稳定实验标识符关联基线成功、三十二次失败、恢复成功和新实例
判定基准；序号固定为基线成功 `0`、失败轮次 `1..=32`、恢复成功 `33` 和新实例
判定基准 `34`。每条运行都在一级字段记录编译器实例身份
（compiler instance identity）；清理对象只记录实验标识符、阶段和序号，不复制第二份
身份。该身份由研究执行器在实例构造时一次性签发并随实例存续，实例销毁后不得复用；
不能从 `childPid`、内存地址、轮次尝试或 `runId` 推断。同一 PID 内销毁并重建实例也
必须获得新身份。序号 `0..=33` 必须共享同一实例身份，序号 `34` 必须使用不同身份；
是否复用子进程不改变该规则。序号 `33` 与 `34` 还必须在结果、逐阶段记录数、研究
语义摘要、诊断摘要、稳定错误状态和部分输出记录数上完全相同。独立验证器只有在
序列、实例、失败迭代和恢复四组条件全部成立时才判该清理组通过；不另创建可自报通过
的清理汇总对象。
...
依赖未声明 MSRV 时不得虚构版本；证据必须原样记录“未声明”，并以冻结 Rust 1.96
工具链的 `--all-features` 实际构建通过作为本研究的兼容性边界。该证据只允许该候选
参加本次研究，不替依赖上游声明长期 MSRV。

未注册候选、候选 ID 与键域错配、快速非加密哈希进入 `external-string`、种子策略
错配或依赖组件集合不一致必须在候选性能比较前失败，不能通过自由字符串 ID 获得性能结论。
...
该要求由机器清单 `constantHashQualificationContract.revision = 1` 和权威 JSON 的
恒定哈希资格验证（constant-hash qualification）闭合，而不是测试报告中的自由文本：

- 只对 XXH3、XXH64 和 FNV-1a 64 三个内部快速哈希候选执行；工作负载固定为
  `LF-COMP-CORRIDOR-v1`、`N = 1`、`wide-star-v1`、`short-unique-v1` 和
  `scaleRole = known-vector`，与基准规模无关；
- 候选侧构建器固定为 `all-keys-u64-zero-v1`，精确预言机侧固定为
  `exact-research-oracle-v1`；合法输入和单一未知引用输入各执行两次候选运行及一次
  独立预言机运行，资格构建器始终排除在候选性能比较外；
- 每个实际执行资格验证的受检候选必须恰有一条资格汇总，整份证据最多三条；资格 ID
  在整份证据中唯一，不得用同一候选的重复记录填满集合，也不得跨候选复用资格；
- 原始正确性运行保存资格 ID、角色、构建器、输入变体、重复序号、预期/实际结果、
  稳定错误码、诊断/截断和部分输出计数；资格汇总必须引用六条精确运行，重算逐阶段
  计数、语义/诊断摘要、重复确定性、稳定结果和部分输出一致性；
- 正确性子进程正常返回时，运行可因外部作废原因成为 `invalid`；发生 panic、abort、
  被监控终止或其他异常终止时，`actualOutcome = abnormal-termination`、运行状态
  `invalid`、进程终态和 `child-abnormal-exit` 必须相互闭合。异常运行仍引用原资格，
  不得为通过 Schema 而丢弃；
- `passed` 当且仅当六条引用运行全部有效且全部闭合检查为真。异常终止或语义负面结果
  必须保留为失败资格；纯外部状态作废且正常返回的运行不构成候选失败，必须以新运行
  补足对应角色后才能形成资格汇总；
- 候选参赛名单中的 `performance-participant` 与 `rejected-correctness` 必须各自
  绑定同候选的唯一资格；`rejected-safety` 或
  `insufficient-qualification-evidence` 可以在未进入正确性求值时省略资格汇总。
  已经执行的负面资格仍必须保留，不能用省略规则删除；未执行时，名单项的资格引用
  分别使用
  `null + qualification-not-run-safety-pre-rejection` 或
  `null + qualification-not-run-insufficient-evidence`，不得制造汇总或六条虚假运行。
...
机器清单 `candidatePerformanceScopeContract.revision = 1` 还冻结本轮完整管线候选
比较的唯一代表范围：`LF-COMP-JUNCTION-GRID-v1`、`wide-star-v1`、
`short-unique-v1`、生成器版本 1、合法规范输入和用例 `not-applicable`。它分别在同一
自然身份正式阶梯派生的基础规模、校准规模和压力规模运行，样本种类固定为冷实例，
二进制模式固定为 `timing`。选择该范围是因为它同时覆盖密集路口中的冲突关系、门、
停止线、等待区和路线等结构，同时把一次性研究的正式执行量约束在可复核范围内；它
不是生产工作负载代表性声明，也不替 #292 选择容器或哈希实现。

候选性能分类只使用完整管线墙钟和父进程监控得到的工作集、私有字节、提交峰值四项
指标。标准库和第三方候选容器不经过研究执行器的逐分配精确记账，因此
`allocated-bytes`、`peak-live-requested-bytes`、`retained-capacity-bytes`、分配次数
和重分配次数不得用于该候选范围的分类；不能把为硬上限保守预占的受控请求字节冒充
候选实际分配量。

每个性能范围在启动轮次前必须形成一条候选参赛名单（candidate roster）。名单自然身份
由键域、工作负载/修订、模块图/字符串配置档、生成器、`N`、`B`、规模角色和用例组成，
不包含后续性能指标、样本种类或二进制模式。名单按候选注册表顺序覆盖该键域允许的全部
候选，每项恰好一次，并使用下列封闭处置：

- `baseline-participant`：唯一键域基线已通过安全与正确性资格；
- `performance-participant`：非基线候选已通过安全与正确性资格，可进入平衡性能轮次；
- `rejected-safety`：依赖、许可证、MSRV 或安全公告检查失败；
- `rejected-correctness`：通用正确性或适用的恒定哈希资格可重算地失败；
- `insufficient-qualification-evidence`：资格证据缺失、作废或不能完成，但没有形成
  可重算的安全/正确性失败。

名单项保存通用正确性运行对和适用的恒定哈希资格引用。`C` 唯一定义为
`baseline-participant` 与 `performance-participant` 项的数量；被拒绝或证据不足项
不占执行位置。若没有恰好一个基线参与者或 `C < 2`，该名单不得产生候选性能比较。
因此预先淘汰不会留下“`C` 是全部注册项还是实际参赛项”的解释空间。

- 标准随机化 `HashMap` 是前两个键域的功能与安全基线，不是假定性能最快候选；稳定 `Vec`
  排序是规范输出顺序键域的基线；
...
- 候选比较每次只替换一个键域或排序组件，其他组件保持基线配置；组合候选只有在各
...
- 从名单中保持原顺序提取 `C` 个参与者后，每批固定执行 `2C` 个平衡候选顺序轮次；
  前 `C` 轮的第 `r` 个顺序为 `participant[(j + r) mod C]`，后 `C` 轮为
  `participant[(r - j) mod C]`，其中 `r,j = 0..C-1` 且下标按 `C` 取模。这样每个
  参与者在每个执行位置恰好出现两次，并同时覆盖正向/反向相邻关系；
- 每个候选样本使用一个全新子进程，每轮由全新父协调进程按冻结顺序依次启动；第二
  独立批次完整重复全部 `2C` 轮，不得只复测首批表现最佳者；
...
- 每条候选比较必须保存候选比较分层（candidate comparison stratum）：键域、工作
  ...
  样例用例、样本种类和二进制模式；不得跨任一字段聚合。`rosterId` 必须唯一解析到
  投影后身份相同的候选参赛名单；`candidateId` 必须是其中的
  `performance-participant`，`baselineId` 必须是唯一 `baseline-participant`；
...
- `batch0` 与 `batch1` 对象始终存在，但其证据内容由清单
  ...
  完整覆盖参赛名单派生的 `2C` 轮且 `medianRatio` 非空；`insufficient-evidence`
  可以保存零项、未完成的有效前缀或完整但两批分类不一致的配对，未完成批次的中位
  比值必须为空。原始作废轮次仍保存在 `runs[]`，不能为填满 batch 而制造
  `roundSummaryId`。安全/正确性拒绝只存在于名单项，不再复制成每个指标的一条空
  候选比较；
- 每个轮次对的比值方向固定为 `candidate round median / baseline round median`。
  ...
  没有重复性包络，只作机制诊断，不得生成 `candidateComparisons[]` 或形成性能结论。比值
...
- 候选结论算法固定为 `two-batch-envelope-classification-v1`。令同指标唯一重复性
  ...
  `R_i = A_i / B_i`，其中 `i ∈ {0, 1}`、`A_i > 0`、`B_i > 0` 且已约分。安全与
  正确性已由参赛名单先行处理；任一批缺失、不可比较或轮次不完整时结论为
...
- 独立验证器必须把每个轮次对的两个 `roundSummaryId` 唯一解析回轮次汇总及其原始
  ...
  再独立重算
  `medianRatio`。任何包含空比值、缺轮、重轮或非一一配对的批次只能形成
  `insufficient-evidence`，不能形成可重复性能分类；
- 基线候选运行完整规模阶梯；其他候选至少运行 `B`、校准规模和压力规模；
- 单项内核（kernel）可作机制归因，但候选性能分类以完整研究管线结果为主；
- 候选的分层改善若未超过第 5.6 节同指标重复性包络，只能判为“噪声内无差异”；
  超过包络的候选必须在第二独立批次重现后才能分类为
  `repeatable-improvement`。该值只表达性能分类，不等于选择建议；
- v1 的完整管线候选性能范围只把 `wall-time-ns` 纳入分类。父进程仍把
  `peakPrivateBytes` 保存在每条候选原始运行中，供诊断和后续研究使用；但当前正式
  基线路径不能在与候选运行相同的冷实例分层上形成私有字节重复性包络，因此不得把它
  填入 `candidateComparisons[]`。工作集和提交峰值未由现行父进程协议观测，必须保持
  `null + reason`，不得依据 Schema 中允许的字段名补造数值；
- 三个内部快速哈希候选只有在参赛名单绑定同候选且 `passed = true` 的恒定哈希资格
  后才能使用 `performance-participant`；失败资格使用 `rejected-correctness`。
  安全预先淘汰或资格证据不足可以不运行该专项，但不得进入性能轮次；其他候选必须
  显式记录不适用原因；
- 不按统计离群值删除单个样本。只有出现预先声明的外部状态事件时才允许整轮作废：
  AC/电池、电源计划或厂商性能模式改变；系统睡眠/会话锁定；操作系统报告热/功耗
  throttling；监控缺样；子进程异常退出。作废事件、原始监控值和全部样本仍进入证据，
  并重跑该批的完整 `2C` 轮；环境无法提供某项监控时不得执行正式候选性能比较；
- 每轮后台 CPU 时间总量和后台写入字节总量都是结构化可空观察值。无监控缺样时两项
  必须保存实测非负整数且 `reason = null`；发生监控缺样时，至少一项必须保存
  `value = null` 与稳定原因码 `monitoring-gap`，该运行必须标为 `invalid` 且
  `invalidationReasons` 包含 `monitoring-gap`。不得用零、旧快照或空数组冒充
  未采集总量；没有监控缺样的运行也不得登记 `monitoring-gap` 原因。后台 CPU/写入总量
  是解释重复性和跨批差异的诊断事实，不以未经实测校准的固定绝对阈值单独作废轮次；
  两次完整进程快照之间创建或退出的后台进程属于操作系统正常周转，不构成必需监控缺样；
  这两项总量只合计终点快照仍可观察或区间中新出现的进程，是诊断下界而非系统级精确
  计账。只有任一完整进程快照、电源状态、电源计划或时钟观察本身不可用时，才登记
  `monitoring-gap` 并作废轮次；
  若其干扰使两批结论不一致，必须按第 5.5 节判为证据不足并在稳定环境中重跑，不能删除
  单个样本或放宽重复性要求；
- 依赖版本、特性、许可证、MSRV、安全公告和锁文件摘要进入证据；
- 研究结果只向 #292 提供原始证据与可重复性能分类，不形成私有实现选择建议，也不
  冻结公共哈希器或容器类型；是否进入生产候选以及所需生命周期资格由 #292 后续
  实现 Gate 裁决。
...
`insufficient-qualification-evidence`，但 Schema 不能为了只接受参赛项而拒绝保存
负面事实。只有
第三方候选才禁止把安全公告状态写成 `not-applicable`。

完整候选绑定与依赖审计保存在证据级候选绑定目录 `candidateBindings[]`，不能只依附
于 `runs[].candidate`。目录必须包含 `runs[]` 中出现的每个候选，以及每条候选参赛
名单和候选比较引用的候选；因此执行前即被安全或证据检查拒绝、没有任何正确性或
性能运行的候选仍有
...
- 第三方组件只有许可证、MSRV、锁文件 package/checksum 均可验证且安全状态为
  `no-known-advisories` 时，才有资格成为 `performance-participant`。
  `advisories-present` 必须 `rejected-safety`；`audit-unavailable` 只能
  `rejected-safety` 或 `insufficient-qualification-evidence`。

独立验证器必须从绑定的 harness `Cargo.lock` 和审计制品重建上述来源，不得只信任
状态字符串。审计工具、数据库快照或观察时间全为空却自报成功，或者负面审计仍进入
参赛子序列，都必须失败。
...
`f2e0170a80517460bd0a76b4a699e35cdd09c5a1fd1c7ae243296e741d00e809`。该摘要是 PR/Gate
...
>
> 当前 Schema 为 `5485` exact bytes，SHA-256 为
> `0d5b724b018e3e834614cd49a276f2c061f60cf4d900bce27c68ecaf8a3ce267`。Schema 负责紧凑
...
- 每个候选资格自然身份的候选参赛名单、唯一名单 ID/基线、按注册表顺序覆盖的全部
  候选项、封闭资格处置、通用正确性运行对与适用的恒定哈希资格引用；参与者子序列
  唯一派生 `C` 和两批 `2C` 平衡轮次；
...
- 每条派生候选比较的完整候选比较分层、按两个独立批次拆分的同轮
  `roundSummaryId` 配对、精确轮次比值、中位比值、纯性能决策和候选参赛名单引用；
  汇总与运行引用必须能由
  独立验证器按清单中的精确分类契约回算，不得只保存最终数；分配次数与重新分配次数
  只保留在原始指标中，不进入候选比较；安全/正确性拒绝只在名单项保存一次，不按
  性能指标重复；
...
- 每个失败契约样本的用例/维度、研究输入变体标识符、输入摘要、预期/实际结果、
  ...
  阶段和 `0..=34` 序号；六组基线生命周期观察直接由原始运行重算，不复制第二份
  清理汇总；
- 正式研究处置，以及恒定哈希资格的闭合运行角色、构建器/变体、重复序号、预期/
  实际结果、六条运行引用和逐项资格结论；
- 每轮电源/厂商模式、会话、热/功耗节流、后台 CPU/写入增量、两项总量的结构化
  可空观察与监控缺样原始值，以及作废原因；
...
`runs[].candidate` 必须唯一解析到同 ID 目录项并逐字段相等；每个候选参赛名单项也
必须解析到唯一目录项，即使候选在安全预淘汰后没有运行。不能从自由 ID 推测算法、用
运行承载唯一审计前像，或把同名候选的不同依赖实现合并比较。验证器按名单键域过滤
候选注册表并保持原顺序，要求每个允许候选恰好出现一次，再按
`baselineByKeyDomain` 重算唯一 `baselineId`、资格处置及参与者子序列。遗漏、重复、
重排、错误基线、错误处置或把被拒绝项计入 `C` 都必须失败。第三方组件的已完成安全
审计还必须有非空工具、数据库快照和观察时间；任一来源不可用时状态必须降为
`audit-unavailable`。验证器按许可证/MSRV/锁文件/公告结果约束名单资格处置，负面或
不可用审计不能形成参赛项。

对三个内部快速哈希候选，验证器还必须从
`constantHashQualificationContract` 重建固定工作负载、两个输入变体和候选/预言机
构建器，要求每个已执行候选至多一条资格且资格 ID 全局唯一，再按资格 ID 解析两组各
两次候选运行与一次预言机运行。它必须独立比较逐阶段计数、语义/诊断摘要、重复
确定性、稳定结果和部分输出，重算 `passed`；异常终止运行必须保持
`invalid + child-abnormal-exit` 并留在失败资格中，正常返回但仅因外部状态作废的运行
不得充当六条资格运行之一。验证器随后逐条核对候选参赛名单项：
`performance-participant` 要求通过资格，`rejected-correctness` 要求可重算的失败
资格；安全预淘汰或资格证据不足可以没有资格，但若已经执行仍必须完整验证并保留；
未执行时，资格引用的空值原因必须等于清单按名单处置登记的唯一原因。未执行恒定哈希
构建器却制造汇总、重复候选、跨候选复用资格、只引用性能运行、缺少必需资格、自由
省略原因或失败资格仍进入参赛子序列都必须失败。
...
验证器随后从全部基准规模和完整正式级别覆盖重算 `formalStudyDisposition`。全部
自然身份都未取得 `B` 时，只允许 `no-reliable-base-scale` 和空正式派生数组；至少有
一个 `B` 但正式阶梯不足时不能形成候选性能比较或预算；只有确有可解析轮次与阶梯汇总时
...
`roundAttemptId` 与原始运行一致，不能用后续尝试补齐前次缺口。`cold-instance` 要求唯一
`sampleOrdinal = 0`；`stable-capacity-reuse` 要求同一子进程的
`sampleOrdinal = 0..6` 完整集合。验证器还必须按候选、工作负载、批次、轮次、位置和
进程模式配对一次冷实例与七次复用运行，要求八条运行的 `compilerInstanceId` 完全相同
且在整份证据中没有代表另一构造事件；只共享 `childPid`、尝试身份或样本序号不足以
证明容量复用。正式阶梯批次汇总必须引用同一候选、分层、指标与批次的
`round = 0..4` 五个轮次汇总，并重算跨轮中位数和中位绝对偏差。相邻级别轮次对必须
解析到除 `N`/规模角色外相同完整测量分层的上下级正式阶梯汇总，并重算
...
第 9.2 节的交叉乘法重算纯性能 `decision`。边界被写成改善/回退、两批
混合结果被写成可重复结论、指标错配或使用浮点容差都必须失败。它还必须按
`rosterId` 唯一解析候选参赛名单，从参与者子序列重算 `C`，再按
`batchEvidenceRules` 验证两批：比值结论各有完整 `2C` 轮和非空中位比值；证据不足
允许空、未完成或完整但不可分类的批次，空批和未完成批次分别使用清单登记的唯一
空值原因。比较候选不是 `performance-participant`、基线不是唯一
`baseline-participant`、分层不能投影到名单、无有效轮次却制造引用、未完成批次自报
中位数或性能结论缺完整配对都必须失败。通用正确性运行对与恒定哈希资格只在名单项
验证一次，不得在每个指标比较中复制出第二套资格事实。
...
重算 `predictedCompilerControlledBytes`、`predictedPrivateBytes` 和
`predictedWallTimeNs`。三项都使用同一主记录数比例、`5 / 4` 因子、受检 `u128`
乘法、商加非零余数的上取整和最终 `u64::try_from`；只有编译器控制预测再与清单
下界取最大值。证据中出现自由固定开销、下界操作数缺失、跨工作负载主记录公式、
任一预测未重算或不同整数舍入结果时，整条运行无效。

独立验证器还必须按清理实验标识符分组，并只接受
`baseline-std-randomstate-stable-vec-v1` 的六组基线生命周期观察。每组恰好包含序号
`0`、`1..=32`、`33`、`34`，且工作负载、候选、规模和失败用例在组内一致；序号
`0..=33` 的
运行级 `compilerInstanceId` 必须完全相同，序号 `34` 必须不同。验证器不得用相同
...
验证器按 `baselineCleanupObservationContract` 重算上述六组基线观察，但不得把清理
结果写入候选性能分类或据此形成私有实现建议。受检算术失败、监控缺样、异常退出或
不可用预测可以留在原始证据中，但不能被计入有效阶梯、重复性包络、性能分类或通过的
基线清理组。Schema 对单条记录的 closed 约束与独立验证器对整份证据的关系约束缺一
不可。
...
#292 G2 随后确认，研究清单中的抽象实体组合与 `crossModuleReferences` 不能按原自然
身份无损构造为合法生产 `Synthetic DSL`；生产 HIR/MIR/LIR 的实际阶段与内存观测也不
等于研究替身记录。该发现不追溯改写 #308 的 G4 证据，但使原 `LF-COMP-P100-R0-v1`
...
- 最低产品硬件，以及 P100 等价/替代机型；P100 当前物理配置的推荐角色已由产品负责
  人选定，不属于 #308 的性能推导结论；
...
- [x] 本文 exact head 通过全面本地文档/架构审阅；
- [x] 外部审阅未发现未回应的主要/阻断发现（Major/Blocking finding）；
- [x] 研究工作负载清单 exact bytes 摘要已冻结，三个合成工作负载 × 三种模块图、
  字符串/来源位置类别、记录布局、计数对象、证据角色和禁止外推边界闭合；
- [x] 候选注册表的闭合 ID、允许键域、种子策略、算法常量与依赖组件身份闭合；
- [x] `B`、至少五级规模阶梯、拐点确认、校准/压力规模纯函数选择和停止护栏规则
  闭合；
- [x] 冷实例、稳定容量复用、轮次/批次精确汇总、失败清理和内存记账协议闭合；
- [x] 外部可控键与内部定长键的安全域、候选矩阵、进程隔离、平衡顺序和整轮作废
  纪律、候选参赛名单、同指标重复性包络与纯性能分类闭合；
- [x] 非自指契约描述符、证据格式 v1 schema、研究/生产 workload ID 所有权与 #292
  rebase 回写边界闭合；
- [x] 术语 SSOT、设计索引和相关 Agent Skill 阅读入口同步；
- [x] Issue #308 留下可永久引用的 G1 判断评论。
...
5. 完成全部限制维度资格、失败恢复和两个独立归因副本；
6. 完成冻结候选注册表的安全/正确性资格与平衡顺序性能矩阵；
7. 生成并独立验证 Evidence v1，确认失败/无效运行与全部派生关系均闭合；
8. 在冻结研究机执行一次完整 R0 协议，由其中的 batch 0/1 取得两批正式测量，发布原始
   JSON、Markdown 报告和精确复现命令；
9. 完成本地与外部 clean re-review 后，#308 按 G3/G4 收口；
10. #292 只引用最终 exact evidence commit 冻结生产实现预算。

在第 2 步前不得实现研究代码。既有临时预算不能改写成正式 R0 研究预算、生产
`CompileLimits`、产品服务等级协议或真实城市容量结论；只有第 8 步单次完整协议内的
两批正式证据可以支撑最终 R0 报告。
