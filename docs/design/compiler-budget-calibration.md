...
五级初始阶梯只执行一次两批五轮平衡轮转：若已经确认拐点，直接结束该自然身份；若仍
未确认拐点，后续倍增级别先只执行插桩预检、非插桩时延护栏观察和预言机，直到研究停止
护栏确定最终安全的完整 `K` 级集合。仅当 `K > 5` 时，执行器清空初始五级中仅用于分析
的正式轮次，再按最终 `K` 级集合**一次性**执行两批五轮平衡轮转；不得为每个中间
`K = 6, 7, ...` 重复全量正式矩阵，也不得把发现阶段的连续观察与正式轮次按相同轮号
配对。发现阶段三项观察保留为级别启动、停止和正确性证据，但不得进入最终正式轮次汇总。
...
下一二倍级别在启动前或运行中触发研究停止护栏时，必须把预检或受监控子进程事实保存为
终端边界；只要此前至少五级完整、规模选择已经闭合且没有环境作废原因、终止/监控错误，
该终端边界不把此前完整级别降格为无效，也不阻止其他自然身份继续。父进程为执行墙钟、
私有字节或可用物理内存护栏而终止子进程时产生的非零退出状态，只是该受检终止的伴随
事实；只有脱离具名资源护栏单独出现时，才按子进程异常退出作废。
