...
恢复、候选矩阵以及 Evidence v1 写出/独立验证代码路径已经实现，等待冻结研究机的两批
正式 R0 执行与报告。本文不实现生产编译器、不冻结公共应用程序接口（API），也不形成
产品服务等级协议（product SLA）
...
观察或已完成正式轮次的墙钟观察，不得使用插桩进程的诊断时长。正式轮次中，偶数轮
先运行插桩内存进程，奇数轮先运行非插桩时延进程。两种进程使用相同
...
若五级初始阶梯之后仍需追加倍增级别，执行器必须清空此前仅用于分析的正式轮次，按
新的完整 `K` 级集合重新执行两批五轮平衡轮转；不得把追加级别的连续运行与旧轮次按
相同轮号配对。插桩预检、非插桩时延护栏观察和预言机仍保留为该级启动与正确性证据，
但不得进入重启后的正式轮次汇总。
...
8. 在冻结研究机执行两批正式 R0，发布原始 JSON、Markdown 报告和精确复现命令；
...
`CompileLimits`、产品服务等级协议或真实城市容量结论；只有第 8 步的两批正式证据可以
支撑最终 R0 报告。
