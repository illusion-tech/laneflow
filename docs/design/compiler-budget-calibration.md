...
- `no-reliable-base-scale`：所有适用 `baseScales[]` 都在找到 `B` 前由真实终止护栏
  ...
  建议必须全部为空，但试运行、护栏和独立正确性资格验证仍须保留；
...
构建器，要求三候选各恰好一条资格且资格 ID 全局唯一，再按资格 ID 解析两组各两次
候选运行与一次预言机运行。它必须独立比较逐阶段计数、语义/诊断摘要、重复确定性、
稳定结果和部分输出，重算 `passed`；然后逐条核对候选比较中的资格引用。未执行恒定
哈希构建器、重复/缺少候选、跨候选复用资格、只引用性能运行或失败资格仍形成性能
结论都必须失败。
