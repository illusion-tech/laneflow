...
- 不按统计离群值删除单个样本。只有出现预先声明的外部状态事件时才允许整轮作废：
  ...
  完整 `2C` 轮；环境无法提供某项监控时不得执行正式候选性能比较；
- 每轮后台 CPU 时间总量和后台写入字节总量都是结构化可空观察值。无监控缺样时两项
  ...
  未采集总量；没有监控缺样的运行也不得登记 `monitoring-gap` 原因；
...
`9e3dc42b3458db912036394110228f61a2d2df6b0b9b0c403e677135332ca40d`。该摘要是 PR/Gate
...
G1 候选冻结 schema `227906` exact bytes，SHA-256 为
`382dca541c6bd9eee2dd42399670e152382666a12cd6552e0c014025780e7c3b`。
顶层格式标识：
