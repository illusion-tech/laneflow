...
Instrumentation）的分类定界、外移落点与 instrumentation 边界设计；本文是 #380 的
G1 冻结方案，冻结生效以 #380 Gate Ledger 的 G1 记录为准<br>
**关联文档**:
...
| B 性能归因计时         | 五段纳秒计时（occupancy、longitudinal proposal / projection、post-longitudinal、research-commit）与 Criterion release 取证                                                                                  | whole-step 机制归因，产出 audit D5 归因表                                                                    | baseline §10.1 优化队列（#216–#219 靶点数字来源）；baseline §8.4 契约"coarse stage timing 只用于机制归因" | 活跃：#216–#218 的取证入口；audit §6 已记录一次内嵌计时生命周期 bug 导致整轮归因作废重跑                                                                           |
...
| D retained memory 账本 | `retained_bytes` 系列、`CompleteRetainedComponents` 19 组件穷尽解构账本                                                                                                                                     | 产品认证容量证据 + 常规 PR 回归门禁                                                                          | baseline §5/§8.3/§11；`../reference/rust-code-style.md` §5 常规 PR smoke                                  | 长期测试基础设施，非研究遗留                                                                                                                                       |
...
原则：生产路径与生产类型只保留两类正式接缝——instrumentation 探针边界（承接
B 类）与 test-support seam（承接 C/D 类）；研究原型代码一律移出生产 crate。
...
- **B 类 → instrumentation 探针边界**。生产路径持有 no-op 默认的探针句柄
  （静态分发泛型或 `Option<&mut dyn …>` 的取舍由 G2 定稿），五个计时点与
  `ReducedRateMetrics` 指标收集收敛为探针方法；Criterion 取证迁移为探针实现。
  默认发布构建零成本、零行为变更；audit D5 归因能力与其复现命令等价物完整
  保留并回写 `core-runtime-scalability-audit.md`。探针边界同时消除已发生过的
  内嵌计时生命周期 bug 事故面。
- **C 类 → 正式 test-support seam**。故障注入能力以 crate feature 门控或
  `#[doc(hidden)]` 公共 API（取舍由 G2 定稿）获得长期身份；baseline §4.3/§7.1
  点名的 W3/W4 产品 Gate 验证路径保持不变。
- **D 类 → 同 C 类**。retained 账本获得正式 test-support 身份；穷尽解构设计
...
- 预期新增：至多 1–2 个 crate feature 与若干探针 / seam 公共类型。这构成 API
  新增（新增而非既有行为变更），在此冻结记录；属实现组织调整，不新增 ADR。
- G2 定稿若需要超出上述范围的公共类型，必须回本节追加记录（append-only），
  不得静默扩大。
...
- **#217 / #218**：尚未开工。其未来仪器需求（同类阶段归因计时、#212 semantic
  oracle）由本方案的探针边界与 research 归档包覆盖，不得再次嵌入生产路径。
...
1. C/D 类 test-support seam 化：移除 2 个故障注入字段，retained 账本正式化。
2. B 类探针边界：移除 5 处热路径计时钩子与 `ReducedRateMetrics` 内嵌收集。
3. A 类归档 `research/`：摘除 `#[path]` 挂载，移除 reduced-rate Active 分叉与
   `*_for_research` seam。
...
本身（降级项须在 #380 记录并拆后续）；不做 CoreWorld 命令域拆分（由 #381 承担，
建议在本 Issue 之后）；不改动 Data/Spatial/Scenario/Bevy crate。
