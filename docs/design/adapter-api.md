...
- `ReplaceError::DownstreamSpeedUnsatisfiable`、`InvalidDepartureState` 和 `InitialSpeedExceedsDepartureBound` 不是可重试结果。Adapter 把它们记入 `last_error` 并返回 `LaneFlowAdapterError::VehicleReplace`。世界和 Entity 映射保持不变。
