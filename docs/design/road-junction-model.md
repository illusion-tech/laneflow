...
- 同一 Junction 内两条 ManeuverPath 不得拥有完全相同的 edge sequence。
...
6. 派生 internal-edge owner，并返回首个 cross-Junction/internal-boundary conflict；
7. 按 Junction/Movement input order校验 non-empty child cardinality；
8. 构造 dense definitions、flat member ranges、flat path-edge storage 与 candidate index。
...
- identical path sequence；
- shared entry transition 后分叉；
- zero/multiple full match；
- repeated Route edge 与 occurrence index；
- finite loop；
