...
业务时间、历法和规则启用时机由宿主产品拥有，适用于游戏、数字孪生、仿真等接入场景。
宿主可以依据真实日期、历史回放时间或自定义历法选择策略；LaneFlow 按显式选择的
策略身份与版本执行规则，不要求真实公历、不读取宿主墙钟，也不按日期筛选策略。
法规版本及来源用于固定规则含义和审计追溯。外部业务时间与数据的接入由宿主转换为
既有正式编制输入、策略选择和运行时命令；核心不内置特定产品的业务日历或数据服务。
固定步进的模拟时长仍用于运动、信号相位与间隙计算，不能替代业务日期或由墙钟隐式推进。
...
实际类别收集只遍历一次车辆配置关系，通过标记按类别 ordinal 顺序输出；标记初始化、
收集和去重工作及临时存储计入既有预算。
...
与候选消费负责按实际 coverage 聚合最小值，纯 Waiting 保持 absent。
