...
**文档状态**: Accepted（#285；只读观测、参考场景、有限回归与有界性能报告）<br>
**最后更新**: 2026-09-14（#285 经用户授权调整为可运行示例与有界性能报告）<br>
...
smoke，均基于检入 catalog 0.1 + LFCA 与固定四车 spawn 计划）：同根几何生成的连续
道路、相切路缘、同向双车道回流与车辆 box 表现；四车从环路末段的固定槽位出发。
F2 默认关闭的调试 overlay（静态 Gate 标记、Waiting 区间 ribbon、Conflict
...
每项登记已有测试／制品证据，成功与失败都通过生产入口。该有限矩阵保留为行为回归；
§6 的规模运行报告实际观察范围，不要求把全部矩阵或理论组合乘进两档规模窗口。
...
#285 的交付是可运行的复杂路口参考示例和有界性能报告。每档一万／十万各运行一个
integrated process，每进程最多推进 4,096 个 tick，两档测量批次合计墙钟不超过
600 秒；tick 上限与时间上限先到即停。该协议不另加仿真预热，采用冷启动采样；
运行时初始化、零步渲染准备和结果收尾均计入批次墙钟，受控构建、输入生成与启动前
摘要／环境核对在测量批次外记录。

runner 按剩余时间与剩余档数分配进程时间，子进程预留收尾时间并在帧边界正常退出，
父进程以硬截止保护失去响应的执行。到时间上限时，只要有成功 tick、有效呈现和完整
的实际窗口校验，即可作为有界可运行证据；初始化失败、崩溃、硬超时杀进程、零 tick
或校验失败必须报告失败。实际 ticks、仿真时长、帧数、进程／批次墙钟、停止原因、
未消化 backlog 和未观察到的帧类别均如实报告，不补齐或外推为 4,096 ticks。

一万 Runtime p95 ≤ 2 ms/tick、十万 ≤ 16 ms/tick、Spatial+Adapter ≤ 4 ms/frame
等现行预算只作比较，不作为 #285 完成门槛。P10/P100 具名硬件、完整产品窗口、
三轮重复性、额外 allocation／组件账本运行与产品认证不属于该有界交付。统一产品
性能基线及其工作负载合同保持原义，不能用本专项的短窗口宣称产品认证。

| 证据                       | 拥有范围与复用方式                                                                                                                                                                                   |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 复杂路口专项（#285）       | 只读观测、参考场景、调试验证、§5 已有有限行为回归及一万／十万有界 integrated run；报告实际窗口、组件延迟、资源活动与进程峰值 |
...
Waiting/Conflict 的持有、申请、事件与候选数量按实际窗口报告；短窗口未出现的行为
记为零或未观察到，不添加最小持续时长与重复申请门槛，也不外推高并发或长时间行为。
领域观测保留全量遍历，不把“仅读取少数车辆”写成全量批量成本。
...
按实际窗口给出 p50/p95/p99/max 和普通／catch-up 帧分类；无样本的类别留空。
跨层 percentile 只能来自同一 integrated run，不相加历史分位。进程内存峰值包含
校验器、截图与序列化，不能当作 Runtime 组件账本；未测量项明确列出。

已有逐拍／逐帧校验继续覆盖实际推进与呈现的全部行，失败立即保留诊断制品；有限
领域矩阵继续承担该规模输入未执行的生命周期与失败分支。输入、完整初态、程序摘要、
PID 与进程参数绑定继续核对。长期行为合同不因缩短采样而改变。本次验收调整在 #285
中登记用户授权与理由，原长测记录保留为历史证据，不作为新协议的前置条件。
...
收口 PR 按修订后的可运行与有界报告验收映射本设计，核对实现、已有行为回归、
API／格式影响、实际测量与未测量项；不另设长期取证或认证审批阶段。原始运行记录、
