...
**实现状态**: 未实现；#292 G1 正在冻结，当前生产路径仍是 `Traffic v0.10` /
`SpatialPackage v0.1` / `ScenarioManifest v0.1` / `laneflow-data` /
...
无限生产模式。G1 必须按产品容量、安全边界、代表工作负载与目标硬件预算冻结默认值，
至少限制：
...
| 工作负载标识符             | 目的                                         | 主要计数对象                       |
| -------------------------- | -------------------------------------------- | ---------------------------------- |
| `LF-COMP-ID-v1`            | 标识 v1 全部实体种类 / 标签与父项闭包        | 模块、稳定实体、标识字段 / 字节    |
| `LF-COMP-CORRIDOR-v1`      | 线性走廊、信号 / 停车 / 横断面扩展           | 边、关系、路线、信号状态、几何点   |
| `LF-COMP-JUNCTION-GRID-v1` | 密集路口、路径 / 机动门 / 等待区出现项与索引 | 路口、路径、机动门、等待区、出现项 |
| `LF-COMP-CURRENT-EQUIV-v1` | 两个当前态固定样例的端到端迁移               | 阶段记录、投影记录、诊断           |
...
### 11.3 G1 预算待决与 G2/G3 实测

G1 必须以产品容量、安全边界、现有固定样例和目标硬件角色冻结：
...
- 峰值 / 保留内存预算与增长斜率；
- 哈希 / 缓存候选、选择规则与拒绝服务边界；
- 冷实例和稳定容量复用的测量协议、失败清理要求。

这些是实现必须满足的输入，不要求在 `G1 Pass` 前先编写生产编译器来证明结果。G2
按冻结协议取得实际数据并选择不泄漏到公共契约的私有容器；G3 必须记录同一固定性能
机上的完整结果。若实现不能满足预算，或容器选择需要改变公共接口、确定性语义或依赖
方向，必须暂停并回到 G1。若维护者要求在 G1 前取得实现级数据，应另立具有自身
G0–G2 的非生产研究切片，不能在 #292 中隐式越过 G2。

不得把任意未解释数字、运行时一万 / 十万交通参与单元或多世界吞吐直接换算成
编译器通过条件；预算来源、计数对象、单位和目标硬件必须可追溯。
...
- [ ] `CompileLimits`、工作负载规模、时延 / 内存预算及其产品 / 安全依据替换待决项；
