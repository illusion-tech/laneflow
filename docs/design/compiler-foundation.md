...
（no-go）”结论的当前核心 `f32` 生产迁移：ADR 0014 本身仍是 Accepted；Spatial
精度条款已被 Accepted ADR 0015 部分取代。首次生产迁移未通过百分之五性能门槛并
完整回退，当前生产交通数值权威继续使用 `f64`。Proposed ADR 0028（未 Pass）若
通过，将取代该一维目标合同；G2 完成前不得按残差 `f32` 或整数毫米实施。未来若
通过新的数值 G1 改变交通权威表示，必须原子修订编译器中间表示、可移植规范制品、
共享静态路网（Shared Static Network）、目标交通运行时和迁移预言机，不能由编译
发射器在后端私自窄化。
