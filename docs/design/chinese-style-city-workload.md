...
- `urban-demand-harness.md`（#544 G1 Accepted，细化需求与无界面验证）
...
同义声明共享避免编制复制；准入与路权解析依照
[路权专题合同](traffic-runtime-right-of-way-policy.md)，按各策略下实际使用的参与者类别共享。
增加同类别的不同车辆配置仍有配置本身的编译和存储成本，但不增加对应门/流的解析行。
不同类别分别解析，不能因当前规则结果相同就合并；门/流解析行与车辆配置数量分别报告。
...
无界面工作负载的既有正式性能证据由 #544 交付并保留原始窗口及轮次。
#545 的新增性能观测采用有限时长协议：`MIXED-PEAK` 的 10k/100k 各一条
headless 和 Adapter 行，每行在新进程中运行，最多提交 4096 ticks；四行串行执行的
整批墙钟上限为 600 秒。构建及静态制品生成另外记录，不计入运行批次；进程启动、
世界安装、运行和结果写出计入。每行按剩余时间与剩余行数分配预算，提前预留结果
写出时间；到期在完整提交边界停止，强制终止、空样本或缺失结果视为失败。
报告实际起止 tick、样本数、墙钟、停止原因、Runtime step、全量 pose 提取、选择/映射
及 Transform 应用的 p50/p95/p99/max、进程峰值驻留内存和环境。每行的全量正确性
观测成本单独标明，不与组件计时相加冒充整帧或产品延迟；截断窗口只代表实际前缀。
本次不要求三个独立进程轮次或完整信号周期性能窗口，不以 #544 旧轮次补齐本次
统计，也不把这些有限观测作为性能基线 §8 或 #539 的产品认证。
停车、保存/恢复和切换的功能正确性仍按预先固定的有限输入及实际触发验证，不能
因性能到期而把未触发行为记为通过；复用 #544 完整无界面证据，避免重跑其全部矩阵。
