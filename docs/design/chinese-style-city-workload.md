...
首版车型定义在共同来源模块中共享。相同含义的 `ParticipantClass`、`VehicleProfile`
不按 cell/tile 复制；类别与参数档数量属于固定生成配置，扩大地图不扩大车型目录。
真正不同的长短车型可使用不同参数档，必须在计数和配置中明确，不能用同义副本制造
策略的“门/流 × 参数档”组合膨胀。
...
本 case 的必需转换；信号周期按 fixed step 的逐相位取整语义计算。无信号时按有限
