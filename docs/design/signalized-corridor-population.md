...
每个 logical slot 使用 `corridor-vehicle-{index:03}` external ID。`prepare` 对完整规范 slot catalog 执行从末尾到开头的 Fisher–Yates 后取前 N 个 slot，初始速度固定为零。50、100、200 三种目标人口都必须通过同 seed 整批 golden、Core batch no-overlap 和 tick-0 bind 验证。
