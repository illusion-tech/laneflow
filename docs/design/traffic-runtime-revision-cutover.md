...
路网修订不可变（ADR 0025），而宿主应用的改路与制品重发布要求运行中世界换绑
...
[执行配置与资源设计](traffic-runtime-execution-config.md)进一步区分活动世界的执行
资源和候选计划，覆盖两条切换路径的准备/失败/发布寿命。该文已 Accepted，
切换继承活动世界的 `ExecutionConfig`，不改变本文的公开切换签名；执行资源与候选
计划的生命周期在实际后端接入时实施，不能把配置字段复制当作资源所有权证明。
...
| 候选规范制品字节 | 同修订换根不产生候选制品；夹具 LFCA exact bytes `420,332`                                                                                                                                                                                                                                                                                |
| 旧修订回收延迟   | 最后借用退出后同构根析构墙钟中位 `0.020 ms`（孤立析构；含在调用内回收的混合形态见静默提交窗口行上界口径）                                                                                                                                                                                                                                |

### 历史切片 C 初值（跨修订直移，宿主泵式在线形态）
...
上述表格是早期八车夹具记录，含历史日志布局和格式大小，不是当前规模预算。
当前普通 tick 增量为 `37 B` 头加每辆发生变化的车 `78 B`，另加实际 Waiting/Conflict
增量；默认 arena 仍为 `8 MiB`，不能直接把历史 64 tick 窗口扩成万车夹具。

### 当前重验证与规模证据（#531）

重叠复核沿用道路准入的车身截断和零进度入口点唯一性判据（`vehicle-following.md`
§7.4），从相关物理边桶查询并排除自身，保留 live 序的实体校验与首错。候选首次
需要时可失败重建；同一轮批量复核复用它。车辆 tick 增量使其失效，生命周期增量
维护旧/新登记，晋升一并交换候选索引。预留失败返回 `StagingAllocFailed`，旧世界与
事件不受影响。跟车占用索引不登记退化入口点，不能直接替代准入索引。

摘要按路线实例预分组，源侧期望仍独立捕获和规范化；保留两份摘要比较、摘要版本 7
及全部既有失败原子性。该优化消除每条路线全扫车辆的分组成本，不改变摘要前像。

规模证据使用同机 Rust 1.98 release、单世界千车/四千车/万车、256 条八公里物理边，
其中使用 16 或 256 条边，4.5 米车辆按十米间距分布；target 仅把限速从 15 提高到
16 米每秒，路线、profile 和当前状态均可直移。准备时不步进，在线窗口只推进两拍；
万车普通日志为 `1,560,074 B`。路线数另取万条验证分组成本，不展开全部组合。

- `cutover_scale_evidence`：未插桩墙钟，三个样本中位；区分准备、泵入、尾已泵空／
  有界尾排空的静默提交和零步进维护暂停。
- `cutover_scale_allocation`：独立计数分配器二进制，分别报告准备、摘要和提交分配。
- `kernel::spawn_overlap_tests::cutover_overlap_scale_evidence`：候选访问计数与索引
  retained-memory；普通回归另验证批量复核只冷建一次以及生命周期维护。

同机对照的基线为 `d5bc2b64f277294f532844df5fa04ae4194b6b29`。每轮万车重叠候选从
`99,990,000` 次降为 16 边的 `6,240,000` 次、256 边的 `380,640` 次；该夹具候选索引
保留 `137,216 B`，纳入 Derived owner 账本。完整耗时与分配记录由交付 PR 关联。

不再使用旧秒数推算或「仅有两个超线性项」概括当前实现。密集单边仍可二次退化，
日志重放中的活动序／Waiting 聚合重建、快照捕获、规范字节构造和回收仍计入真实停顿。
这些规模夹具不证明代表性城市工作负载或在线停顿预算已满足；分段提交、墙钟中止与
其他聚合优化属于独立范围，后续整体画像由 #583 承担。
...
- 正式审阅轮回写（切片 C）：Prepare 对目标修订执行信号程序 × 本世界步长的
  ...
  新旧根的任何 Arc 与动态表都不滞留（观测入口对已结算事务按内部不变量
  panic）。
