...
### D2. handle 是不透明 typed newtype

状态：拟接受。

v0.2 public API 使用三个互不兼容的 handle 类型：
...
```rust
let vehicle_id = world.vehicle_external_id(event.vehicle());
```
...
v0.2 优先使用自有的最小 typed handle newtypes 和 registry，不引入 `slotmap`、`generational-arena` 等 handle 管理 crate。

原因：

- LaneFlow 只需要少数 domain-specific typed handles、resolver 和生命周期规则，不需要通用 arena 的完整抽象面。
- 自有 newtype 可以保持 public API 极小，并避免过早冻结第三方 crate 的 key 类型、生命周期模型和许可证评审。
