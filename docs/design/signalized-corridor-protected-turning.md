...
相对每个 Junction 中心，主干道 entry/exit boundary 为 `x = ±10.5 m`，次干道
entry/exit boundary 为 `z = ±14.0 m`。停止线位于横向道路铺装边缘之外一个 lane
width，即额外 setback 为 `3.5 m`。lane index `0` 从道路中心侧向外递增。
