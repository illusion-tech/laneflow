...
**文档状态**：Accepted（#681；2026-09-17 用户接受；切片 1 已由 #711 实施、切片 2 已由 #712 实施）<br>
**最后更新**：2026-09-18<br>
...
`committed_pose_sources()` 已由 #712 迁移为借用世界的惰性迭代器，Adapter
全量封闭提取复用 Session 候选缓冲并在成功后交换提交；按显式选择减少采样仍是
后续独立切片（#713），不为 1.0 前接口保留别名或兼容双轨。
...
1. Spatial 成功提交改为 Vec 交换（已由 #711 实施）：内部局部优化，现有 API 和
   ...
   API 迁移。
2. Runtime 借用来源与 Adapter 全量缓冲迁移（已由 #712 实施）：依赖本设计
   G1，使用同一来源原语，一次迁移所有调用方；验收稳态 source 分配消失、
   原有首错和全量输出等价（测量见 research/issue-712-borrowed-sources）。
