...
`spawn_vehicle`、`replace_completed_vehicle`、`leave_parking` 与跨修订重验证使用独立的
...
整值改写或包含车辆状态的 tick 增量重放使缓存失效，下一次查询从活动车辆重建。
切换候选的构建与增量登记使用可失败预留；一轮批量重验证复用同一索引并排除自身。
生命周期增量在清除旧状态前注销、校验通过后登记；若该次查询已经冷重建到新状态，
不重复登记。仅路线或时钟变化不使索引失效，晋升时一并交换候选索引。失败操作
...
定位/更新与离场 follower 安全校验保留各自成本。跨修订每轮重验证的候选数受相关桶
约束；生命周期重放中的活动序、Waiting 聚合重建不属于本索引优化。
