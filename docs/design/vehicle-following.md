...
Vehicle Profile shape 由 current `schemas/laneflow-data-v0.5.schema.json` 继续承载。概念 package：
...
- 当前 v0.5 沿用已接受的 Vehicle Profile 领域语义，并要求显式 Signals/Parking objects。
- 顶层 `vehicleProfiles` 必填，允许空数组。
- Core-defined objects 继续采用 closed shape。
- production loader 只接受 `"0.5"`；旧版和未来版在当前 shape validation 前返回 version error。
...
依据 ADR 0007/0008，current v0.5 production loader 位于 `laneflow-data`，依赖方向为 `laneflow-data -> laneflow-core`。Core 不依赖 Serde、JSON、JSON Schema 或文件系统；pre-1.0 的 production loader 只维护当前格式。
...
spawn/atomic replace 的初始速度不得超过当前 edge 限速，v0.8 默认初始与回流速度为零。限速值、14 条 route 与测试矩阵见 `example-scenarios.md`；Core API、行为和性能验证由 #185 实施。
