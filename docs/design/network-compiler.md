...
返回 `G1 In Progress`，旧 Geometry JSON 实现只作历史证据，新的 FlatBuffers 道路编辑
来源在取得 exact-head G1 Pass 前不构成已接受或当前生产事实；#297 不再建立 current
JSON 编译器前端
...
私有接入的规则，不交付或冻结 Geometry 公共签名。#296 新 G1 选择借用完整
...
长期 production authoring frontend。#296 新 G1 选择
...
兼容承诺；新契约在取得 exact-head G1 Pass 前仍不构成当前生产事实。
