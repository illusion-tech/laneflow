//! Public-API reduction of the 10k tick-172 finding. Step, cursor, or restore errors return nonzero.
use std::path::Path;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let edges: Vec<_> = spec
        ...
        .map(|key| {
            let id: LaneEdgeId = format!(
                ...
                catalog.edge_ids[key]
            )
            .parse()
            .unwrap();
            revision.identity().ordinal(id).unwrap()
        })
        .collect();
    ...
    println!(
        "following step: {}",
        world.step(TickInput::new(16))?.tick_index()
    );
    drop(world);
    ...
    let length = revision.traffic().lane_lengths_millimetres()
