...
- 固定时制信号程序 9 相位 5 组：主路直行（进入待转区）→主路保护左转（清空
  待转区）→次路直行，各放行段间含黄灯与全红。主路直行与次路许可左转同相位
  （许可左转必须在主路直行车流中找间隙）。第五组 `main-waiting-entry` 约束待转
  路径的 admission / waiting-entry，在主路直行或主路左转绿灯均允许准入；release
  只受 `main-left` 控制。容量与下游净空仍由 Runtime 判定，不因绿灯绕过。
  待转区停车线位于南北车行道边缘之外至少 0.5 m，迟到而跨周期等候的车辆也
  不占用南北车道。五个逻辑信号组不表示五根实体灯杆。相位时长来自配置。
- 西进口左转的连接、储车、驶出取自同一条相切直线与四分之一圆弧。圆弧采用
...
- 1 个 WaitingZone（西进口的一条保护左转路径）。名义容量按同一配置车型与整数毫米
  储车长度计算：`floor((pocket_length + min_gap) / (vehicle_length + min_gap))`。
