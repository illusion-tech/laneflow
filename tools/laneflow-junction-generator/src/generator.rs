...
pub fn generate(config: &JunctionConfig) -> Result<GeneratedScenario, Error> {
    ...
    let counts = ScenarioCounts {
        ...
        movements: topology.movements.len() + 2 * topology.merges.len(),
        maneuver_paths: topology.paths.len() + 2 * topology.merges.len(),
        maneuver_gates: topology.gates.len() + 2 * topology.merges.len(),
        stop_lines: topology.stop_lines.len() + 2 * topology.merges.len(),
        waiting_zones: 1,
        conflict_zones: topology.zones.len() + topology.merges.len(),
        streams: topology.streams.len() + 2 * topology.merges.len(),
