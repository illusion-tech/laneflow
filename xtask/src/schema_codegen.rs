...
/// 钉版的 FlatBuffers 运行时与 flatc 版本；wire 生成物比对与审计常量均以该版本为准。
...
/// 单个 wire 家族的 codegen 检查入口：校验仓库根、受审计 unsafe 边界、flatbuffers 运行时与 flatc 钉版后重跑 flatc，比对 Rust 生成物 canonical bytes 并生成 C++/C# probe。
...
/// 复核受审计 mmap 例外 crate 的全部 Rust 源码：禁绝 `#[path]`、`include!`、`cfg_attr`、`macro_rules!` 等逃逸文本扫描的形态。断言唯一豁免源恰好一次模块级 allow、一次固定只读映射调用与一处 unsafe token，其余源文件零 unsafe。
...
/// 经 `cargo metadata --no-deps --locked` 列出 workspace 全部成员的 manifest 路径。
...
/// 统计文本中独立 `unsafe` token 的数量：仅当前后均非标识符字符的出现才计数。
...
/// 递归收集 `root` 目录下扩展名为 `extension` 的全部文件，追加到 `files`。
