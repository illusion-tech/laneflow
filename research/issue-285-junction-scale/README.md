...
policy、车型、几何和 catalog 绑定入口。32/320 个独立参考单元分别容纳 10,000/100,000
辆标准车；每单元 312/313 辆，按已有 portal lane 槽位轮流生成，初速为零。
每单元包含一个中心路口和两个外环汇合路口，对应两档 96/960 个静态 Junction 实体；
输入中的 `cells` 记录参考单元数。
