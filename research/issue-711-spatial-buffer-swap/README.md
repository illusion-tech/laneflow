...
- 运行：固定单个 logical processor（≥17 核时固定第 17 个，否则最高位），
  未锁频；电源方案取证失败（powercfg 调用报错，environment.json 中为空），
  **未验证、不作声明**。三组 A/B 墙钟进程按 A₁B₁B₂A₂A₃B₃ 交错；分配构建每
  变体一次。详细环境见各 `evidence/*/**/environment.json`。
...
1.5 倍差；电源方案未能取证，频率波动只是推测之一）。快模式成本两侧相等：
