...
pub fn run_formal_protocol(
    ...
    Ok(FormalProtocolOutcome {
        ...
        artifact_kind: "base-scale-pilot-checkpoint".to_owned(),
...
struct BaseScaleCheckpointWriter {
    output_path: PathBuf,
    checkpoint_directory: PathBuf,
    next_sequence: u64,
}

impl BaseScaleCheckpointWriter {
    ...
    fn persist(
        &mut self,
        checkpoint: &BaseScalePilotCheckpoint,
    ) -> Result<(), FormalProtocolError> {
        let path = self
            .checkpoint_directory
    ...
    fn finish(&self, checkpoint: &BaseScalePilotCheckpoint) -> Result<(), FormalProtocolError> {
...
pub enum FormalProtocolError {
    ...
    #[error("无法序列化基础规模试运行检查点")]
    SerializeCheckpoint {
        #[source]
        source: serde_json::Error,
    },
    #[error("无法写入基础规模试运行检查点 {path}")]
    WriteCheckpoint {
        path: PathBuf,
        #[source]
        source: std::io::Error,
...
mod tests {
    ...
    fn checkpoints_are_immutable_and_final_output_is_atomic() {
        ...
        let output = directory.join("base-scale-pilot.json");
        let mut writer = BaseScaleCheckpointWriter::prepare(&output).expect("prepare writer");
        ...
        assert!(matches!(
            BaseScaleCheckpointWriter::prepare(&output),
    ...
    fn empty_checkpoint() -> BaseScalePilotCheckpoint {
