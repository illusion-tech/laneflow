...
- `run --protocol compiler-calibration-v1 --output <path>` 正式研究入口：入口在任何
  ...
  （SLA）。正式阶梯遇到无效轮次时保存事实并停止，不在单次研究运行内自动重试；唯一
  例外是已有至少五级完整证据和闭合规模选择后，下一二倍级别触发具名资源停止护栏且没有
  环境、终止或监控错误：此时仅把该运行保存为当前阶梯的终端边界，并继续其他自然身份。
  排除其他无效轮次的环境干扰后使用新输出路径重跑完整协议。若受测子进程先于下一二倍级别预检触发运行中
