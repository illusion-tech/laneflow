...
- 父子进程启动握手与父进程持续监控：父进程在放行管线前取得子进程私有字节初始快照，
  ...
  达到阈值或监控缺样时终止子进程，并构造内存中的结构化无效结果后返回错误；正式
  Evidence 落盘仍属后续切片。正常完成的每个样本在试运行 JSON 中保存最后/峰值私有
  字节、观察次数、退出码和实际父进程墙钟。当前私有字节提供程序只在 Windows 上绑定
...
投影、正式新进程轮次编排、完整跨平台终止状态编码、证据写出器和正式
...
父进程运行中监控已经执行，但 POSIX 信号/原生平台状态编码、正式监控缺样无效运行
封套、`timing`/`attribution` 二进制分离和 Evidence v1 独立验证尚未建立，所以
`guardPreflightEvaluated` 仍只表示完成启动前预检，不等于协议 v1 护栏闭合。当前
试运行中的原子额度记账是护栏机制验证，不能把其墙钟冒充第 7.2 节禁止逐分配插桩的
正式 `timing` 结果。上述原语单独输出的墙钟值、保留容量和内存观察没有预算资格，
试运行中即使满足时钟分辨率、离散程度与摘要一致性条件，也不得据此选择参考规模 `B`。
