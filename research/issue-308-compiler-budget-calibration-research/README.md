...
引用和同长度字符串替换均有拒绝测试。当前固定样例研究投影、正式新进程轮次编排、
证据写出器和正式
...
写入向量前，命令会先要求身份、走廊与路口网格三个工作负载各自的生产者/独立预言机
六个用例完全一致；任一用例不一致时不会写入。

执行 `LF-COMP-ID-v1`、`LF-COMP-CORRIDOR-v1` 与
`LF-COMP-JUNCTION-GRID-v1` 的生产者/独立预言机各六个用例交叉验证（同时核对
身份阶段精确内容、两类结构工作负载的完整记录流与各自公式结果）：
