...
- `LF-COMP-CORRIDOR-v1` 对研究内部冻结的走廊单元配方完成有类型模板投影，
...
- `run --protocol compiler-calibration-v1 --output <path>` 正式研究入口：入口在任何
  ...
  语义摘要一致性和护栏状态；`B` 只使用时钟分辨率、独立预言机正确性和护栏资格，MAD
  作为跨进程抖动诊断保留，不得因抖动把工作负载继续二倍扩大。无效尝试完整保留并以新
  尝试身份重试，不能进入贡献集合。
  找到 `B` 后执行固定五级正式阶梯；每个新级别先取得一次插桩预检和一次独立预言机
  结果，再对 `B、2B、4B、8B、16B` 固定五级按两批、每批五轮运行非插桩时延与插桩
  归因子进程。每个子进程取得一次冷实例、三次不计时预热和七次稳定容量复用，随后从
  完整有效原始值重算中位数、MAD、相邻级别比值、成本拐点和校准/压力规模。检查点保留
  已完成状态，成功结束时发布正式执行
  检查点；该制品仍不是编译器校准证据 v1，也不包含预算建议或产品服务等级协议
  （SLA）。五级内确认拐点时选择其前一级与拐点级；未确认时选择 `8B` 与 `16B` 并明确
  标记“测量窗口内未观察到拐点”，不再仅为寻找极限而探测 `32B` 以上规模。正式阶梯
  遇到无效轮次时保存事实并停止，不在单次研究运行内自动重试；排除环境干扰后使用新输出
  路径重跑完整协议。若受测子进程在固定五级内触发运行中停止护栏，执行器会持久化统一
  作废后的尝试，并禁止该不完整自然身份进入下游资格；它不会重试超限规模，也不会把
  冷实例运行伪装成 `terminalGuardRunId` 所要求的实际 `guard-preflight` 运行。
...
- `known-vectors/junction-grid-summary-v1.json`：精确长度 `8601` 字节，SHA-256
  `2d6c56d682792f8919654404c38b658be0c8eedee3eff1831b5871d6ea592949`。
...
`N = 2` 六个用例中逐条、逐字节核对生产者。current JSON 生产加载器已随 #301 删除，
走廊单元配方不再对照 JSON 文件。
...
结论。正式阶梯已经分离冷实例与稳定容量复用、时延与归因指标，并从两批五轮的有效
  原始数据选择校准规模与压力规模。重复性包络、相邻级别精确比值、失败清理、候选矩阵和完整
...
独立预言机六个用例完全一致；任一用例不一致时不会写入。

执行 `LF-COMP-ID-v1`、`LF-COMP-CORRIDOR-v1` 与
`LF-COMP-JUNCTION-GRID-v1` 的生产者/独立预言机各六个用例（核对
身份阶段精确内容、结构工作负载的完整记录流与各自公式结果）：
...
`laneflow.compiler-calibration-formal-execution-checkpoint` 执行检查点（当前内部
