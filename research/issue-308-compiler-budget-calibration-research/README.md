...
投影、正式新进程轮次编排、子进程受控分配硬上限与父进程连续监控、证据写出器和正式
...
只用于父子进程通信，不是正式证据封套。父进程预检已经执行，但子进程受控分配硬上限、
运行中私有内存/墙钟监控和完整终止分类尚未建立，所以
`guardPreflightEvaluated` 只表示已完成当前预检，不等于协议 v1 护栏闭合。上述原语
单独输出的墙钟值和保留容量没有预算资格，试运行中即使满足时钟分辨率、离散程度与
摘要一致性条件，也不得据此选择参考规模 `B`。
