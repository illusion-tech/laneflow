...
本次完整测量源码为已发布提交
[`c7da60e5`](https://github.com/illusion-tech/laneflow/commit/c7da60e5a2eab3bb5541454d8365b6aea991d271)，
tree 为 `033600e25db75de9c61a24d3caf5823bff3239f8`；全部证据在该干净工作区重跑。
...
| ------------------- | -------: | --------: | ------------------------------ |
| Runtime 按值 source | 0.169416 |  2.665966 | 生产查询、收集及释放 Vec       |
| Spatial 全量        | 2.851363 | 14.708944 | 全部采样和成功复制             |
| Adapter 全量        | 3.488597 | 18.767659 | 生产封闭提取（包含前两类工作） |
| 仅 records 复制内核 | 0.007113 |  0.175569 | 已有记录到复用 Vec             |
| 选取探针：100%      | 1.880094 | 16.577172 | 已知 Active 句柄查询与采样     |
| 选取探针：10%       | 0.174250 |  1.647334 | 同上，1000 / 10000 条          |
| 选取探针：1%        | 0.013244 |  0.155872 | 同上，100 / 1000 条            |

十万档 Transform 转换在全量后选择 10% 时仍为 10.923788 ms；先选再转换的一万辆
为 1.037778 ms。这里是实际 Bevy `Transform::looking_to`，没有渲染器、GPU 或
...
| 探针                                      |  ms/调用 | 稳态申请                    |
| ----------------------------------------- | -------: | --------------------------- |
| 重建全量 live HashMap + 核对绑定          | 6.252209 | 1 次分配，2228240 B         |
| 仅核对保留绑定与有效 handle               | 0.679328 | 0 次                        |
| 真实 ECS 移除/插入 Transform 的可见性切换 | 1.542013 | 每 32 调用仍约 1 次 realloc |
...
128 个互异静态泊位在同根上重复采样约 0.051005 ms/批；预计算记录复制内核约
0.000093 ms/批。缓存 payload 是 5120 B 的 record（含研究用固定 record id）；
