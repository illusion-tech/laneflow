...
def validate_frozen_index(index, frozen):
    canonical = json.dumps(index, sort_keys=True, separators=(',', ':')).encode('utf-8')
    require(hashlib.sha256(canonical).hexdigest() == frozen['raw_evidence_index']['canonical_sha256'], 'raw evidence index changed')
...
def captured_streams(meta_path, label):
    ...
    require(all(p.is_file() for p in streams), 'missing captured stream')
    return streams
...
def validate_frozen_run(meta_path, run, frozen):
    ...
    for path in paths:
        ...
        require(record is not None, 'run absent from frozen evidence')
        require(path.stat().st_size == record['bytes'] and digest(path) == record['sha256'], 'frozen evidence hash: ' + path.name)
...
def analyze(meta_path):
    ...
    require(meta['scale'] in frozen['inputs'], 'unknown scale')
    require(meta['exit_code'] == 0, 'failed process')
    for field in ['source_unchanged', 'binary_unchanged', 'plan_unchanged', 'bundle_head_unchanged']:
        require(meta[field] is True, field)
    require(meta['source_identity_before'] == meta['source_identity_after'], 'source changed')
    require(meta['source_identity_before'] == frozen['source_hash'], 'unfrozen source')
    require(meta['bundle_head'] == meta['bundle_head_after'], 'bundle HEAD changed')
    require(meta['bundle_status'] == '', 'uncommitted bundle')
    ...
    require(digest(binary_path) == meta['binary_sha256'].lower(), 'binary hash')
    require(digest(plan_path) == meta['plan_sha256'].lower(), 'plan hash')
    require(meta['plan_sha256'].lower() == frozen['plans'][meta['scale']]['sha256'], 'unfrozen plan')
    require(binary_name in frozen['binaries'] and meta['binary_sha256'].lower() == frozen['binaries'][binary_name]['sha256'], 'unfrozen binary')
    run = meta_path.parent / meta['label']
    summary = json.loads((run / 'summary.json').read_text())
    require(summary['status'] == 'research-prefix-complete', 'incomplete run')
    require(summary['error'] is None, 'run error')
    require(summary['mode'] == meta['mode'] and summary['workers'] == meta['workers'], 'mode/worker mismatch')
    require(summary['counting'] == meta['diagnostic'], 'counting mismatch')
    require(summary['plan_sha256'] == meta['plan_sha256'].lower(), 'summary plan hash')
    require(summary['input_manifest_sha256'] == frozen['inputs'][meta['scale']]['manifest.toml']['sha256'], 'unfrozen input manifest')
    ...
    require(summary['run_identity'] == expected_identity, 'summary run identity')
    rows = [{k: int(v) for k, v in r.items()} for r in csv.DictReader((run / 'timing.csv').open())]
    require([r['tick'] for r in rows] == list(range(1, meta['ticks'] + 1)), 'missing/repeated ticks')
    require(summary['completed_ticks'] == meta['ticks'], 'completed tick count')
    quality = json.loads((run / 'extended-quality.json').read_text())
    require(quality['ticks'] == summary['completed_ticks'], 'quality tick count')
    require(quality['run_identity'] == expected_identity, 'quality run identity')
    require(digest(run / 'extended-quality.json') == summary['quality_sha256'], 'quality hash')
    require(digest(run / 'individual-quality.csv') == summary['trips_sha256'], 'trip hash')
    work = [{k: int(v) for k, v in r.items()} for r in csv.DictReader((run / 'work.csv').open())]
    require([r['tick'] for r in work] == [r['tick'] for r in rows], 'work tick identity')
    if meta['diagnostic']:
        require(meta['workers'] == 1, 'TLS counters omit helper threads')
        require(sum(r['waiting_entries'] for r in work) > 0, 'missing diagnostic counts')
    else:
        require(all(v == 0 for r in work for k, v in r.items() if k != 'tick'), 'unexpected instrumentation')
    ...
    with (run / 'individual-quality.csv').open() as trip_file:
        require(all(row['end_kind'] != 'parked' for row in csv.DictReader(trip_file)), 'unsupported command-parked trip')
    windows = {}
