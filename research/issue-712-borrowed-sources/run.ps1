param(
    ...
    # 传递给测量程序的 cargo features；A 侧必须传 legacy-source。
    [string]$Features = ''
)
...
$featureArgs = @()
if ($Features -ne '') { $featureArgs = @('--features', $Features) }
& cargo build --locked --offline --release --manifest-path $manifest --target-dir target @featureArgs
if ($LASTEXITCODE -ne 0) { throw 'Normal build failed' }
Copy-Item -LiteralPath target/release/laneflow-issue-712.exe -Destination (Join-Path $out 'wall.exe')
& cargo build --locked --offline --release --features "$( (@($Features, 'allocation') | Where-Object { $_ -ne '' }) -join ',' )" --manifest-path $manifest --target-dir target
if ($LASTEXITCODE -ne 0) { throw 'Allocation build failed' }
Copy-Item -LiteralPath target/release/laneflow-issue-712.exe -Destination (Join-Path $out 'allocation.exe')
Assert-StableBaseline
