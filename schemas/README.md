...
<!-- schema-source-current: traffic=0.9;spatial=0.1;scenarioManifest=0.1 -->
...
| ----------------- | -------------------------------------------------------------------------------------------- | --------------------------- |
| Traffic           | [`laneflow-data-v0.9.schema.json`](laneflow-data-v0.9.schema.json)                           | 待发布（provenance 已登记） |
...
当前 Traffic production source 是 v0.9，其固定 `main` provenance 已登记到
publication catalog；按 documentation-policy，canonical URL 经 live monitor 证实
HTTP 200 与 byte equality 后方可声称已发布（部署后确认提交回写）。已发布 v0.8
与 v0.7 继续按固定 provenance 永久保留。
当前 schema contract 是：

- [`laneflow-data-v0.9.schema.json`](laneflow-data-v0.9.schema.json)（provenance 已登记，publication 待部署验证）
